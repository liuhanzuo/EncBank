...
the bottom-``j`` writes Encbank already performs: they consume only the cached
``h_j`` tensors and/or the raw token ids, so retrieval adds no extra model
forward and Encbank's compute saving is preserved.
...
selectors match ``eval_qcmem_babilong`` — so Encbank reproduces the published
