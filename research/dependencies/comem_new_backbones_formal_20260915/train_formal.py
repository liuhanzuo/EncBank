"""4,000-step PG-19 suffix distillation; fixed final checkpoint, no eval tuning."""
...
def main():
    ...
    try:
        ...
        recipe = dict(steps=STEPS, window=WINDOW, chunk=CHUNK, context_chunks=7,
                      rank=32, alpha=32, dropout=0, seed=42, initial_skip_windows=8,
                      ...
                      training_tokens=STEPS*WINDOW, checkpoint_selection='fixed final step 4000',
        ...
        for step in range(completed, min(STEPS, args.stop_after or STEPS)):
            ...
            if completed % 250 == 0 or completed == (args.stop_after or STEPS):
                adapter = dict(j=cfg['j'], model=cfg, rank=32, alpha=32, step=completed,
        ...
        if completed == STEPS:
            ...
            save(out/'adapter-final.pt', dict(j=cfg['j'], model=cfg, rank=32, alpha=32,
                 step=completed, recipe=recipe, modules=flat_state(reader)))
            dump(out/'complete.json', dict(complete=True, steps=completed,
