...
base = Path('/srv/encbank/COMem_Migration_20260920')
pe = 'Part1/payload/Paper_Evolve/'
qc = 'Part2/payload/qcomem/'
...
def selected(name, size):
 ...
 if name.startswith(pe+'COMem/'): return True
 ...
 if name.startswith(qc):
  ...
  if str(sub).startswith('paper_autonomous_multifork_iteration/evidence/comem_honly_formal_20260911/runtime/') and p.suffix=='.py': return True
 return False
...
for row in manifest['files']:
    data = (increment / 'qcomem' / row['path']).read_bytes()
