...
class HOnlyEntry:
    ...
    def inventory(self) -> dict:
        ...
        return {
            ...
            'native_BF16_sink_bytes': sink_bytes,
...
class HOnlyMemory:
    def __init__(self, model, tokenizer=None, *, resume_j: int = 12,
        ...
        if bits not in (16, 8, 4) or group_size != 64:
            raise ValueError('This fixed candidate only supports H16/H8/H4 group64')
        ...
        if self.engine.dtype != torch.bfloat16:
            raise ValueError('The qualified reader contract requires native BF16')
    ...
    def encode_ids(self, context_ids, *, document_id: str = '',
        ...
        for index, chunk in enumerate(raw.split(CHUNK_SIZE)):
            ...
            if hidden.dtype != torch.bfloat16 or hidden.device != self.device:
    ...
    def select(self, entry: HOnlyEntry, bare_question_ids) -> tuple[int, ...]:
        ...
        if len(indices) != min(TOPK, len(entry.chunks)) or list(indices) != sorted(set(indices)):
    ...
    def materialize_selected(self, entry: HOnlyEntry, indices: Sequence[int]) -> SelectedRead:
        ...
        if (not indices or len(indices) > TOPK or
            any(type(i) is not int or not 0 <= i < len(entry.chunks) for i in indices) or
            list(indices) != sorted(set(indices))):
            raise ValueError('Selection must be nonempty, unique, in document order, and at most 12 chunks')
    ...
    def generate_ids(self, entry: HOnlyEntry, query_ids, *, bm25_query_ids,
        ...
        return {
            ...
            'precision': f'H{self.bits}; native BF16 BOS sink',
            'stats': stats,
        }
