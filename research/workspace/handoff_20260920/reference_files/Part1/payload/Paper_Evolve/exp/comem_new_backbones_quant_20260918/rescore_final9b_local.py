...
assert all(v['cohort'] == 'Qwen3.5-9B' and v['arm'] == 'cache_r128_s8000_h16' and v['status']=='ok'
...
assert receipt['generation_complete'] and all(v['actual_wait'] and v['returncode']==0 for v in receipt['parent_exits'])
report = dict(at=datetime.datetime.now(datetime.timezone.utc).isoformat(), model='Qwen3.5-9B',j=6,
              ...
              generation_evidence='summary.json; saved_outputs.tar.gz; actual parent_wait and Slurm0:0 for all four shards')
