...
plan=read(B/'full_plan.json');prior=read(D/'heartbeat_babilong_20260909_0931.json')
old={(x['task'],x['length'],x['arm']) for x in prior['carried_forward_baseline_cells']+prior['writeback']}
assert len(old)==56
snapshot={'timestamp':datetime.now(timezone.utc).isoformat(),'host':os.uname().nodename,'ssh_alias':'longjing-1','prior_babi_cells':56,'completed_jobs':[],'incomplete_jobs':[],'invalid_receipts':[]}
...
D.mkdir(parents=True,exist_ok=True);(D/'heartbeat_accuracy_20260909_1032_snapshot.json').write_text(json.dumps(snapshot,indent=2)+'\n')
...
(D/'heartbeat_accuracy_20260909_1032_health.json').write_text(json.dumps(health,indent=2)+'\n')
