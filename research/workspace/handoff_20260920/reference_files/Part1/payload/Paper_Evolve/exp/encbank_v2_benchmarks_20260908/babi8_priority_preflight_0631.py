...
from babi8_priority_bootstrap import R,B,read,alive,verify_scope,verify_predecessor,launch_allowed
...
plan=read(B/'babi8_priority_full_plan.json');main=read(B/'full_plan.json');state=read(R/'outputs/queue_v2/full/state.json')
...
if args.launch:
    ...
    argv=[sys.executable,'-u',str(B/'babi8_priority_bootstrap.py')]
    ...
    report.update(launched=True,bootstrap_pid=child.pid,argv=argv,bootstrap_dir=str(out),status_file=str(out/'status.json'))
name='heartbeat_babi8_priority_20260909_0631_launch.json' if args.launch else 'heartbeat_babi8_priority_20260909_0631_preflight.json'
(R/'outputs/diagnostics'/name).write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report,indent=2))
