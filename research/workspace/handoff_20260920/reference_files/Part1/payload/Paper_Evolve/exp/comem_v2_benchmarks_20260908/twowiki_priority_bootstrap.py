"""Six canonical 2Wiki baseline shards on idle GPU1; leave the main queue intact."""
...
def verify_scope(plan,main_plan):
    ...
    if len(plan['jobs'])!=6 or sum(j['expected_n'] for j in plan['jobs'])!=600:raise RuntimeError('Not the six authorized 2Wiki shards')
    for job in plan['jobs']:
        if job!=originals.get(job['id']):raise RuntimeError('Canonical job changed')
        if job['benchmark']!='longbench' or job['arm'] not in ('pub','pub_sink','cbos') or any(c['task']!='2wikimqa' for c in job['cells']):raise RuntimeError('Unexpected scope')
...
def main():
    ...
    out=R/'outputs/bootstrap_twowiki_priority';out.mkdir(parents=True,exist_ok=True)
    ...
    plan=read(B/'twowiki_priority_full_plan.json');main_plan=read(B/'full_plan.json');verify_scope(plan,main_plan)
    state={'protocol':'twowiki-canonical-priority-v1','pid':os.getpid(),'status':'waiting','gpu':1,'accuracy_only':True,'plan':str(B/'twowiki_priority_full_plan.json'),'canonical_outputs_shared_with_main':True,'jobs':{},'target_jobs':6,'target_predictions':600}
    ...
    try:
        ...
        for number,job in enumerate(plan['jobs']):
            ...
            qdir=R/'outputs/queue_twowiki_priority/full'/job['id'];qdir.mkdir(parents=True,exist_ok=True)
