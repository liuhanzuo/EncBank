"""Local Encbank infrastructure supplement; fixed workloads, no accuracy scoring.

Fresh independent processes use the same unmerged adapter in every arm. Results
belong to this RTX 5090 reproduction, not the historical ARR H20 experiment.
"""
...
def main():
    ...
    admission = acquire_gpu(need_gb=23, cap_gb=26, idle_slack_gb=5, poll=10,
                            max_wait=600, tag='encbank_infra_20260912')
    ...
    meta = {'args': {k: str(v) if isinstance(v, Path) else v for k, v in vars(a).items()},
            ...
            'checkpoint_scope': '2026-09-08 retrained final4000; not established as ARR checkpoint',
    ...
    arms = [('replay_k12', rp, 12), ('replay_k10', rp, 10), ('encbank_k12', cm, 12)]
