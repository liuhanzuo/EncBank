"""Group-level native CoMem/prefix free-QA checks; no loading, queue or timers.
...
def run_native_prefix_group(native,prefix,sink,docs,rows,tokenizer,*,stop_ids,max_new_tokens=128,
    ...
    _require(type(native) is NativeCoMemReader and isinstance(prefix,NativePrefixCoMemReader),
             "Expected native cold and native prefix readers")
    _require(native.model is prefix.model and native.comem is prefix.comem,"Readers must share the exact same model/CoMem")
    ...
    for row in rows:
        ...
        _require(total+max_new_tokens-1<=native.comem.config.max_position_embeddings,"Requested generation exceeds declared model window")
    prefix.invalidate()
