...
def main():
    ...
    for model,info in summary['models'].items():
        ...
        lines += ['', 'CoMem 相比 without LoRA 的增益（依次对应表中四项，百分点）：'+
            '、'.join(f'{deltas[model][b]:+.2f}' for b in BENCHMARKS)+'。','']
    lines += ['## 如何解释','',
        'LoRA 在两个模型的四项评测上均改善 CoMem 的分数，支持适配有助于恢复缓存接口质量。它没有在全部任务上超过其他对照：例如 9B 的四项均低于全源 KV-Direct；27B 的 BABILong 高于全源 KV-Direct，而其他三项仍低于它。不能据此宣称跨任务一致的质量优势，也不能用本次质量实验推导速度优势。','',
        ...
        'Selected-text replay 重放与 CoMem 相同的检索文本；shared LoRA 是将 CoMem adapter 用于 replay，不是另训 replay adapter。KV-Direct 读取完整 source。StreamingLLM-style 是 sink/recent 窗口重算参考；HCache-style 是不检索、不适配的独立 chunk hidden-state 参考，都不宣称复现原系统的全部实现。InfLLM/MemoryLLM 在这两种 backbone 上没有本次可用的兼容实现，因此不编造结果。','',
