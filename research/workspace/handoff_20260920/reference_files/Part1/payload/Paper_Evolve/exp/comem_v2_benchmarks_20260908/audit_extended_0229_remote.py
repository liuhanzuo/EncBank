...
prior=read(B/'heartbeat_extended_20260909_0129.json')
...
d={'timestamp_utc':datetime.now(timezone.utc).isoformat(),'execution':'remote CPU; CUDA hidden; no model construction or GPU tasks',
   'writeback':[],'comparisons':[],'pending':[],'checks':{},'prior_reference':'heartbeat_extended_20260909_0129.json'}
...
for length in sorted({length for arm,length in new_le},key=lambda s:int(s[:-1])):
    targets=[arm for arm,l in new_le if l==length];allarms=targets+['fix_all','j0'];objects={};dbs={};options=None;origins={}
    for arm in allarms:
    ...
    d['comparisons'].append({'benchmark':'longeval','length':length,'scores_percent':scores,'n':50,'source_and_exact_input_generation_keys_equal':True,
...
p=B/'heartbeat_extended_20260909_0229.json';p.write_text(json.dumps(d,indent=2)+'\n')
out=['# 02:29 扩展基线准确率独立 CPU 核查','',
     '相对01:29已核结果，仅检查本轮新增的LongEval/RULER基线完整格及InfiniteBench/LoCoMo覆盖情况。全部评分、源prompt重建、tokenizer与SQLite检查在远端CPU执行，CUDA隐藏，未构造模型；未改论文、同步器、队列或GPU任务。', '',
...
out+=['','`pub`=原始CoMem；`pub_sink`=CoMem+BOS；`cbos`=Full KV隔离缓存控制，不是CacheBlend。0.00是50例全部失败的实测结果；缺失项仍为空。','',
      '## LongEval证据和边界','',
      f"新格及必要V2/j0参照共 **{le_checks}** 次源label/expected/line count、完整token输入与generation参数缓存键及原评分检查通过。每格连续sample_index0–49，completion receipt new+reused=50，实际reader/effective j/sink和共享参数均正确。每个预测只与它自己的精确输入SQLite缓存记录匹配，不要求不同方法输出相同。",'',
...
out+=['','每个已完成shard核查completion与完整索引，跨shard无重复。JSON另列LoCoMo各类别的已完成样本数和官方总数；当前没有任一完整类别。未发布局部均值，也未重新评分尚未完整的任务。上述数字只计完整shard，不包括活动shard已产生但未完成的行。', '',
      '## 紧凑写回建议','',
      '1. 当前Table16(a)保留六长度列：在现有CoMem行补8k实测值，增加+BOS、Full KV两行，只填本轮已核格；其余用dash。V2/j0已有完整两行保持。',
      ...
      '机器结果：`heartbeat_extended_20260909_0229.json`；复算脚本：`audit_extended_0229_remote.py`。文件只新增本轮审计和报告。','']
(B/'heartbeat_extended_20260909_0229.md').write_text('\n'.join(out))
