...
def download(url, path, expected=None):
    ...
    if not reused:
        ...
        request = urllib.request.Request(url, headers={"User-Agent": "CoMem-SCBench-CPU-preparation/1.0"})
...
def main():
    ...
    p.add_argument("--ib-dir", type=Path, default=HERE.parent/"comem_v2_benchmarks_20260908/data/infinitebench")
    p.add_argument("--old-exp", type=Path, default=HERE.parent/"comem_v2_benchmarks_20260908")
