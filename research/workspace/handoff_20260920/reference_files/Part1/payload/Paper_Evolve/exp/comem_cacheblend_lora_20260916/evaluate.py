...
def main():
    ...
    with patch('peft.tuners.lora.model.dispatch_torchao',return_value=None):
        ...
        wrapper.load_adapter(config.PRINCIPAL,adapter_name='comem',is_trainable=False)
    ...
    original.dump(out/'protocol.json',dict(arms=ARMS,samples=500,records=2000,bos=151643,eos=tok.eos_token_id,
        ...
        reference='comem_followups_20260913/run_kv_quality.py; all four arms freshly generated on the same model instance',
        own_adapter=str(config.ROOT/'training/final'),comem_adapter=config.PRINCIPAL,
    ...
    cm=CoMem(model,12,tokenizer=tok);cb=KVControl(CoMem(model,0,tokenizer=tok))
    ...
    with file.open('a',encoding='utf-8') as f:
        for row in rows:
            ...
            for arm in ARMS:
                ...
                wrapper.set_adapter('cacheblend' if is_kv else 'comem')
