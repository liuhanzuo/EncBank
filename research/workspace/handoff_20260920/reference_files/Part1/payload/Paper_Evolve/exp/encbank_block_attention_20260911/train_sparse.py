"""Matched four-arm, answer-only LoRA SFT for the SparseEncbank pilot.

Runs on one explicitly reserved remote GPU. All arms start from the same strong
Encbank LoRA and consume the same prepared QA examples. This is an accuracy and
...
def run(args):
    ...
    del saved
    encbank = Encbank(model, resume_j=args.j)
