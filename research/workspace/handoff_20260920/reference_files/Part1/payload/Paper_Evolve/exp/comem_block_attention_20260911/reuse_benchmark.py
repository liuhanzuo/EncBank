...
def _readers(args, comem):
    from native_infra_readers import FullRecomputeReader, NativeCoMemReader
    from native_prefix_reader import NativePrefixCoMemReader
    from sparse_reader import SparseCoMemReader
    if args.arm == "FULL":
        return FullRecomputeReader(comem), None
    reference = SparseCoMemReader(comem, fusion_layer=args.m,
    ...
    return (NativePrefixCoMemReader(comem), reference) if args.arm in ("D0", "NATIVE") else (reference, reference)
...
def run_reuse_benchmark(args, torch, comem, tokenizer, result, progress):
    ...
    reader, unused_reference = _readers(args, comem)
    del unused_reference
    device = comem.device
    data = workload(tokenizer, comem.config, args)
    chunks, prompts = data["chunks"], data["prompts"]
    probes = data["probe_indices"]
    writer = reader if hasattr(reader, "write_chunk") else comem
    ...
    with torch.no_grad(), _amp(torch, device):
        warm_reader, warm_ref = _readers(args, comem)
        warm_writer = warm_reader if hasattr(warm_reader, "write_chunk") else comem
...
def run_reuse_quality_probe(args, torch, comem, tokenizer, chunks=None, prompts=None, source_ids=None,
                            probe_indices=None):
    ...
    reader, reference = _readers(args, comem)
    ...
    device, tokens = comem.device, args.generation_tokens
    if not 1 <= tokens <= 32:
        raise ValueError("Quality probe generation is bounded to 1..32 tokens")
    if sum(map(len, chunks)) + 1 + max(map(len, prompts)) + tokens > comem.config.max_position_embeddings:
    ...
    with torch.no_grad(), _amp(torch, device):
        ...
        if args.arm in ("D0", "NATIVE"):
            paths["native_cold"] = (NativeCoMemReader(comem), {}, direct_sink, direct_docs)
        previous = {}
        for request_index, prompt_index in enumerate((0, 1, 0)):
            ...
            if args.arm == "FULL":
                ...
                expected, hf_logits = [], []
                output = comem.model(input_ids=inputs, use_cache=True)
                past = output.past_key_values
                for index in range(tokens):
                    if index:
                        output = comem.model(input_ids=torch.tensor([[expected[-1]]], device=device), past_key_values=past, use_cache=True)
                        past = output.past_key_values
