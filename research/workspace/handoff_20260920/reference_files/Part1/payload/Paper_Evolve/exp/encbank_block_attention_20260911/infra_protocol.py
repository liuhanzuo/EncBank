...
GIB = 2 ** 30
MIB = 2 ** 20
VERSION = "sparse-encbank-5090-infra-v1"
GPU_NAME = "NVIDIA GeForce RTX 5090"
INCREMENTAL_CAP_BYTES = 28 * GIB
ALLOCATOR_RESERVE_BYTES = GIB // 2
LABELS = {"D0": "Encbank control (sparse wrapper)", "A": "Block-all", "B": "Block-pruned",
          "D1": "Dense-pruned", "NATIVE": "Native Encbank + LoRA", "FULL": "Full recompute (same upper LoRA)"}
...
def render_report(folder, plan, states):
    ...
    save_json(folder / "INFRA_SUMMARY.json", {"protocol": VERSION, "generated_at": now(), "rows": rows,
        ...
        "pending_baselines": (["full recompute"] if not any(j["arm"] == "FULL" for j in plan) else []) + ["Encbank exact prefix reuse"],
        "128k_status": "pending a common validated model/window configuration; never auto-expanded"})
    text = ["# Encbank + sparse attention: local infrastructure precheck", "",
    ...
    pending = (["Full recompute"] if not any(j["arm"] == "FULL" for j in plan) else []) + ["Encbank exact prefix reuse"]
