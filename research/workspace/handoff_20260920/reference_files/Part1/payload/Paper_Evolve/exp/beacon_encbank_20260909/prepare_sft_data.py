...
def prepare(archive: Path, benchmark_dir: Path, output: Path, train_limit: int = 2000, dev_limit: int = 100, seed: int = 20260909) -> dict:
    ...
    metadata = {"format": "beacon-encbank-qasper-sft-v1", "created_utc": datetime.now(timezone.utc).isoformat(),
...
def main() -> None:
    ...
    parser.add_argument("--benchmark-dir", type=Path, default=ROOT.parent / "encbank_v2_benchmarks_20260908/data/longbench")
