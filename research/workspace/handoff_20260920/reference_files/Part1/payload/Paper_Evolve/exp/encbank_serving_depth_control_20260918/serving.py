...
class Engine:
    ...
    def infer(self,ids,method,count=config.OUTPUT_TOKENS,capture_logits=False):
        ...
        if method!='dense':
            for di,qi in picks:
                ...
                ix=iter_bm25_indices(self.chunks[di],q['query'][:32],12,iter_hop_topk=2,iter_rounds=0)
    ...
    def check(self,out):
        ...
        for method in ['encbank','dense','selected_replay']:
            batch=self.infer([0,1],method,count=2,capture_logits=True)[2]
            singles=[self.infer([i],method,count=2,capture_logits=True)[2] for i in [0,1]]
        ...
        reader=Encbank(self.model,self.j,tokenizer=self.tok)
        ...
        actual=self.infer([0],'encbank',count=2)[2]['generated_ids'][0]
...
def main():
    ...
    out=root/(config.tag(a.j,a.seed) if depth else f'rep{a.rep}')/str(a.length)
    ...
    with sdpa_kernel([SDPBackend.FLASH_ATTENTION,SDPBackend.EFFICIENT_ATTENTION]):
        ...
        engine=Engine(model,tok,a.j,docs)
        atomic_json(out/'write.json',engine.write_records)
        cells=[(m,c) for m in (['encbank'] if depth else ['encbank','dense','selected_replay'])
            for c in ([1] if depth else config.CONCURRENCIES)]
        if a.smoke:cells=[('encbank',2),('selected_replay',4),('dense',1)]
        ...
        for method,c in cells:
            ...
            try:
                ...
                phase='formal'
                count=4 if a.smoke else (96 if depth else config.REQUESTS)
                records,span=closed_loop(lambda ix:engine.infer(ix,method),c,count,config.MAX_BATCH)
