"""CPU-only audit of the newly complete BABILong qa3/2k pair; no model load."""
import os
...
for arm in ('fix_all', 'j0'):
    run = ROOT/'outputs/benchmarks_v2/full/babilong'/arm/'qa3_2k'
    ...
    assert opts['tasks']==['qa3'] and opts['lengths']==['2k'] and opts['max_new_tokens']==20
    ...
    assert [row['id'] for row in rows]==[f'qa3/2k/{i}' for i in range(100)]
    ...
    for i,(row,sample) in enumerate(zip(rows,source)):
        ...
        assert row['task']=='qa3' and row['length']=='2k' and row['output']!='[OOM]'
        value=score_prediction(row['output'],sample,'qa3')
        assert value in (0.,1.) and value==float(row['score'])
        values.append(value)
        marked=format_prompt(dict(sample,input=sample['input'].strip()+BOUNDARY),'qa3')
    ...
    records.append({'benchmark':'babilong','task':'qa3','length':'2k','arm':arm,'n':100,'expected_n':100,
...
result={'timestamp':datetime.now(timezone.utc).isoformat(),'execution':'remote CPU only; CUDA hidden; tokenizer only; no model',
    'writeback':records,'comparisons':[{'benchmark':'babilong','task':'qa3','length':'2k','n':100,
    'v2_minus_j0_pp':scores['fix_all']-scores['j0'],'paired_source_and_pack':True}],
    'protocol':{'data':str(B/'data/babilong/qa3/2k.json'),'prompt':'vendored official DEFAULT_PROMPTS qa3, DEFAULT_TEMPLATE; instruction/examples/post-prompt',
    ...
    'metric':str(B/'vendor/babilong/babilong/metrics.py')+'::compare_answers / TASK_LABELS[qa3]'},
    'limitations':['qa3 requires three supporting facts; this is a 2k diagnostic, not completion of the long-context multi-fact grid.',
...
dest=ROOT/'outputs/diagnostics/heartbeat_accuracy_20260909_0027_babilong.json'
