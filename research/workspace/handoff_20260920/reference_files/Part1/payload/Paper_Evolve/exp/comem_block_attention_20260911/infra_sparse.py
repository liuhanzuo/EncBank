...
ROOT = HERE.parents[1]
for location in (ROOT / "exp", ROOT / "COMem", ROOT / "exp/comem_v2_benchmarks_20260908"):
...
def run(args):
    ...
    identity = {"protocol": VERSION, "model": str(Path(args.model).resolve()),
                ...
                "comem_sha256": digest(ROOT / "COMem/comem/model.py"),
    ...
    try:
        ...
        from sparse_reader import SparseCoMemReader, HotBlock, KVPair
        if args.reader_implementation == "decode_v2":
            from optimized_sparse_reader import OptimizedSparseCoMemReader
            SparseCoMemReader = OptimizedSparseCoMemReader
        elif args.reader_implementation == "backend_v3":
            from backend_sparse_reader import BackendAlignedSparseCoMemReader
            SparseCoMemReader = BackendAlignedSparseCoMemReader
        ...
        result["adapter_metadata"] = {"kind": args.adapter_kind, "step": saved.get("step"),
            "comparison": "same strong CoMem LoRA engineering control" if args.adapter_kind == "strong"
            else "arm-specific trained adapter; quality evaluated separately"}
        del saved
        for parameter in model.parameters():
            parameter.requires_grad_(False)
        comem = CoMem(model, resume_j=args.j, tokenizer=tokenizer)
        if reuse_contract is not None:
            torch.cuda.synchronize()
            result["model_load_s_excluded"] = time.perf_counter() - load_start
            from reuse_benchmark import run_reuse_benchmark
            result = run_reuse_benchmark(args, torch, comem, tokenizer, result, progress)
            save_json(args.out / "result.json", result)
            progress("complete")
            return 0
        if args.arm in ("NATIVE", "FULL"):
            from native_infra_readers import NativeCoMemReader, FullRecomputeReader
            reader = (NativeCoMemReader if args.arm == "NATIVE" else FullRecomputeReader)(comem)
        else:
        ...
        for repetition in range(args.repetitions):
            ...
            with torch.no_grad(), torch.autocast("cuda", dtype=torch.bfloat16):
                ...
                if args.same_math_diagnostic_only:
                    ...
                    reference_reader = OptimizedSparseCoMemReader(comem, fusion_layer=args.m,
