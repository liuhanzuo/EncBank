...
limits=[
 'RULER uses original Encbank adaptation answer-substring recall and legacy metadata/CLI/seed pairing only; exact full input tokens, selected runtime pack and generation-cache keys were not persisted.',
...
negative=[
 'LoCoMo adversarial option accuracy reverses the answerable-category ranking: Encbank130/446=29.1479820628 and BOS255/446=57.1748878924 exceed V2 101/446=22.6457399103 and replay120/446=26.9058295964.',
 'BOS exceeds V2 on adversarial accuracy by34.5291479821pp (160 wins,280 ties,6 losses); original Encbank exceeds V2 by6.5022421525pp. No claim that V2 improves every LoCoMo category is supported.',
 ...
 'At256k single-needle isolated KV100 ties V2/replay100 while BOS94 and original Encbank previously34. Original multi-key256 scores8 versus V2/replay92/100. No universal V2 advantage over isolated KV is supported.'
]
...
md+=['','150条raw通过原Encbank RULER substring-recall复算、连续index0–49、完整summary、无OOM、CLI/seed和准入日志。与旧参考task/length/i/answers/n_tokens一致。单针BOS47/50=94%，isolated50/50=100%；多键原版均值8%。','',
 ...
 '## LoCoMo完整类别','',
 '| Category | n | Encbank | +BOS | V2 (prior) | Replay (prior) |','|---|---:|---:|---:|---:|---:|']
...
md+=['','类别1–4分别为multi-hop、temporal、open-domain、single-hop，使用官方F1；类别5为adversarial，使用明确记录的option-decoding accuracy适配，不能混成一个总均值。','',
 ...
 '完整源13,539–25,686 tokens，read pack5,738–6,252 tokens；完整时间顺序会话及图片caption、原HF短答prompt、Qwen chat模板且thinking关闭、完整query无截断/padding。固定cap50、greedy、首步EOS抑制后自然EOS；原版j12且不写sink，+BOS j12写sink，统一repeat_kv SDPA且禁GQA。它是Encbank检索生成适配，不是未经修改的官方HF生成路径。','',
...
md+=['','**必须保留的负结果：** 四个可回答类别中V2和Replay均高于新两基线；对抗类别则相反：Encbank130/446=29.15%，+BOS255/446=57.17%，V2旧101/446=22.65%，Replay旧120/446=26.91%。+BOS对V2的逐例胜/平/负为160/280/6，原版为72/331/43。新BOS提高了对抗选项准确率这一事实不能被混合总分隐藏，也不能据此直接推出其机制或一般性拒答校准更好。全部20个LoCoMo描述性配对对照均保留在机器JSON，未做显著性检验。','',
 '## 尚缺完整结果及写回建议','',
 ...
 '既有附录LoCoMo块可扩为Category/n/Encbank/+BOS/Isolated KV/V2/Replay七列，在caption说明前四类F1和对抗option accuracy；KV继续空白。不增正文表。RULER仅补本轮三格。正文/附录需明确LoCoMo改进限于四个可回答类别，对抗类别的baseline反超单独写出。','',
