...
METHODS={'kvdirect':'KV-Direct (full source)',
         ...
         'replay_shared_lora':'Replay (shared CoMem LoRA)',
         'streamingllm':'StreamingLLM-style',
         'hcache_style':'HCache-style',
         'cache_without_lora':'CoMem (without LoRA)',
         'cache_lora':'CoMem'}
...
def main():
    ...
    lines=['# Qwen3.5-9B 与 Qwen3.8-27B：五项 benchmark 汇总','',
        '按论文预定任务与长度范围，两个模型的五项 benchmark、七种方法均已完成并核验。共 101,304 条预测，0 OOM、0 未评分。9B 的缓存深度 j=6，27B 为 j=21；两模型的 CoMem adapter 分别训练 4,000 步。','',
    ...
    lines+=['','## 评测范围与解释','',
        ...
        '', 'Selected-text replay 重放与 CoMem 相同的检索文本；shared LoRA 是复用 CoMem adapter，并非单独训练 replay。KV-Direct 使用完整 source。StreamingLLM-style 和 HCache-style 是兼容参考实现。InfLLM 与 MemoryLLM 未纳入这两种 backbone 的本批比较；没有其兼容运行结果，不作填值。本表也不包含量化 H8/H4 配置。',
        '', 'CoMem 的 LoRA 在两模型的五项指标上均改善 without-LoRA 分数，但并非各项都优于其他方法。27B 的 BABILong 为本模型组最高；两模型的五项等权均值均低于完整 source 的 KV-Direct。这些是单训练种子的质量结果，不据此推导显著性或速度优势。',
    ...
    tex=[r'\begin{table*}[t]',r'\centering',r'\small',r'\setlength{\tabcolsep}{4pt}',
        r'\caption{\textbf{Five-benchmark quality on two additional backbones.} CoMem uses $j=6$ for Qwen3.5-9B and $j=21$ for Qwen3.8-27B. Scores are 0--100 ($\uparrow$); Avg. is their descriptive unweighted mean. Bold marks the largest value within each backbone.}',
