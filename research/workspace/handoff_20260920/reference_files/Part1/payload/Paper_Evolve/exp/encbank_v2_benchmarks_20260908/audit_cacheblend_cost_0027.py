...
meta = load(root/'results/protocol/cacheblend_cost_0027/bundle_metadata.json')
...
previous_cb32 = load(root/'heartbeat_cacheblend_cost_20260909_0005.json')
previous_cb32_rows = {(s['tier'],s['G'],s['Q']):s for s in previous_cb32['rows'] if s['arm']=='cacheblend16'}
...
for job in meta['jobs']:
    ...
    for s in summary:
        ...
        if is_cb and n == 32768:
            assert compact == previous_cb32_rows[s['tier'],s['G'],s['Q']]
        rows.append(compact)
...
result={'audited_at_utc':dt.datetime.now(dt.timezone.utc).isoformat(),'success':True,'aggregation_host':'longjing-1 CPU only, stdlib; CUDA not imported',
    ...
    'Earlier V2 and Encbank 32k have legacy gate logs only; do not claim exact structured dual-admission values for them.']}
result['newly_audited_length']=131072
result['newly_audited_cells']=12
result['newly_audited_queries']=400
result['previous_32k_rows_unchanged']=True
result['prototype_peak_limitations']=['The CacheBlend port retains selected GPU payload and merged full-depth KV while cloning the upper 34 layers and holding fresh lower-two-layer KV during selective read.', 'The port computes logits for all selected positions; V2 read_prefill computes only final-position logits.', 'Unused staging and merged KV are released after read; no reference inference or inventory probes occur in formal runs. These are real prototype costs, not an algorithmic memory minimum.', 'Phase-level online inventories do not fully attribute transient allocation peaks.']
...
for n in sorted(configs):
    ...
    result['writeback_summary'].append({'context_tokens':n,'payload_bytes':w['payload_tensor_bytes'],
        ...
        'cells':subset})
out=root/'heartbeat_cacheblend_cost_20260909_0027.json'
