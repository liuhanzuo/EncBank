...
for proc in ([0] if a.smoke else [1,2,3]):
    dest=HERE/('smoke' if a.smoke else 'results')/f'process_{proc:02d}'
    if (dest/'complete.json').exists(): continue
    assert not dest.exists(),'Inspect incomplete output before retrying: '+str(dest)
    cmd=[str(ROOT/'.venv/Scripts/python.exe'),'-u',str(HERE/'bench_dense.py'),
         '--model','/srv/encbank/legacy_workspace/models/Qwen3-8B',
         '--adapter',str(ROOT/'exp/encbank_v2_benchmarks_20260908/checkpoints/8b_j12_pub_4k/final'),
         '--process',str(proc)]
    if a.smoke: cmd.append('--smoke')
    with (HERE/f'process_{proc:02d}.log').open('w',encoding='utf-8') as log:
