...
OUT = ROOT / "outputs/diagnostics/heartbeat_babilong_20260909_1132_final.json"
PRIOR = ROOT / "outputs/diagnostics/heartbeat_babilong_20260909_0129.json"
BASELINE_PRIOR = ROOT / "outputs/diagnostics/heartbeat_babilong_20260909_1132.json"
...
baseline_prior = read(BASELINE_PRIOR)
require(not baseline_prior["validation_errors"], "Previous baseline audit failed")
already_audited = {(x["task"], x["length"], x["arm"]): x for x in baseline_prior.get("carried_forward_baseline_cells", []) + baseline_prior["writeback"]}
snapshot=read(ROOT / "outputs/diagnostics/heartbeat_babilong_20260909_1132_final_snapshot.json")
snapshot_keys={(j["cells"][0]["task"],j["cells"][0]["length"],j["arm"]) for j in snapshot["completed_jobs"] if j["benchmark"]=="babilong"}
require(len(already_audited)==77,"Prior77 baseline cells expected")
ready, pending = {}, []
for task in TASKS:
    for length in LENGTHS:
        for arm in ARMS:
            if (task, length, arm) in already_audited:
                continue
            run = ROOT / "outputs/benchmarks_v2/full/babilong" / arm / f"{task}_{length}"
            receipt = read(run / "COMPLETED.json") if (task,length,arm) in snapshot_keys else None
            ...
            if receipt and receipt.get("status") == "completed" and n == 100:
                ...
                pending.append({"task": task, "length": length, "arm": arm, "run": str(run),
                                "status": "not_complete_n100_in_fixed_snapshot", "completed_receipt_records": n,
                                "score_percent": None})
...
require(sum(map(len,ready.values()))==7,"Expected seven new cells in independent FINAL snapshot")
...
result = {"timestamp": datetime.now(timezone.utc).isoformat(), "scope": "New complete BABILong pub/pub_sink/cbos method cells only", "snapshot_timestamp":snapshot["timestamp"],
          "prior_v2_j0_audit": str(PRIOR), "prior_v2_j0_recomputed": False,
          "prior_baseline_audit": str(BASELINE_PRIOR), "prior_baselines_recomputed": False,
          "carried_forward_baseline_cells": list(already_audited.values()),
          ...
          "protocol": {"model": "Qwen3-8B BF16, frozen base weights", "requested_j": 12,
                       ...
                       "reader_class": {"pub": "CoMem", "pub_sink": "CoMem", "cbos": "CoMemLower"},
                       ...
                       "generation_cap": 20, "generation_cap_scope": "CoMem benchmark setting, not a universal BABILong cap",
...
result.update(completed_new_baseline_cells=len(result["writeback"]), newly_audited_predictions=100*len(result["writeback"]),
              pending_baseline_cells=len(pending),
              total_complete_baseline_cells=len(already_audited)+len(result["writeback"]),
              all_new_baseline_grid_complete=len(already_audited)+len(result["writeback"]) == 84,
...
cumulative = list(already_audited.values()) + result["writeback"]
result["base_condition_panel"] = []
for task in TASKS:
    base = {a: None for a in ARMS}
    for row in cumulative:
        if row["task"] == task and row["length"] == "0k":
            base[row["arm"]] = row["score_percent"]
    result["base_condition_panel"].append({"task":task,"length":"0k","n_per_complete_method":100,
        "baseline_scores_percent":base,"reference_scores_percent":prior_by_cell[task,"0k"]["scores_percent"],
        "all_five_methods_complete":all(v is not None for v in base.values())})
