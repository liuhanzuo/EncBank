"""Six canonical BABI32 baseline shards on idle GPU1; leave the main queue intact."""
...
def verify_scope(plan,main_plan):
    ...
    if len(plan['jobs'])!=6 or sum(j['expected_n'] for j in plan['jobs'])!=600:raise RuntimeError('Not the six authorized BABI32 cells')
    actual=set()
    for job in plan['jobs']:
        ...
        cell=job['cells'][0]
        if cell['task'] not in ('qa2','qa3') or cell['length']!='32k' or cell['indices']!=list(range(100)) or job['expected_n']!=100:raise RuntimeError('Unexpected native dataset cell')
...
def main():
    ...
    out=R/'outputs/bootstrap_babi32_priority';out.mkdir(parents=True,exist_ok=True)
    ...
    plan=read(B/'babi32_priority_full_plan.json');main_plan=read(B/'full_plan.json');verify_scope(plan,main_plan)
    state={'protocol':'babi32-canonical-priority-v1','pid':os.getpid(),'status':'waiting','gpu':1,'accuracy_only':True,'plan':str(B/'babi32_priority_full_plan.json'),'canonical_outputs_shared_with_main':True,'jobs':{},'target_jobs':6,'target_predictions':600}
    ...
    try:
        ...
        for number,job in enumerate(plan['jobs']):
            ...
            qdir=R/'outputs/queue_babi32_priority/full'/job['id'];qdir.mkdir(parents=True,exist_ok=True)
