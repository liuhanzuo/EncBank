"""Single-GPU answer-only SFT for Beacon residuals and matched Encbank controls.
...
from beacon_encbank import BeaconEncbank, BeaconMemory


class SFTModel(BeaconEncbank):
    ...
    def __init__(self, model, *, mode, split, compression_ratio, sink_token_id,
        ...
        self.mode = mode
        if mode not in {"beacon", "encbank", "pool"}:
            raise ValueError("Unknown SFT arm")
        if mode == "encbank" and compression_ratio != 1:
    ...
    def answer_loss(self, document_chunks, prompt_ids, answer_ids):
        ...
        # Encbank's checkpoint loop checks the input's requires_grad once. A tiny
...
def main():
    ...
    ap.add_argument("--mode", choices=["beacon", "encbank", "pool"], required=True)
