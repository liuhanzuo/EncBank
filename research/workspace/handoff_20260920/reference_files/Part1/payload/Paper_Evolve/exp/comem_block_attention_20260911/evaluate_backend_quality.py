...
def source_hashes():
    ...
    paths.update({"evaluate_sft.py":HERE.parent/"beacon_comem_20260909"/"evaluate_sft.py",
        "train_8b_baseline.py":HERE.parent/"comem_v2_benchmarks_20260908"/"train_8b_baseline.py",
        "COMem/comem/model.py":HERE.parents[1]/"COMem"/"comem"/"model.py"})
...
def run(args,lease_receipt):
    ...
    model=readers=comem=sink=memories=None
    ...
    try:
        ...
        comem=CoMem(model,resume_j=12)
        kwargs=dict(fusion_layer=16,retain_ratio=1.,probe_mode="dense",gradient_checkpointing=False)
        readers={"decode_v2":OptimizedSparseCoMemReader(comem,**kwargs).eval(),
                 "backend_v3":BackendAlignedSparseCoMemReader(comem,**kwargs).eval()}
        ...
        memories=sink=readers=comem=model=None
        gc.collect()
