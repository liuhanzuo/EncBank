...
def summarize(folder, completed_native_from=None):
    ...
    if completed_native_from is not None:
        text += ["", f"原生CoMem复用此前完整诊断：{completed_native_from}。此前D0因监督进程心跳文件替换失败而被watchdog终止，无完整profile结果；未当作OOM。该失败保留原目录，修复Windows共享读和异常收尾后仅重试D0/B三项。只有监控实现改变，模型/reader/profiler保持原配置；不是跨版本正式速度比较。"]
