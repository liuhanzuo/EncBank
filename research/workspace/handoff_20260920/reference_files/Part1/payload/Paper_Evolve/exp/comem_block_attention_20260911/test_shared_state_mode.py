...
import shared_state_protocol as protocol
from compare_decode_infra import collect


class SharedStateModeChecks(unittest.TestCase):
    def setUp(self):
    ...
    def argv(self):
        return ['--shared-state-diagnostic-only', '--shared-state-cpu-receipt', str(self.receipt),
    ...
    def test_one_bounded_diagnostic_and_remote_cpu_contract_required(self):
        ...
        with patch.object(protocol, 'digest', side_effect=lambda p: 'stub:' + Path(p).name):
            self.assertEqual(protocol.validate_shared_mode(args), self.data)
        for changed in (dict(passed=False), dict(device='cuda'), dict(cuda_visible_devices='0'),
            ...
            with self.subTest(changed=changed):
                with self.assertRaises(ValueError):
                    protocol.validate_shared_mode(args)
    ...
    def test_receipt_does_not_enable_formal_measurement(self):
        args = launcher.parser().parse_args(['--shared-state-cpu-receipt', str(self.receipt)])
        with self.assertRaisesRegex(ValueError, 'only to diagnostic'):
            protocol.validate_shared_mode(args)
        folder = self.root / 'run'
        attempt = folder / 'case/attempts/0001'
        ident = dict(shared_state_diagnostic_only=True)
        result = dict(status='complete', identity=ident, timing_eligible=False,
                      requests=[], summary={}, shared_state_receipt=dict(status='complete', protocol_checks_passed=True))
        ...
        with self.assertRaisesRegex(ValueError, 'invalid measurement'):
            collect(folder)
        self.assertIsNone(launcher.find_prior(folder / 'case', dict(shared_state_diagnostic_only=False)))
        result['shared_state_receipt']['protocol_checks_passed'] = False
        self.write(attempt / 'result.json', result)
    ...
    def test_dry_diagnostic_never_starts_worker_or_renders_infra_table(self):
        ...
        self.assertTrue(state['shared_state_diagnostic_only'])
