...
OUT = ROOT / "outputs/diagnostics/heartbeat_babilong_20260909_1132.json"
PRIOR = ROOT / "outputs/diagnostics/heartbeat_babilong_20260909_0129.json"
BASELINE_PRIOR = ROOT / "outputs/diagnostics/heartbeat_babilong_20260909_1032.json"
...
snapshot=read(ROOT / "outputs/diagnostics/heartbeat_accuracy_20260909_1132_snapshot.json")
snapshot_keys={(j["cells"][0]["task"],j["cells"][0]["length"],j["arm"]) for j in snapshot["completed_jobs"] if j["benchmark"]=="babilong"}
require(len(already_audited)==58,"Prior58 baseline cells expected")
ready, pending = {}, []
for task in TASKS:
...
require(sum(map(len,ready.values()))==19,"Expected nineteen new cells in fixed snapshot")
