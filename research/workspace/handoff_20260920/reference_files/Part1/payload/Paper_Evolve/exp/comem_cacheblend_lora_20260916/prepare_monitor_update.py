...
automation=Path('/srv/encbank/client/.codex/automations/comem-followups-progress-3h/automation.toml')
...
NEW authorized independent CacheBlend-style baseline: local F:/Paper_Evolve/exp/comem_cacheblend_lora_20260916, remote /srv/encbank/comem_cacheblend_lora_20260916 on gpu-node1. Run its collect.py and inspect STATUS.json. Pipeline job PIPELINE, dependent CPU release job RELEASE, both already submitted. Do not deploy or submit duplicates. Read PROTOCOL_zh.md. CPU tests passed locally and remotely before submission; pipeline also checks actual8B forward parity initially and after training. This is a differentiable version of the prior two-bootstrap-layer fixed15% HF CacheBlend-style control, not the native engine. Cached writer and sparse repair both receive gradients; backbone frozen. Fresh suffix LoRA12-35, rank/alpha32,168modules/58,195,968parameters. Same exact PG19 64books/7,231,927token stream as principal CoMem, same initial skip8windows,4096tokens/step,seed42,teacher top64 support-normalized bidirectional KL .6/.4,AdamW1e-4/warmup50/cosine/clip1,4000steps/16.384Mprocessedtokens. Preserve original training tokenizer-BOS-else-EOS and evaluationBOS151643; actual runtime versions recorded. Matched scientific settings/budget, not equal FLOPs/hours. Never initialize from CoMem adapter or tune ratio/checkpoint on test outcomes.

After training, same pipeline generates four fresh arms on exact prior500examples (LongEval8/16/32k100each,budget16; Qasper200,budget128): CacheBlend frozen, CacheBlend own LoRA, CoMem frozen, CoMem principal own LoRA. Identical saved tokens/retrieval/order,chunk512/top12,plain prompts/greedy/scorers. 2000records total; report.py independently decodes/rescores complete support. This support is the paired KV appendix experiment, not all five main-table benchmarks. Do not conflate LongEval16 here with newer-model48. Failure JSONs and pipeline logs must be inspected; if a real implementation defect occurs, fix only the defect without changing method/budget/samples, preserve failure evidence, and resume last250-step checkpoint with restored optimizer/RNG/cursor. Before any recovery submission, inspect live jobs and current array throttles: the CPU release job may already have restored4 after failure, so reserve a fourth slot safely again.
