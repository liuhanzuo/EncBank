"""
gpu_gate -- the ONLY way GPU jobs in exp/ may start.

Why this exists (incident, 2026-09-05 13:59): two background chains each ran their own
"is the GPU idle?" poll.  One used torch.cuda.mem_get_info, which on Windows/WDDM counts
memory that the driver could page out of OTHER processes as free; it reported 31 GB free
while an 8B RULER eval held the card, loaded a second Qwen3-8B on top, and both jobs
thrashed (per-sample time went from ~10 s to ~250 s).  The owner's rule is: never start
while another job is on the card.

Three guards, all mandatory:
  1. IDLE CHECK from nvidia-smi (memory.used < idle_slack_gb AND no python compute
     process), never from torch.  nvidia-smi reports the real residency.
     The admission threshold has a hard ceiling of 5.0 GiB, including legacy callers.
  2. LOCK FILE exp/results/.gpu.lock holding {pid, cmd, t}.  A second gate waits while
     the lock's pid is alive, so two chains can never pass the idle check in the same
     gap.  Stale locks (dead pid) are removed.  The lock is released at exit.
  3. PER-PROCESS CAP via torch.cuda.set_per_process_memory_fraction (in-process use
     only) so an over-allocation raises in OUR process instead of evicting someone else.

Use in a script:
    from gpu_gate import acquire_gpu
    acquire_gpu(need_gb=22, cap_gb=28)      # blocks until idle + locked, then caps torch

Use as a wrapper for code you do not own (holds the lock for the child's lifetime):
    python exp/gpu_gate.py --need-gb 22 -- python -m eval.ruler --model ... --j 12
"""
