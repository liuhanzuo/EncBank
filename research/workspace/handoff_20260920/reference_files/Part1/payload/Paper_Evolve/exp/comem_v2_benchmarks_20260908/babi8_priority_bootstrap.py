"""Six canonical BABI8 baseline shards on idle GPU1; leave the main queue intact."""
...
def verify_scope(plan,main_plan):
    ...
    if len(plan['jobs'])!=6 or sum(j['expected_n'] for j in plan['jobs'])!=600:raise RuntimeError('Not the six authorized BABI8 cells')
    actual=set()
    for job in plan['jobs']:
        ...
        cell=job['cells'][0]
        if cell['task'] not in ('qa2','qa3') or cell['length']!='8k' or cell['indices']!=list(range(100)) or job['expected_n']!=100:raise RuntimeError('Unexpected native dataset cell')
...
def main():
    ...
    out=R/'outputs/bootstrap_babi8_priority';out.mkdir(parents=True,exist_ok=True)
    ...
    plan=read(B/'babi8_priority_full_plan.json');main_plan=read(B/'full_plan.json');verify_scope(plan,main_plan)
    state={'protocol':'babi8-canonical-priority-v1','pid':os.getpid(),'status':'waiting','gpu':1,'accuracy_only':True,'plan':str(B/'babi8_priority_full_plan.json'),'canonical_outputs_shared_with_main':True,'jobs':{},'target_jobs':6,'target_predictions':600,'after_completion':'exit after these six canonical jobs; no automatic next workload'}
    ...
    try:
        ...
        for number,job in enumerate(plan['jobs']):
            ...
            qdir=R/'outputs/queue_babi8_priority/full'/job['id'];qdir.mkdir(parents=True,exist_ok=True)
