"""LongBench/LoCoMo official metrics with a controlled CoMem retrieval protocol.
...
This is an explicitly named CoMem adaptation, not an unchanged HF benchmark run.
...
for path in (ROOT / "COMem", ROOT / "exp", HERE):
...
BOUNDARY = "\n<COMEM_CONTEXT_QUERY_BOUNDARY_20260908>\n"
...
def main(argv=None):
    ...
    files = [Path(__file__), HERE / "reader_adapter.py", ROOT / "exp/s15_ruler_lower.py",
        ROOT / "COMem/comem/model.py", ROOT / "COMem/comem/selectors.py"]
    if args.arm == "cacheblend16":
        files += [HERE / "cacheblend_contextual.py", ROOT / "COMem/comem/cacheblend.py"]
    ...
    config = {"schema_version": 1, "benchmark": args.benchmark, "arm": args.arm,
        ...
        "protocol": {"source_context": "whole source; no truncation", "query": "explicit complete query",
            ...
            "decoding": "greedy; native CoMem first-token EOS suppression; official per-task token caps",
