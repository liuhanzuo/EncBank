"""Run an existing benchmark with an explicitly selected CoMem reader.
...
for folder in (ROOT / "COMem", ROOT / "exp", HERE, HERE / "vendor" / "babilong"):
...
def main(argv=None):
    ...
    sources = [Path(module.__file__), HERE / "reader_adapter.py", Path(__file__),
               ROOT / "exp" / "s15_ruler_lower.py", ROOT / "COMem" / "comem" / "model.py",
               ROOT / "COMem" / "comem" / "selectors.py", ROOT / "COMem" / "eval" / "_common.py"]
    ...
    if wrapper.arm == "cacheblend16":
        sources.extend([HERE / "cacheblend_contextual.py", ROOT / "COMem/comem/cacheblend.py"])
    ...
    with OutputLock(output):
        ...
        try:
            with ExitStack() as stack:
                ...
                stack.enter_context(patch.object(module, "CoMem", make_reader_factory(wrapper.arm, cache, constructed, explicit_pack=explicit)))
