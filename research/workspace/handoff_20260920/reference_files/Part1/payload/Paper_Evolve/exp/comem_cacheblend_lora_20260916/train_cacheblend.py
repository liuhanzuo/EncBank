"""Train a fresh CacheBlend-style suffix LoRA with the exact CoMem recipe."""
...
def parity(reader,tok):
    ...
    ref=KVControl(CoMem(reader.model,0))
...
def main():
    ...
    original=json.loads((config.ROOT/'comem_training_metadata.json').read_text())
    ...
    metadata=dict(recipe=config.RECIPE,source=data,matched_recipe_keys=list(comparable),training_bos=bos,
        ...
        trained_for='CacheBlend-style independent writer and sparse repair, not transferred CoMem weights')
    completed=0
    if a.resume:
