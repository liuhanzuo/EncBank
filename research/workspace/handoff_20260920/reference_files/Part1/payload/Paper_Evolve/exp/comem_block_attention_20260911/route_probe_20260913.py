...
def reader_type():
    ...
    class RouteReader(SparseCoMemReader):
...
def main():
    ...
    for arm, ident in CASES:
        ...
        if arm != current_arm:
            checkpoint = root/f'outputs/sparse_comem_20260911/train/{arm}/step250.pt'
            ...
            current_arm = arm
        row = rows[ident]
        reader = Reader(comem, fusion_layer=16, retain_ratio=.5,
        ...
        with torch.no_grad():
            ...
            sink = comem.write_chunk([sink_id]).detach()
        with torch.no_grad(), torch.autocast('cuda', dtype=torch.bfloat16):
            memories = [comem.write_chunk(c).detach() for c in row['document_chunks']]
            ...
            del forced
            rebuilt = [comem.write_chunk(c).detach() for c in row['document_chunks']]
        ...
        original = json.loads((root/f'outputs/sparse_comem_20260911/train/{arm}/eval_step250.json').read_text())
