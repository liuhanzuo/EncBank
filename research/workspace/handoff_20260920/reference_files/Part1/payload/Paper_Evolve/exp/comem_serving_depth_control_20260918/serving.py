...
class Engine:
    ...
    def check(self,out):
        ...
        for method in ['comem','dense','selected_replay']:
...
def main():
    ...
    with sdpa_kernel([SDPBackend.FLASH_ATTENTION,SDPBackend.EFFICIENT_ATTENTION]):
        ...
        cells=[(m,c) for m in (['comem'] if depth else ['comem','dense','selected_replay'])
            for c in ([1] if depth else config.CONCURRENCIES)]
        if a.smoke:cells=[('comem',2),('selected_replay',4),('dense',1)]
