...
for folder in (ROOT / "Encbank", ROOT / "exp"):
    sys.path.insert(0, str(folder))

import torch
from transformers.cache_utils import DynamicCache
from encbank import Encbank
from encbank import selectors as selectors
from s15_ruler_lower import EncbankLower  # Same attention-kernel policy for all arms.
...
class ReusableReader:
    def __init__(self, model, j, tokenizer, arm="fix_all", model_id="unspecified", adapter_info=None):
        ...
        self.cm = EncbankLower(model, j, tokenizer) if arm == "fix_all" else Encbank(
