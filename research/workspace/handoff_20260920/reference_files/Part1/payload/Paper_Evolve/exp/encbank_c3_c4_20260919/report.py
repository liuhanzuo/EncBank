...
def main():
    ...
    for method in ['selected_replay','encbank']:
        for c in [1,4]:
    ...
    text+='## C3：同一参考服务承接并发请求\n\n同一RTX5090、Qwen3-8B、共享rank32/alpha32/final4000 adapter；每格独立模型进程，一份权重。FIFO同步微批，最多4请求/批，独立batch行KV状态，不是连续batching。3个32k PG19文档、96个固定续写请求模板重复构造负载，固定重复顺序、无答案/前缀缓存。BM25索引和H bank离线准备，实际迭代检索在线执行。Encbank j12/w0/CPU-pinned H。CPU线程2、CUDA allocator cap27GiB。\n\n'
    text+='| 方法 | 并发 | 完成/失败 | req/s | TTFT p95/p99 ms | E2E p95/p99 ms | allocated/reserved峰值 GiB |\n|---|---:|---:|---:|---:|---:|---:|\n'
    for r in serving:
        s=r['result'];name='Raw replay' if r['method']=='selected_replay' else 'Encbank'
        if s['status']=='ok':
    ...
    encbank4=next(x['result'] for x in serving if x['method']=='encbank' and x['concurrency']==4)
    if raw4['status']==encbank4['status']=='ok':
        text+=f"\nRaw replay 对同样检索出的文本块重新执行全部层；32k指源文档长度。此参考负载下，并发4时Encbank的请求吞吐提升{100*(encbank4['requests_per_s']/raw4['requests_per_s']-1):.2f}%，TTFT p99降低{100*(1-encbank4['ttft_s']['p99']/raw4['ttft_s']['p99']):.2f}%，E2E p99降低{100*(1-encbank4['e2e_s']['p99']/raw4['e2e_s']['p99']):.2f}%。\n"
    ...
    text+='| j | 参数 M | 输入/计损失 tokens | Multikey raw/Encbank | Qasper raw/Encbank | Encbank Write ms | Encbank Read ms | 在线 TTFT raw/Encbank ms |\n|---:|---:|---:|---:|---:|---:|---:|---:|\n'
    for q,t in zip(quality,latency):
        s=q['scores'];a=t['arms']
        text+=f"| {q['j']} | 43.647 | 16384000/2048000 | {s['raw']['multikey']:.2f}/{s['encbank']['multikey']:.2f} | {s['raw']['qasper']:.2f}/{s['encbank']['qasper']:.2f} | {a['encbank']['write_ms']:.2f} | {a['encbank']['read_ms']:.2f} | {a['raw']['ttft_ms']:.2f}/{a['encbank']['ttft_ms']:.2f} |\n"
    text+='\n在相同适配层集合、参数量与数据/步数下，更深的切分仍使本次multikey质量下降，同时降低在线读取延迟、增加离线Write成本。Qasper的Encbank F1也随深度下降，但raw参考本身随独立adapter变化，需按各自配对结果解释。该共同布局控制与主实验的suffix-adapter布局不同，不替换原部署结果，也不证明j=12是通用最优深度。\n'
