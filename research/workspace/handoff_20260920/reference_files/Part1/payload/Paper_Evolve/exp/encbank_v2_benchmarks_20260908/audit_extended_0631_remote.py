...
md+=['',f"合计{len(d['writeback'])}个新格、{sum(c['n'] for c in d['writeback'])}个新预测。pub=原始Encbank，pub_sink=Encbank+BOS，cbos=isolated full KV控制，不是CacheBlend。实测0保留0，未知分数保持空白。",'',
    ...
    '统一stock Qwen3-8B BF16，SDPA且repeat_kv/禁GQA，j12（cbos有效j36），chunk512、BM25 top12、seed1234、cap16。这里是Encbank-adapted LongEval的completion prompt与首数字精确匹配，不能称为官方LongChat prompt/scorer。', '',
...
md+=['','## RULER：完整raw重评分，保留legacy限制','',
    f"{ruler_checks}个新输出按原Encbank RULER answer-substring recall重评分，50例/格与summary一致；快照job completed/exit_code0、连续i=0–49、非OOM、CLI/reader参数和准入日志均通过。与已核V2/j0逐例task/length/i/answers/n_tokens一致；旧参考输出不重新评分。",'',
