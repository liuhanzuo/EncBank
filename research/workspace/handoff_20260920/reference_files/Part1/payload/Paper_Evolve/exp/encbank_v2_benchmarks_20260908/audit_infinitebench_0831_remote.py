"""One newly complete InfiniteBench En.MC baselines; CPU, no model loading."""
import os
...
task='longbook_choice_eng';n=229;targets=('cbos',);references=('fix_all','j0')
...
d={'timestamp_utc':datetime.now(timezone.utc).isoformat(),'scope':'new cbos English long-book MC only; V2/j0 metadata and packs used as previously scored reference, not rescored',
...
for i,src in enumerate(ib.iter_task(task)):
    ...
    if (i+1)%25==0:print(f'IB MC one-arm official/source/token/pack/cache checks {i+1}/{n}',flush=True)
assert len(keys)==n
...
d['protocol']={'model':'stock Qwen3-8B BF16, SDPA, same repeat_kv policy; j12, isolated full KV effective j36',
    ...
    'coverage':'One complete English long-book MC task for one newly complete isolated-KV arm, not full InfiniteBench; all three MC baseline tasks now complete',
    'token_ranges':{k:{'min':min(v),'max':max(v)} for k,v in ranges.items()}}
d['checks']={'official_rescores':229,'new_source_token_pack_exact_input_own_cache_checks':229,'reference_scorer_calls':0,'reference_cache_lookups':0,
...
dest=B/'heartbeat_infinitebench_20260909_0831_new.json';dest.write_text(json.dumps(d,indent=2)+'\n')
