"""Native Encbank whole ordered upper-prefix reuse, with private query-only KV.
...
from native_infra_readers import NativeEncbankReader,NativeQueryState
...
class NativePrefixEncbankReader(NativeEncbankReader):
    """Exact causal graph reuse for one complete ordered sink+document pack."""
    implementation="native-encbank-whole-upper-prefix-query-only-cache"

    def __init__(self,encbank):
        super().__init__(encbank)
        self._prefix=None
        self._epoch=0
        self.build_count=0
        self.hit_count=0
    ...
    def _check_contract(self):
        ...
        if self.model is not cm.model or not 0<self.j<self.L:
            raise ValueError("Fixed Encbank/model binding with0<j<L required")
        ...
        if first.device!=cm.device or first.dtype!=cm.dtype:
            raise ValueError("Encbank cached device/dtype is stale; construct a new wrapper")
    ...
    def _signature(self,sink,documents,position_offset):
        ...
        for tensor in tensors:
            if (not torch.is_tensor(tensor) or tensor.ndim!=3 or tensor.shape[0]!=1
                    or tensor.shape[1]<1 or tensor.shape[2]!=self.encbank.hidden_size):
    ...
    def build_prefix(self,sink,documents,*,position_offset=0):
        ...
        if count>self.encbank.config.max_position_embeddings:
        ...
        self._prefix=None
        cache=DynamicCache(config=self.encbank.config)
        if count:
            packed=torch.cat(pieces,dim=1)
            positions=torch.arange(count,device=self.encbank.device).unsqueeze(0)
            mask,rope=self.encbank._make_mask_and_rope(packed,positions)
            self.encbank._run_layers(packed,slice(self.j,self.L),mask,positions,rope,
                                   past_key_values=cache,use_cache=True)
        pairs={i:PrefixKV(cache.layers[i].keys,cache.layers[i].values) for i in range(self.j,self.L)} if count else {}
        for pair in pairs.values():
            if pair.k.shape[1]!=self.encbank.config.num_key_value_heads or pair.v.shape[1]!=self.encbank.config.num_key_value_heads:
    ...
    def prefill(self,sink,documents,prompt,probe_indices=None,*,position_offset=0):
        documents=list(documents)
        signature=self._signature(sink,documents,position_offset)
        prompt_ids=self.encbank._as_ids(prompt)
        prompt_length=int(prompt_ids.shape[1])
        prefix_length=sum(int(t.shape[1]) for t in documents)+(0 if sink is None else int(sink.shape[1]))
        if prompt_length<1 or prefix_length+prompt_length>self.encbank.config.max_position_embeddings:
        ...
        if not hit:
            ...
            self.hit_count+=1
        q_h,bottom,q_position=self.encbank.write_prefill(prompt_ids)
        if q_position<1 or prefix.token_count+q_position>self.encbank.config.max_position_embeddings:
            raise ValueError("Nonempty query within declared model window required")
        top=PrefixQueryCache(prefix,self.encbank.config,self.j)
        positions=torch.arange(prefix.token_count,prefix.token_count+q_position,device=self.encbank.device).unsqueeze(0)
        mask=create_causal_mask(config=self.encbank.config,inputs_embeds=q_h,attention_mask=None,
                                past_key_values=top,position_ids=positions,layer_idx=self.j)
        rope=self.encbank.rotary_emb(q_h,position_ids=positions)
        hidden=self.encbank._run_layers(q_h,slice(self.j,self.L),mask,positions,rope,past_key_values=top,use_cache=True)
        logits=self.encbank.lm_head(self.encbank.norm(hidden[:,-1:,:]))
    ...
    def decode_step(self,token_id,state):
        self._check_contract()
        self._assert_request(state)
        window=self.encbank.config.max_position_embeddings
