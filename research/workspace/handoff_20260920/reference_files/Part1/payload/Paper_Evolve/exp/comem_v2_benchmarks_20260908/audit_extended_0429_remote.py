...
prior={'writeback':read(B/'heartbeat_extended_20260909_0129.json')['writeback']+read(B/'heartbeat_extended_20260909_0229.json')['writeback']+read(B/'heartbeat_extended_20260909_0329.json')['writeback']}
...
d={'timestamp_utc':datetime.now(timezone.utc).isoformat(),'execution':'remote CPU; CUDA hidden; no model construction or GPU tasks',
   'writeback':[],'comparisons':[],'pending':[],'checks':{},'prior_reference':['heartbeat_extended_20260909_0129.json','heartbeat_extended_20260909_0229.json','heartbeat_extended_20260909_0329.json']}
...
d['narrative_review']={'body_change_required':False,'current_comparator':'full recompute',
    ...
    'new_full_IB_requires_second_stage_audit':True}
p=B/'heartbeat_extended_20260909_0429.json';p.write_text(json.dumps(d,indent=2)+'\n')
out=['# 04:29 扩展基线准确率独立 CPU 核查','',
     '相对03:29已核结果，仅检查本轮新增的LongEval/RULER基线完整格及InfiniteBench/LoCoMo覆盖情况。全部评分、源prompt重建、tokenizer与SQLite检查在远端CPU执行，CUDA隐藏，未构造模型；未改论文、同步器、队列或GPU任务。', '',
...
out+=['','## RULER证据和边界','',
      ...
      '## InfiniteBench和LoCoMo覆盖：IB完整组等待第二阶段官方核验','',
...
out+=['','每个已完成shard核查completion与完整索引，跨shard无重复。JSON另列LoCoMo各类别的已完成样本数和官方总数；LoCoMo当前没有任一完整类别。InfiniteBench新完整组交第二阶段官方评分和精确输入核验；未发布局部均值。上述数字只计完整shard，不包括活动shard已产生但未完成的行。', '',
      '## 紧凑写回建议','',
      '1. 当前Table16(a)保留现有方法行和六长度列，只补本轮32k/64k新格；其余用dash。V2/j0已有完整两行保持。',
      '2. RULER的新基线只有4k等少量格，放在原RULER附录文字或一个紧凑4k行组：列Original/+BOS/Full KV/V2/full recompute。缺失值dash，不扩大正文表，不将cbos命名为CacheBlend。',
      '3. Table16的InfiniteBench等待第二阶段正式全组核验结果；LoCoMo新baseline未完整，不能加入局部分数。',
      ...
      '机器结果：`heartbeat_extended_20260909_0429.json`；复算脚本：`audit_extended_0429_remote.py`。文件只新增本轮审计和报告。','']
