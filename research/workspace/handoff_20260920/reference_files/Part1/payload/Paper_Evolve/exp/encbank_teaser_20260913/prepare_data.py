...
OUT=ROOT/'Encbank/paper_iclr2027_rewrite_20260912/figures/teaser_data.json'
def read(rel): return json.loads((ROOT/rel).read_text(encoding='utf-8'))
short=read('exp/encbank_dense5090_20260913/short_summary.json')
long=read('exp/encbank_dense5090_20260913/summary.json')
e2e=read('exp/encbank_e2e_20260913/summary.json')
assert short['complete'] and long['complete'] and e2e['complete']
data={}
for n in (8192,16384): data[str(n)]=short['summary'][str(n)]
for n,folder in [(32768,'encbank_infra_20260912'),(131072,'encbank_infra_128k_20260912')]:
    cm=read(f'exp/{folder}/summary.json')['summary']['encbank_k12']
    data[str(n)]={'dense':long['summary'][str(n)],'encbank':{
        'prefill':{'status':'ok',**cm['read']},'ttft':{'status':'ok',**cm['ttft']},
        'e2e':{'status':'ok',**e2e['summary'][str(n)]['encbank_k12']}}}
result={'source':'Existing Table 2(a) measurements; no new experiments or inferred Dense timings',
        ...
        'retained_encbank_cap_GB':26,'aggregation':'median of three process medians; memory is maximum formal allocated peak',
        'whiskers':'range of three process medians, not confidence intervals',
        'ttft':'pretokenized query through first logits; retrieval/fetch/query Write included for Encbank, document Write excluded',
        'e2e':'fresh full-source preparation (all document Write for Encbank) through 128 generated IDs; no amortization',
        ...
        'repetitions':'new 8k/16k: 1 warmup + 3 repetitions per document/process; retained 32k/128k Encbank Prefill/TTFT: 5+20; retained E2E: 1+3',
        'data':data}
