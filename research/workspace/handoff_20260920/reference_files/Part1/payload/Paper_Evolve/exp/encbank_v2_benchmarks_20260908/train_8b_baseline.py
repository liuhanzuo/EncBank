...
def main():
    ...
    if args.stop_after is not None and args.stop_after <= 0:
        raise ValueError("--stop-after must be positive; use --prepare-only for CPU preparation.")
    out = Path(args.out)
    out.mkdir(parents=True, exist_ok=True)
    ...
    # Match S21's seed placement: initialize adapters after pretrained loading.
    torch.manual_seed(args.seed)
    random.seed(args.seed)
    ...
    config = {k: getattr(args, k) for k in ("j", "rank", "alpha", "chunk", "n_ctx", "topk", "lam",
              "loss", "adapter_dtype", "steps", "lr", "warmup", "seed", "grad_accum", "skip_windows", "logit_chunk")}
