...
def fixture(arm):
    ...
    reader = CoMemLower(model, 2, tok) if arm == "fix_all" else CoMem(model, 0, tokenizer=tok)
...
class QACostRunnerTests(unittest.TestCase):
    ...
    def test_waits_for_complete_scope_and_actual_child_exit(self):
        ...
        processes = [{"Name": "python.exe", "CommandLine": "python F:/Paper_Evolve/exp/comem_v2_benchmarks_20260908/serving_reuse.py --out existing"}]
