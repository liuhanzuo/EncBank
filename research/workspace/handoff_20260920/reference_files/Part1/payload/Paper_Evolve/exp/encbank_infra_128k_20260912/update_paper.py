...
base=ROOT/'exp/encbank_infra_20260912'
...
display_arms=('replay_k12','encbank_k12')
rows=[]
...
for length,summary in summaries.items():
    for arm in display_arms:
        name='Encbank' if arm=='encbank_k12' else 'Raw replay'
    ...
    for phase in ('read','ttft'):
        for arm in display_arms:
            name='Encbank' if arm=='encbank_k12' else 'Replay'
            k=10 if arm=='replay_k10' else 12
            v=summary[arm][phase]
...
if 'At 128k, the residual tensor occupies 1 GiB' not in s:
    s=s.replace('Encbank writes the complete source before timing and retains its residuals in CPU-pinned memory; replay stores token IDs.', 'Encbank writes the complete source before timing and retains its residuals in CPU-pinned memory; replay stores token IDs. At 128k, the residual tensor occupies 1 GiB per document, computed as $131{,}072\\times4{,}096\\times2$ bytes, separate from GPU peak allocation.')
...
import runpy
b300=ROOT/'exp/encbank_b300_recheck_20260912'
...
e2e=ROOT/'exp/encbank_e2e_20260913'
