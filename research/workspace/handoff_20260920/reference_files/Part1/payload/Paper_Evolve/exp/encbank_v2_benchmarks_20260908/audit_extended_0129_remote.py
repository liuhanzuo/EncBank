...
for arm in ('fix_all','j0'):
    ...
    for shard in range(4):
        ...
        assert reader['class']==('EncbankLower' if arm=='fix_all' else 'Encbank')
...
for task,length in cells:
    ...
    cell={'benchmark':'ruler','task':task,'length':length,'n':50,'metric':'answer substring recall (Encbank RULER adaptation)',
...
if (run/'COMPLETED.json').exists():
    ...
    report['writeback'].append({'benchmark':'longeval','task':'lines','length':'4k','arm':'pub','n':50,'expected_n':50,'complete':True,'metric':'Encbank-adapted lines accuracy',
