...
limits=[
 'RULER original CoMem adaptation answer-substring recall; legacy metadata/CLI/seed pairing only. Exact full input tokens, selected runtime pack and generation-cache keys were not persisted.',
...
md=['# 11:32 扩展基础基线收尾核验','',f"固定实际快照：{s['timestamp_utc']} UTC（北京时间11:34:38），main333/374。只纳入10:32后、该快照前新完整单元。",'',
 ...
 '原版8/V2 92/Replay100沿用旧已核记录。新100条raw通过原CoMem substring-recall、连续索引、完整summary、无OOM、CLI/seed和native准入日志核验。stock Qwen3-8B、j12、top12/chunk512、BM25、seed42、greedy cap48。历史没有完整输入tokens、runtime selected pack或generation-cache存证，所有RULER配对仅限同runtime metadata/CLI/seed，不升级为exact-token/pack。','',
 f"isolated KV比+BOS低24点，其逐例胜/平/负为{bos_pair['new_arm_wins']}/{bos_pair['ties']}/{bos_pair['new_arm_losses']}（n50）。保留较大缓存反而更低的结果。",'',
 '## LoCoMo五方法完整矩阵','',
 '| Category | n | CoMem | +BOS | Isolated KV | V2 | Replay |','|---|---:|---:|---:|---:|---:|---:|']
...
md+=['','类别1–4依次为multi-hop、temporal、open-domain、single-hop，使用官方F1；类别5为adversarial，采用明确记录的robust option-decoding accuracy适配，不能计算混合类别总分。只有KV列为本轮新评分，其他四列沿用已核报告。','',
 ...
 '完整源13,539–25,686 tokens，read pack5,738–6,252 tokens。完整时间顺序对话与caption、官方短答prompt、seed42选项映射、Qwen chat模板关闭thinking、完整query无截断/padding；greedy cap50、首步EOS抑制后自然EOS。isolated KV为CoMemLower有效j36、写sink、stock BF16/SDPA、统一repeat_kv禁GQA，是CoMem适配而非未经修改的HF生成或CacheBlend。','',
