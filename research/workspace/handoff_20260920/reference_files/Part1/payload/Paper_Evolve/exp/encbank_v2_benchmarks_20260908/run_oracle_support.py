...
def wait_for_remote_gpu(metadata, metadata_path):
    require(os.environ.get("ENCBANK_REMOTE_QUEUE") == "1", "Accuracy runner requires remote queue admission")
    ...
    with qa.OutputLock(out):
        ...
        try:
            ...
            files = [Path(__file__), HERE / "prepare_oracle_support.py", HERE / "official_qa_driver.py",
                HERE / "reader_adapter.py", qa.ROOT / "exp/s15_ruler_lower.py", qa.ROOT / "Encbank/encbank/model.py",
                qa.ROOT / "Encbank/encbank/selectors.py", HERE / "protocol_sources/longbench/metrics.py",
