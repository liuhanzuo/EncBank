...
def validate_options(args, arm):
    ...
    if arm in LOWER_ARMS:
        if args.sink_tokens != "bos":
            raise ValueError("EncbankLower's lower/upper pack positions require --sink_tokens bos.")
        if args.selector not in LOWER_SELECTORS:
            raise ValueError(f"EncbankLower does not provide hidden states to selector={args.selector!r}.")
...
class CachedReader:
    ...
    def generate_from_ids(self, input_ids, **kwargs):
        if type(self.reader).__name__ == "EncbankLower":
            if kwargs.get("sink_tokens", "bos") != "bos" or not kwargs.get("use_kv_cache", True):
                raise ValueError("EncbankLower requires a BOS pack sink and KV-cache decode.")
            if kwargs.get("selector", "bm25") not in LOWER_SELECTORS:
                raise ValueError("This selector is unsupported by EncbankLower.")
    ...
    def factory(model, resume_j, top_prepay_b=0, block_diagonal=False, tokenizer=None):
        ...
        if arm in LOWER_ARMS:
            if getattr(model.config, "model_type", None) != "qwen3":
                raise ValueError("The current EncbankLower capture implementation is validated for dense Qwen3 only.")
            ...
            reader = EncbankLower(model, effective_j, tokenizer=tokenizer,
                                lower_layers=[] if arm == "fix_none" else None)
        else:
            effective_j = 0 if arm == "j0" else int(resume_j)
            reader = Encbank(model, resume_j=effective_j, top_prepay_b=top_prepay_b,
