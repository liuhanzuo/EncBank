...
dest=R/'outputs/diagnostics/heartbeat_accuracy_20260909_0531_qa.json';tok=AutoTokenizer.from_pretrained(R/'models/Qwen3-8B',local_files_only=True)
snapshot=read(R/'outputs/diagnostics/heartbeat_accuracy_20260909_0531_snapshot.json')
assert {j['id'] for j in snapshot['completed_jobs'] if j['benchmark']=='longbench' and j['arm']=='pub'}=={'longbench__pub__musique_s0of2','longbench__pub__musique_s1of2'}
report['snapshot_timestamp']=snapshot['timestamp']
audited={};configs={}
for task,arms,n in [('musique',('pub',),200)]:
  for arm in arms:
...
report['unknown']=[{'task':'musique','arm':a,'score_percent':None,'reason':'incomplete in fixed05:33 snapshot'} for a in ('pub_sink','cbos')]
