...
def main():
    ...
    for model,d in deltas.items():
        lines.append(f"- {model}：Encbank 相比 without LoRA 的总分变化 {d['overall_pp']:+.2f} pp；类别1–4变化 {d['semantic_pp']:+.2f} pp。")
    lines+=['','本结果仅覆盖这两个新模型，不能将旧 Qwen3-8B 的 GPT-4o 分数改标为 Astra。API 服务的具体底层模型快照未经核实；8个固定 sanity checks 不代表人工评审一致性。单训练 seed 结果不支持跨模型显著性或速度优势结论。',
        '', 'Selected-text replay 使用与 Encbank 相同的检索文本；shared Encbank LoRA 是复用 Encbank adapter，并非独立训练 replay。KV-Direct 使用完整 source；StreamingLLM-style 与 HCache-style 均为本文兼容参考实现。本批不包含另一次 Qwen3-8B 的 CacheBlend-style LoRA 实验。',
