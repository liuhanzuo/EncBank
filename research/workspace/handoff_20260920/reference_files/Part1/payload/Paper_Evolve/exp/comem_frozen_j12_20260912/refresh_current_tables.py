...
PAPER=HERE.parents[1]/'COMem/paper_iclr2027_rewrite_20260912'
def replace_block(name,new_rows):
    p=PAPER/'sections'/name;s=p.read_text(encoding='utf-8');lines=s.splitlines()
    start=next(i for i,l in enumerate(lines) if l.startswith('CoMem (without LoRA)'))
    end=start+1
...
def main():
    ...
    for i,(task,name) in enumerate((('niah_single_2','single-2'),('niah_multikey_1','multikey'),('variable_tracking','tracking'))):
        lines.append(row(['CoMem (without LoRA)' if i==0 else '',name,*[f'{val("ruler",task,l):.1f}' for l in lengths]]))
    replace_block('tab_ruler_a.tex',lines)
    values=[val('longeval','lines',l) for l in lengths]
    replace_block('tab_longeval_detail.tex',[row(['CoMem (without LoRA)',*[f'{v:.0f}' for v in values],f'{sum(values)/len(values):.1f}'])])
    lengths=('0k','1k','2k','4k','8k','16k','32k');lines=[]
    for i,task in enumerate(('qa1','qa2','qa5')):
        values=[val('babilong',task,l) for l in lengths]
        lines.append(row(['CoMem (without LoRA)' if i==0 else '',task,*[f'{v:.0f}' for v in values],f'{sum(values)/len(values):.1f}']))
    ...
    replace_block('tab_longbench_detail.tex',[row(['CoMem (without LoRA)',*[f'{v:.2f}' for v in values],f'{result["scores"]["longbench"]:.2f}'])])
    ...
    new=row(['CoMem (without LoRA)',*[f'{v:.1f}' if i==1 else f'{v:.2f}' for i,v in enumerate(values)]])
    s=re.sub(r'^CoMem \(without LoRA\).*$',lambda m:new,s,flags=re.M)
