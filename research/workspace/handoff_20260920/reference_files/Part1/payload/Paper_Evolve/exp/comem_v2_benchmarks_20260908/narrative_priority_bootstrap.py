"""Six canonical NarrativeQA baseline shards on idle GPU1; leave the main queue intact."""
...
def verify_scope(plan,main_plan):
    ...
    if len(plan['jobs'])!=6 or sum(j['expected_n'] for j in plan['jobs'])!=600:raise RuntimeError('Not the six authorized NarrativeQA shards')
    for job in plan['jobs']:
        if job!=originals.get(job['id']):raise RuntimeError('Canonical job changed')
        if job['benchmark']!='longbench' or job['arm'] not in ('pub','pub_sink','cbos') or any(c['task']!='narrativeqa' for c in job['cells']):raise RuntimeError('Unexpected scope')
    if {(j['arm'],j['id'].rsplit('_s',1)[1]) for j in plan['jobs']}!={(a,s) for a in ('pub','pub_sink','cbos') for s in ('0of2','1of2')}:raise RuntimeError('Incomplete arm/shard coverage')
...
def main():
    ...
    out=R/'outputs/bootstrap_narrative_priority';out.mkdir(parents=True,exist_ok=True)
    ...
    plan=read(B/'narrative_priority_full_plan.json');main_plan=read(B/'full_plan.json');verify_scope(plan,main_plan)
    state={'protocol':'narrative-canonical-priority-v1','pid':os.getpid(),'status':'waiting','gpu':1,'accuracy_only':True,'plan':str(B/'narrative_priority_full_plan.json'),'canonical_outputs_shared_with_main':True,'jobs':{},'target_jobs':6,'target_predictions':600}
    def save():state['updated_at']=time.time();atomic_json(status_path,state)
    def completed(job):
        mark=read(Path(job['output'])/'COMPLETED.json')
        return mark.get('status')=='completed' and mark.get('n')==job['expected_n']
    try:
        ...
        for number,job in enumerate(plan['jobs']):
            ...
            # completed original V2/j0 stage, and no shared main state is modified.
            ...
            qdir=R/'outputs/queue_narrative_priority/full'/job['id'];qdir.mkdir(parents=True,exist_ok=True)
