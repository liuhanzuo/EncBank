    ...
    if config.LOCAL:
        ...
        try:
            admission=gpu_gate.acquire_gpu(need_gb=24,cap_gb=config.CAP_BYTES/1e9,idle_slack_gb=7,poll=10,
                tag='encbank-c3-c4-local-serving')
...
class Engine:
    ...
    def check(self,out):
        ...
        for method in ['encbank','selected_replay']:
...
def main():
    ...
    p.add_argument('--method',choices=['encbank','selected_replay'],required=True)
    ...
    with sdpa_kernel([SDPBackend.FLASH_ATTENTION,SDPBackend.EFFICIENT_ATTENTION]):
        ...
        engine=Engine(model,tok,a.j,docs,build_store=a.method=='encbank')
