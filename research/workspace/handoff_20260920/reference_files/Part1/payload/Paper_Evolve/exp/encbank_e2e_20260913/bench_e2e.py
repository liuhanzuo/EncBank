...
ROOT = HERE.parents[1]
sys.path[:0] = [str(ROOT/'Encbank'), str(ROOT/'exp'), str(ROOT/'exp/encbank_infra_20260912')]
...
def main():
    ...
    admission = acquire_gpu(need_gb=23, cap_gb=26, idle_slack_gb=5, poll=10,
                            max_wait=3600, tag='encbank_e2e_20260913')
    ...
    metadata = {'process':a.process, 'smoke':a.smoke, 'gpu':torch.cuda.get_device_name(0),
        ...
        'boundary':'pretokenized source/query -> CPU chunk/store preparation (full document Write for Encbank) -> CPU iterative BM25 construction/selection -> pinned fetch -> sink/query Write -> cached prefill -> greedy output token IDs',
        ...
        'checkpoint_scope':'same unmerged FP32 final4000 adapter as Table 2(a); not the historical accuracy checkpoint'}
    ...
    for name, reader in [('replay_k12',rp),('encbank_k12',cm)]:
    ...
    for length in (32768,131072):
        ...
        for wi, w in enumerate(workloads[:1] if a.smoke else workloads):
            ...
            gc.collect(); torch.cuda.empty_cache()
            arms = [('replay_k12',rp),('encbank_k12',cm)]
            rng.shuffle(arms)
