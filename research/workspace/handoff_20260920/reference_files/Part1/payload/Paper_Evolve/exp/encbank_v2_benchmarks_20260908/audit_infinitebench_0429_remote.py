"""Three newly complete InfiniteBench En.QA baselines; CPU, no model loading."""
import os
...
task='longbook_qa_eng';n=351;targets=('pub','pub_sink','cbos');references=('fix_all','j0')
records={};dbs={};sources={};common=None
d={'timestamp_utc':datetime.now(timezone.utc).isoformat(),'scope':'new pub/pub_sink/cbos English long-book QA only; V2/j0 metadata and packs used as previously scored reference, not rescored',
...
for arm in targets+references:
    ...
    for shard in range(4):
        ...
        if arm in targets:
            ...
            assert ri['class']==('EncbankLower' if arm=='cbos' else 'Encbank') and ri['write_sink']==(arm!='pub')
...
for i,src in enumerate(ib.iter_task(task)):
    ...
    if (i+1)%50==0:print(f'IB QA three-arm official/source/token/pack/cache checks {i+1}/{n}',flush=True)
assert len(keys)==n
for db in dbs.values():db.close()
means={}
for arm in targets:
    assert len(scores[arm])==n;mean=100*sum(scores[arm])/n;means[arm]=mean
    d['writeback'].append({'benchmark':'infinitebench','task':task,'arm':arm,'complete':True,'n':n,'expected_n':n,'metric':'official answer F1','score_percent':mean,'display':f'{mean:.2f}',
...
d['protocol']={'model':'stock Qwen3-8B BF16, SDPA, same repeat_kv policy; j12, isolated full KV effective j36',
    ...
    'scoring':'vendored official English long-book QA answer F1; score_percent=100*mean per-example F1 across all351',
    'coverage':'One complete English long-book QA task, not full InfiniteBench; MC baseline partial/unknown remains blank',
    'token_ranges':{k:{'min':min(v),'max':max(v)} for k,v in ranges.items()}}
d['checks']={'official_rescores':1053,'new_source_token_pack_exact_input_own_cache_checks':1053,'reference_scorer_calls':0,'reference_cache_lookups':0,
    'same_source_and_pack_checks_for_reference':702,'models_constructed':0,'CUDA_VISIBLE_DEVICES':os.environ['CUDA_VISIBLE_DEVICES'],
    'input_keys_sha256':hashlib.sha256('\n'.join(keys).encode()).hexdigest()}
d['finished_at_utc']=datetime.now(timezone.utc).isoformat()
dest=B/'heartbeat_infinitebench_20260909_0429_new.json';dest.write_text(json.dumps(d,indent=2)+'\n')
