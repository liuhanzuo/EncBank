...
Examples:
  python aggregate_sparse.py --input /.../outputs/sparse_encbank_20260911
...
def route_audit(route, prepared, recipe, model):
    ...
    return dict(candidate_tokens=candidate, selected_tokens=kept, actual_retain_ratio=kept / candidate,
                ...
                dense_encbank_document_kv_bytes=baseline,
...
def summarize(records):
    ...
    keys = ("candidate_tokens", "selected_tokens", "actual_retain_ratio", "analytic_document_kv_bytes",
            "tensor_document_kv_bytes", "dense_encbank_document_kv_bytes", "document_kv_reduction_fraction")
    ...
    result["pooled_document_kv_reduction_fraction"] = 1 - sum(r["tensor_document_kv_bytes"] for r in records) / sum(r["dense_encbank_document_kv_bytes"] for r in records)
    return result
...
def aggregate(root, data_dir=None, maps=(), metadata_only=False, include_pairs=False):
    root = Path(root)
    output = dict(schema="sparse-encbank-pilot-summary-v1", generated_utc=datetime.now(timezone.utc).isoformat(),
...
def render_report(summary):
    lines = ["# Encbank + Sparse Attention pilot", "", PROTOCOL, "",
