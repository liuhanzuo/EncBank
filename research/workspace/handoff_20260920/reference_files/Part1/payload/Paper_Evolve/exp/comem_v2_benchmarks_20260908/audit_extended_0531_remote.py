...
prior={'writeback':read(B/'heartbeat_extended_20260909_0129.json')['writeback']+read(B/'heartbeat_extended_20260909_0229.json')['writeback']+read(B/'heartbeat_extended_20260909_0329.json')['writeback']+read(B/'heartbeat_extended_20260909_0429.json')['writeback']}
...
d={'timestamp_utc':datetime.now(timezone.utc).isoformat(),'execution':'remote CPU; CUDA hidden; no model construction or GPU tasks',
   'writeback':[],'comparisons':[],'pending':[],'checks':{},'prior_reference':['heartbeat_extended_20260909_0129.json','heartbeat_extended_20260909_0229.json','heartbeat_extended_20260909_0329.json','heartbeat_extended_20260909_0429.json']}
...
p=B/'heartbeat_extended_20260909_0531.json';p.write_text(json.dumps(d,indent=2)+'\n')
out=['# 05:31 扩展基线准确率独立 CPU 核查','',
     '相对04:29已核结果，仅检查本轮新增的LongEval/RULER基线完整格及InfiniteBench/LoCoMo覆盖情况。全部评分、源prompt重建、tokenizer与SQLite检查在远端CPU执行，CUDA隐藏，未构造模型；未改论文、同步器、队列或GPU任务。', '',
...
out+=['','`pub`=原始CoMem；`pub_sink`=CoMem+BOS；`cbos`=Full KV隔离缓存控制，不是CacheBlend。0.00是50例全部失败的实测结果；缺失项仍为空。','',
      '## LongEval证据和边界','',
      ...
      '统一stock Qwen3-8B BF16、SDPA/repeat_kv policy、j12（cbos有效j36；j0为0）、chunk512、BM25 top12、seed1234、生成cap16。源prompt按原driver用crc32长度seed重新合成，greedy完成式输出；评分是首个至少四位数字与六位目标精确匹配。采用CoMem-adapted LongEval名称，不冒称官方LongChat prompt/scorer。', '',
      ...
      f'为避免长文原builder每增加64行就重复分词，本轮按已记录n_lines重建同一RNG全部label/value与最终query；逐源验证最后停止边界及前一64行批次未达目标，每长度第0例再与原native builder逐字等价核对。本轮有{len(reconstruction_checks)}个独立源、{sum(r["native_builder_equivalence"] for r in reconstruction_checks)}个完整native-builder代表例比较；所有新预测仍逐例检查完整token和自己的SQLite键。该优化仅在独立CPU审计程序内，不改变数据、driver、模型或评分，不以代表例检查代替逐例缓存验证。','',
...
out+=['','## RULER证据和边界','',
      ...
      '**legacy RULER未存原始完整token、运行时selected pack或generation-cache key。** 因此只有sample metadata/CLI/seed配对证据；不能升级为exact-input/runtime pack验证，也不把旧Windows/hash/encoding的样本与新remote样本当作同一输入。指标是CoMem RULER适配路径的answer substring recall，five-needle仍不是组合推理证明。', '',
...
out+=['','每个已完成shard核查completion与完整索引，跨shard无重复。JSON另列LoCoMo各类别的已完成样本数和官方总数；当前没有任一完整类别。未发布局部均值，也未重新评分尚未完整的任务。上述数字只计完整shard，不包括活动shard已产生但未完成的行。InfiniteBench QA三基线各351例已在04:29完整核验，本轮不重复；这里只列尚未完整的MC覆盖。', '',
      '## 紧凑写回建议','',
      '1. 当前Table16(a)保留现有方法行和六长度列，只补本轮64k/128k新格；其余用dash。V2/j0已有完整两行保持。',
      '2. RULER的新基线为本轮完整的64k/128k等少量格，补既有附录任务/长度行组。缺失值dash，不扩大正文表，不将cbos命名为CacheBlend。',
      '3. Table16已核InfiniteBench QA五方法以及V2/j0 MC和LoCoMo不变；新MC/LoCoMo baseline未完整，不能加入局部分数。',
      '4. caption继续写LongEval adaptation、RULER legacy配对限制及未完成dash；实测0保留0。', '',
      '## 正文概括是否需要修改','',
      '当前正文明确以full recompute为参照：V2在LongEval仍落后，这一概括保持准确。此前16k isolated full KV与V2同为82%，而full recompute为96%的边界继续保留；不能声称V2在所有长度严格优于Full KV。只更新本轮完整基线值，不从这组准确率推算未测的同任务速度优势。', '',
      '机器结果：`heartbeat_extended_20260909_0531.json`；复算脚本：`audit_extended_0531_remote.py`。文件只新增本轮审计和报告。','']
(B/'heartbeat_extended_20260909_0531.md').write_text('\n'.join(out))
