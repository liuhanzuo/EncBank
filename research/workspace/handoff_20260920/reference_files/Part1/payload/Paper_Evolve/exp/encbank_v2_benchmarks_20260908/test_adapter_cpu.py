...
class AdapterTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as temp:
            for arm in (a for a in run_driver.ARMS if a != "cacheblend16"):
                with self.subTest(arm=arm):
                    ...
                    direct = EncbankLower(model, j, tokenizer=tok,
                        lower_layers=[] if arm == "fix_none" else None) if arm in {"fix_all", "fix_none", "cbos"} else Encbank(model, j, tokenizer=tok)
    ...
    def test_native_longeval_interruption_resume_and_completed_skip(self):
        ...
        self.assertIs(module.Encbank, original_class)
