...
CONTROL='/tmp/qcm-budget-control-liuhanzuo-20260919'
...
from pathlib import Path
p=json.load(sys.stdin);control=Path('/tmp/qcm-budget-control-liuhanzuo-20260919')
...
assert not any('|qcm-budget-b300|' in l for l in queue.splitlines())
command=['sbatch','--parsable','--job-name=qcm-budget-b300','--dependency=afterok:108475,singleton','--partition=gpu','--nodelist=gpu-node1','--nodes=1','--ntasks=1','--gres=gpu:nvidia_l20d:1','--cpus-per-task=8','--mem=96G','--time=12:00:00','--chdir=/tmp','--output=/tmp/qcm-budget-%j.out','--error=/tmp/qcm-budget-%j.err']
...
if r.returncode==0:
 record['job']=r.stdout.strip().split(';')[0];assert record['job'].isdigit();record['stage']='/tmp/qcm-budget-liuhanzuo-'+record['job'];record['node']='gpu-node1'
