...
for path in (HERE.parents[1] / "COMem", HERE.parent / "comem_v2_benchmarks_20260908"):
...
class TinyTests(unittest.TestCase):
    def setUp(self):
        ...
        self.comem = CoMem(model, resume_j=2)
    ...
    def test_all_arm_reference_reuse_and_fresh_query_state(self):
        for arm in ("D0", "A", "B", "D1", "FULL"):
            with self.subTest(arm=arm):
                self.args.arm = arm
                out = run_reuse_quality_probe(self.args, torch, self.comem, Tokenizer(),
