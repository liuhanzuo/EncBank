...
def main():
    ...
    names = {'NATIVE': '原生 Encbank + LoRA', 'FULL': 'Full recompute', 'D0': 'D0 同图 wrapper 对照',
             'A': 'A 全块', 'B': 'B 剪枝 50%', 'D1': 'D1 原注意力 + 剪枝'}
    text = ['# Encbank + Sparse Attention：5090 实测表', '',
            '**24/24 配置完成，无 OOM，无采样显存增量超限。** 4K、16K、32K均为实际文档读取量；另有64-token问题和固定32-token输出。每项仅一次合成输入工程测量，使用同一强LoRA，不能代替训练后质量评测。', '',
            '这轮三个长度下，原生Encbank端到端耗时均最低。稀疏方法减少部分活跃张量，但分配器保留量、设备驻留和读取开销仍需优化；不能据解析KV字节宣称总体优势。', '',
    ...
    text += [f'[完整 allocated/reserved、一次写入、热缓存构建和传输明细](<{detail}>)。', '',
             '9组冷/热及D0/native对照的生成token和选块完全一致；这只是所测合成输入上的缓存语义检查。Encbank exact-prefix基线仍待补。128K+仍待共同长窗口配置与质量验证；32K没有替代它。', '',
    ...
    labels = {'NATIVE': 'Encbank + LoRA', 'FULL': 'Full recompute', 'D0': 'D0 control',
