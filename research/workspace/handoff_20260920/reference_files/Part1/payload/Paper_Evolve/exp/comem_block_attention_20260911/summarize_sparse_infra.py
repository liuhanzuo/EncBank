...
def summarize(folder):
    ...
    for shape, rows in grouped.items():
        ...
        diagnostics.append({"shape": shape, "arm": "D0/NATIVE", **parity_pair(rows.get(("D0", "cold_hj")),
            rows.get(("NATIVE", "cold_hj")), "sparse wrapper D0 vs native CoMem")})
    ...
    labels = {"D0": "D0 (wrapper)", "A": "Block-all", "B": "Block-pruned", "D1": "Dense-pruned",
              "NATIVE": "CoMem + LoRA", "FULL": "Full recompute"}
    for job in plan:
    ...
    for arm, label in (("NATIVE", "Native CoMem + LoRA"), ("FULL", "Full recompute"), ("EXACT_PREFIX", "CoMem exact prefix")):
