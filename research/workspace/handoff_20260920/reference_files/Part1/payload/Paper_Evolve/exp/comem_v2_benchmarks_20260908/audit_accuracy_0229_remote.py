...
dest=R/'outputs/diagnostics/heartbeat_accuracy_20260909_0229_qa.json';tok=AutoTokenizer.from_pretrained(R/'models/Qwen3-8B',local_files_only=True)
audited={};configs={}
for task,arms,n in [('multifieldqa_en',('pub','pub_sink','cbos'),150),('qasper',('pub','pub_sink'),200)]:
  for arm in arms:
    ...
    for shard in range(2):
      ...
      if arm=='cbos':assert info['class']=='CoMemLower' and info['effective_j']==36 and info['write_sink'] is True
      else:assert info['class']=='CoMem' and info['effective_j']==12 and info['write_sink']==(arm=='pub_sink')
...
for task,arms,n in [('multifieldqa_en',('pub','pub_sink','cbos'),150),('qasper',('pub','pub_sink'),200)]:
  references=['fix_all','j0']+(['fix_none'] if task=='qasper' else [])
  ...
  pairs=[(ref,arm) for arm in arms for ref in references]+[('pub_sink','pub')]
