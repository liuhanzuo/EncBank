...
def main():
    ...
    protocol = {"repo": REPO, "revision": REVISION, "model_dir": str(args.model_dir), "j": 8,
                ...
                "ruler_template": "existing completion-style Encbank RULER; full terminal question/prefix; no chat template",
    ...
    result = {"status": "ready" if all(x["budget_valid"] for x in all_rows) else "budget_failed",
              ...
              "sources": [file_info(p) for p in sources + [source, prompts_path, caps_path, essay, Path(R.__file__), ROOT / "Encbank/encbank/selectors.py"]],
