"""Interleaved beacon residuals for CoMem: an experimental, trainable baseline.
...
chunks, and CoMem's upper-layer reader. It stores no lower-layer document KV.
...
COMEM_ROOT = Path(__file__).resolve().parents[2] / "COMem"
if str(COMEM_ROOT) not in sys.path:
    sys.path.insert(0, str(COMEM_ROOT))
...
class BeaconCoMem(nn.Module):
    """Keep only every inserted beacon's h_j; preserve CoMem query semantics.

    The last incomplete group also gets one beacon (ceil(T / ratio)), without
    adding padding tokens. Reading uses compact CoMem positions. The constructor
    does not attach LoRA or change the backbone's trainability.
    """

    FORMAT = "beacon-comem-simplified-v1"
    ...
    def __init__(self, model: nn.Module, split: int, compression_ratio: int,
        ...
        self.reader = CoMem(model, resume_j=split)
    ...
    def configuration(self) -> dict:
        return {"format": self.FORMAT, "writer_id": self.writer_id, "split": self.reader.resume_j,
                ...
                "read_positions": "compact-comem", "tail_policy": "append-one-no-padding"}

    def _sync_reader_location(self) -> None:
        # CoMem is an ordinary object: nn.Module.to() cannot update its snapshots.
    ...
    def read_logits(self, query_ids, memories: Sequence[BeaconMemory]) -> torch.Tensor:
        ...
        # Original CoMem query and sink, without beacon insertion or document KV.
