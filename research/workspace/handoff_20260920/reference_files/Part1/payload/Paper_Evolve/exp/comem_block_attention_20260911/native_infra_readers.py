"""Native CoMem and full-recompute controls for the sparse infra harness.
...
if str(HERE.parents[1] / "COMem") not in sys.path:
...
class NativeCoMemReader:
    """Native dense CoMem execution of the passed model and its active adapter."""

    store_kind = "cold_hj"
    implementation = "native-comem-layer-calls-cached-decode"
    model_contract = "same passed model object and active adapter; no merge or replacement"

    def __init__(self, comem):
        if comem.top_prepay_b != 0 or comem.block_diagonal:
            raise ValueError("Native control requires ordinary exact-resume CoMem")
        self.comem = comem
        self.model = comem.model
        self.j = int(comem.resume_j)
        self.L = int(comem.num_layers)
    ...
    def write_chunk(self, token_ids):
        self._check_eval()
        return self.comem.write_chunk(token_ids)
    ...
    def decode_step(self, token_id, state):
        self._check_eval()
        logits = self.comem.decode_step(token_id, state.bottom_cache, state.top_cache,
...
class FullRecomputeReader(NativeCoMemReader):
    ...
    def __init__(self, comem):
        ...
        full = CoMem(comem.model, resume_j=0, tokenizer=comem.tokenizer)
        super().__init__(full)
...
def self_test():
    ...
    class Tests(unittest.TestCase):
        def setUp(self):
            ...
            self.base = CoMem(self.model, resume_j=2)
            self.chunks = [[3, 4, 5, 6], [7, 8, 9, 10]]
            self.prompt = [21, 22, 23]

        def test_native_matches_sparse_D0_prefill_and_decode(self):
            reader = NativeCoMemReader(self.base)
            sparse = SparseCoMemReader(self.base, fusion_layer=4, retain_ratio=1., probe_mode="dense").eval()
