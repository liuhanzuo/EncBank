"""Six canonical BABI qa1/qa5 32k baseline shards on idle GPU1; leave the main queue intact."""
...
def verify_scope(plan,main_plan):
    ...
    if len(plan['jobs'])!=6 or sum(j['expected_n'] for j in plan['jobs'])!=600:raise RuntimeError('Not the six authorized BABI qa1/qa5 32k cells')
    actual=set()
    for job in plan['jobs']:
        ...
        cell=job['cells'][0]
        if cell['task'] not in ('qa1','qa5') or cell['length']!='32k' or cell['indices']!=list(range(100)) or job['expected_n']!=100:raise RuntimeError('Unexpected native dataset cell')
...
def verify_predecessor():
    ...
    return {'babi8_native_receipts':6,'babi8_predictions':600,'babi16_native_receipts':6,'babi16_predictions':600,'non_qwen_rows':1750,'non_qwen_cells':20,'smoke_reused':70,'formal_generations':1680,'non_qwen_child_exit_code':0,'historical_pids':sorted(set(p for p in pids if p)),'all_recorded_processes_exited':True}
...
def main():
    ...
    plan=read(B/'babi_q15_32k_priority_full_plan.json');main_plan=read(B/'full_plan.json');verify_scope(plan,main_plan)
    state={'protocol':'babi-q15-32k-canonical-priority-v1','pid':os.getpid(),'status':'waiting','gpu':1,'accuracy_only':True,'plan':str(B/'babi_q15_32k_priority_full_plan.json'),'canonical_outputs_shared_with_main':True,'jobs':{},'target_jobs':6,'target_predictions':600,'after_completion':'exit after these six canonical jobs; no automatic next workload'}
    ...
    try:
        ...
        for number,job in enumerate(plan['jobs']):
            ...
            qdir=R/'outputs/queue_babi_q15_32k_priority/full'/job['id'];qdir.mkdir(parents=True,exist_ok=True)
