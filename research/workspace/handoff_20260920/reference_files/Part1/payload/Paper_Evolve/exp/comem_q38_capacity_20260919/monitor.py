...
for d in (root/'results').glob('*.json'):
 report['points'][d.stem]={'complete.json':read(d)}
for key,path in [('stderr',root/'stderr.log'),('stdout',root/'stdout.log'),('bootstrap_err',Path('/tmp/qcm-q38-capacity-'+p['job']+'.err'))]:
 if path.exists():
