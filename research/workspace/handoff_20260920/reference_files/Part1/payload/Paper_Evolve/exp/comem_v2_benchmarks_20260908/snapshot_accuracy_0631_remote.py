...
R=Path('/data/liuhanzuo/comem_v2_20260908');B=R/'workspace/exp/comem_v2_benchmarks_20260908';D=R/'outputs/diagnostics'
...
plan=read(B/'full_plan.json');prior=read(D/'heartbeat_babilong_20260909_0531.json')
old={(x['task'],x['length'],x['arm']) for x in prior['carried_forward_baseline_cells']+prior['writeback']}
assert len(old)==22
snapshot={'timestamp':datetime.now(timezone.utc).isoformat(),'host':os.uname().nodename,'ssh_alias':'longjing-1','prior_babi_cells':22,'completed_jobs':[],'incomplete_jobs':[],'invalid_receipts':[]}
for job in plan['jobs']:
    if job['benchmark']=='longbench':wanted=job['arm'] in ('pub_sink','cbos') and job['cells'][0]['task']=='musique'
...
D.mkdir(parents=True,exist_ok=True);(D/'heartbeat_accuracy_20260909_0631_snapshot.json').write_text(json.dumps(snapshot,indent=2)+'\n')
...
for name in ('babi16_priority','non_qwen_formal'):
...
health['non_qwen_actual']=read(R/'outputs/non_qwen_smol_20260909/formal/status.json')
