...
are inherited verbatim. This class references the supplied Encbank model; it does
...
from sparse_reader import KVPair, SparseEncbankReader, SparseQueryState


class OptimizedSparseEncbankReader(SparseEncbankReader):
    ...
    def _decode_rotate(self, x, positions, rope_cache):
        ...
        if pair is None:
            cos, sin = self.encbank.rotary_emb(x[:, 0], position_ids=positions)
    ...
    def decode_step(self, token_id, state: SparseQueryState):
        ...
        hidden = self.encbank.embed_tokens(token)
        ...
        return self.encbank.lm_head(self.encbank.norm(hidden))
