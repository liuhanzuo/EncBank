...
def old_failure():
    ...
    if not any(row[:4] == ['24023', 'encbank-sparse-1gpu', 'FAILED', '1:0'] for row in rows):
...
def prepare_or_submit(submit):
    ...
    with (ROOT.parent/'.codex-encbank-sparse-l20d.submit.lock').open('a') as submission_lock:
        ...
        with lock_path(ROOT).open('a') as worker_lock:
            ...
            record=dict(job_id=job_id, raw_job_id=result.stdout.strip(), job_name='encbank-sparse-1gpu',
