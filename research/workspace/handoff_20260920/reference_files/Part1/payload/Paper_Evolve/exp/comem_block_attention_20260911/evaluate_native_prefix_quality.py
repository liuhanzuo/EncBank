"""Remote eval-only native CoMem cold versus whole-prefix reuse QA.
...
def source_paths():
    ...
    paths.update({
        "evaluate_sft.py": HERE.parent / "beacon_comem_20260909" / "evaluate_sft.py",
...
def run(args, lease_receipt):
    ...
    try:
        ...
        native, prefix = NativeCoMemReader(cm).eval(), NativePrefixCoMemReader(cm).eval()
