...
ARMS = {"beacon4": ("beacon", 4), "comem": ("comem", 1),
...
def aggregate(root: Path) -> dict:
    ...
    reference_recipe = internals["comem"]["recipe"]
    ...
    if reference_recipe:
        ...
        for arm, item in internals.items():
            ...
            if candidate is None:
                ...
                if keys:
                    comparable_recipes = False
                    result["comparison_issues"].append(f"{arm}: shared recipe differs from CoMem: {keys}")
                if item["model"] != internals["comem"]["model"]:
                    comparable_recipes = False
                    result["comparison_issues"].append(f"{arm}: recorded backbone configuration differs from CoMem")
    else:
        result["comparison_issues"].append("CoMem reference recipe unavailable")
    all_steps = sorted({step for item in internals.values() for step in item["evaluations"]})
    for step in all_steps:
        stage = {"step": step, "comparable": False, "issues": [], "by_arm": {}, "deltas_vs_comem": {}}
        for arm, item in internals.items():
        ...
        if not stage["issues"]:
            reference = internals["comem"]["evaluations"][step]
            ids = reference["ids"]
            for arm, item in internals.items():
                ...
                if not paired["comparable"]:
                    ...
                    stage["deltas_vs_comem"][arm] = paired
            stage["comparable"] = not stage["issues"]
            if stage["issues"]:
                stage["deltas_vs_comem"] = {}
...
def render_report(result: dict) -> str:
    lines = ["# Beacon + CoMem SFT pilot", "", f"Report complete: **{str(result['complete']).lower()}**. Scores are internal QASPER development results.", "",
