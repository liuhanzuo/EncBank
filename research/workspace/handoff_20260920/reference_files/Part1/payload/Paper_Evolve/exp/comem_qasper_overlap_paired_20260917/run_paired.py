...
PRIOR = HERE.parent / 'comem_overlap_20260913'
sys.path.insert(0, str(PRIOR))
from common import CoMem, attach, check_paths, decode, dump, parts, score, selection, timed_stamp, torch, write_one
...
def main():
    ...
    parser.add_argument('--adapter', default=str(HERE.parent / 'comem_v2_benchmarks_20260908/checkpoints/8b_j12_pub_4k/final'))
    ...
    admission = acquire_gpu(need_gb=23, cap_gb=28, idle_slack_gb=5, poll=10,
                            max_wait=6*3600, tag='comem_qasper_overlap_paired_20260917')
    ...
    dump(out / 'metadata.json', dict(model=args.model, adapter=args.adapter,
         ...
         inference_source=str(PRIOR / 'COMem'), protocol='PROTOCOL_zh.md',
         timing='Synchronized wall time, full-document lower-layer encoding plus CPU-pinned store'))
    reader = CoMem(model, 12, tokenizer=tokenizer)
