...
control=Path('/tmp/qcm-deploy-control-liuhanzuo-20260919-r1');root=Path('/tmp/qcm-deploy-liuhanzuo-'+job)
...
env['PYTHONPATH']=str(root)+':/srv/encbank/comem_infra_recheck_20260912/deps'
...
with (root/'stdout.log').open('ab') as o,(root/'stderr.log').open('ab') as e:
    child=subprocess.Popen(['/srv/encbank/Paper_Evolve/.venv/bin/python','-B','-u',str(root/'benchmark.py'),'--validate-only'],cwd=str(root),env=env,stdout=o,stderr=e)
