...
import numpy as np
HERE=Path(__file__).resolve().parent;sys.path.insert(0,str(HERE/'Encbank'))
...
def main():
    ...
    if all((HERE/'kv_quality'/f'shard_{i:02d}'/'complete.json').exists() for i in range(4)):
        ...
        for on in (False,True):
            for method in ('replay','encbank','chunkkv'):
                per={}
                for c in CELLS:
            ...
            for ref in ('replay','chunkkv'):
                base={r['id']:r['score'] for r in records if r['method']==ref and r['adapter_on']==on};ours={r['id']:r['score'] for r in records if r['method']=='encbank' and r['adapter_on']==on}
                ...
                for name,cells in [('longeval_macro',CELLS[:3]),('qasper',['qasper'])]:
                    ...
                    if name=='qasper':
                        source=[json.loads(l) for l in (HERE.parent/'encbank_frozen_j12_20260912/data/longbench/qasper.jsonl').read_text(encoding='utf-8').splitlines()]
                        clusters={}
                    ...
                    paired[f'{name}:encbank_minus_{ref}:adapter_{on}']={'delta_pp':float(np.mean([diffs[c].mean() for c in cells])*100),'paired_stratified_bootstrap_95ci':np.quantile(np.mean(boots,axis=0)*100,[.025,.975]).tolist()}
