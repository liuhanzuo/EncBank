...
control=Path('/tmp/qcm-adaptive-control-liuhanzuo-20260919');root=Path('/tmp/qcm-adaptive-liuhanzuo-'+job)
...
with (root/'stdout.log').open('ab') as o,(root/'stderr.log').open('ab') as e:
    child=subprocess.Popen(['/srv/encbank/Paper_Evolve/.venv/bin/python','-B','-u',str(root/'pilot.py')],cwd=str(root),env=env,stdout=o,stderr=e)
