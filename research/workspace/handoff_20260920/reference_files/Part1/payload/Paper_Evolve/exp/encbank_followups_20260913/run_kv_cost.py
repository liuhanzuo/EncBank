...
def main():
    ...
    admission=acquire_gpu(need_gb=23,cap_gb=28,idle_slack_gb=5,poll=10,max_wait=6*3600,tag='encbank_followup_kv')
    ...
    cm,rp=Encbank(model,12,tokenizer=tok),Encbank(model,0,tokenizer=tok);cb=KVControl(rp)
    dump(out/'correctness.json',checks(model,tok))
    dump(out/'metadata.json',{'gpu':torch.cuda.get_device_name(0),'torch':torch.__version__,'transformers':transformers.__version__,'process':a.process,'cap_bytes':28_000_000_000,'admission':admission,'generation_tokens':128,'formal_reps':3,'warmups':1,'adapter_conditions':['on','off'],'store':'whole document pinned CPU; token IDs for replay, BF16 residuals for Encbank, BF16 all-layer K/V for chunkKV','pipeline':'synchronous HF reference, no overlap of fetch and recompute','KV_query':'zeros for query K/V placeholders; every query position recomputed at all layers; identical to full query cached initialization'})
    ...
    def operation(row,method,on):
        ...
        t_write=timed_stamp()
        if method=='encbank':store=[cm.write_chunk(c).cpu().pin_memory() for c in chunks]
        elif method=='chunkkv':
            store=[]
            for c in chunks:
        ...
        if method=='chunkkv':
            ...
            reader=cm if method=='encbank' else rp;states=[store[i].to('cuda',non_blocking=True) if method=='encbank' else rp.write_chunk(raw[i]) for i in ix];fetched=timed_stamp();sink=reader.write_chunk([tok.bos_token_id]);generated,first,ended=decode(reader,sink,states,query,tok.eos_token_id,128,fixed=True);stats={}
    ...
    with (out/'records.jsonl').open('w',encoding='utf-8') as f:
        for row in rows:
            for rep in range(-1,1 if a.smoke else 3):
                arms=[(method,on) for method in ('replay','encbank','chunkkv') for on in (False,True)];rng.shuffle(arms)
