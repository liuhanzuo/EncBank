...
def main():
    ...
    result={};records=[]
    for file in (HERE.parent/'encbank_frozen_j12_20260912/results').glob('shard_*/longbench_*.predictions.jsonl'):
    ...
    if remote_complete:
        for subset in ('full','clean'):
            for reference in ('without_lora','replay_no_adapter'):
                distributions=[];point=[]
                for task in TASKS:
                    raw=[json.loads(l) for l in (HERE.parent/'encbank_frozen_j12_20260912/data/longbench'/f'{task}.jsonl').read_text(encoding='utf-8').splitlines()]
                    groups={}
                    for i,row in enumerate(raw):
                        sid=f'{task}:{i}'
                        if subset=='clean' and not samples[sid]['clean']:continue
                        groups.setdefault(row['context'],[]).append(allscores['encbank'][sid]-allscores[reference][sid])
                ...
                comparisons[f'{subset}:encbank_minus_{reference}']={'mean_pp':float(np.mean(point)*100),'document_cluster_bootstrap_95ci':np.quantile(boot,[.025,.975]).tolist()}
