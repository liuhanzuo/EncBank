...
import config
def main():
    root=Path('F:/Paper_Evolve/exp/comem_cacheblend_lora_20260916')
    load=lambda p:json.loads(p.read_text(encoding='utf-8'))
    cb=load(root/'training/metadata.json');cm=load(root/'comem_training_metadata.json')
    ...
    groups={a:[r for r in rows if r['arm']==a] for a in ['cacheblend_own_lora','comem_own_lora']}
    table={}
