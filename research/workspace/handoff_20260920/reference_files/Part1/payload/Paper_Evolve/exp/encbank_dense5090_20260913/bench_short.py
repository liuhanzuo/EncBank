"""Paired 8k/16k full-source Dense and Encbank measurements under 28 GB."""
from pathlib import Path
import argparse, gc, json, os, platform, random, sys, time
HERE=Path(__file__).resolve().parent; ROOT=HERE.parents[1]
sys.path[:0]=[str(ROOT/'Encbank'),str(ROOT/'exp'),str(ROOT/'exp/encbank_e2e_20260913')]
...
def main():
    ...
    admission=acquire_gpu(need_gb=26.5,cap_gb=28,idle_slack_gb=5,poll=10,max_wait=7200,
                          tag='encbank_dense5090_short_20260913')
    ...
    cm=Encbank(model,12,tokenizer=tok); bos=model.config.bos_token_id
    ...
    meta={'process':a.process,'smoke':a.smoke,'gpu':torch.cuda.get_device_name(0),
        ...
        'encbank_store':'CPU-pinned full-source residual store, as specified by Encbank; weights fully GPU resident',
        ...
        'boundaries':{
            ...
            'encbank_prefill':'GPU-ready sink/selected residuals/query residual -> cached suffix prefill -> first logits',
            'encbank_ttft':'pretokenized CPU query and prewritten CPU store -> BM25 rebuild/select -> fetch -> sink/query Write -> cached prefill -> first logits',
            'encbank_e2e':'pretokenized CPU source/query -> fresh CPU chunk/raw store and full document Write -> BM25 -> fetch -> sink/query Write -> cached prefill -> 128 output token IDs'},
        'excluded':'model load, tokenization/detokenization, network/queue/external I/O; store/cache teardown after output',
        'checkpoint_scope':'same unmerged FP32 final4000 adapter as earlier local costs; not historical accuracy checkpoint',
    ...
    with sdpa_kernel([SDPBackend.FLASH_ATTENTION,SDPBackend.EFFICIENT_ATTENTION]):
        ...
        check['encbank']={'equal':actual==expected,'cached_ids':actual,'recomputed_ids':expected}
        ...
        for length in (8192,16384):
            ...
            for wi,w in enumerate(workloads[:1] if a.smoke else workloads):
                ...
                methods=['dense','encbank']; rng.shuffle(methods)
