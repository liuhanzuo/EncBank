...
def main():
    ...
    cm={r['id']:r for r in groups[('comem',12)]};comparisons=[]
    for k in decision['quality_raw_ks']:
        ...
        ratio=summary[f'raw_k{k}']['TTFT_median_ms']/summary['comem_k12']['TTFT_median_ms']
        comparisons.append(dict(raw_k=k,comem_k=12,latency_ratio=ratio,latency_percent_difference=100*(ratio-1),
            delta_F1=summary['comem_k12']['F1']-summary[f'raw_k{k}']['F1'],paired_cluster_CI95=np.percentile(boot,[2.5,97.5]).tolist(),
    ...
    md=['# 相同BGE的Qasper质量与在线TTFT：邻近工作点比较','',f"20个额外Qasper训练文档用于校准，200题/{spec['evaluation_contexts']}个完整context簇用于质量；完整context哈希、规范化文本和开头64词检查隔离。",'',
        ...
        '|---|---:|---:|---:|---:|---:|---:|']
    for key in [f'raw_k{k}' for k in decision['quality_raw_ks']]+['comem_k12']:
        x=summary[key]
        md.append(f"| {'Raw replay（共享adapter）' if x['method']=='raw' else 'CoMem（共享adapter）'} | {x['k_max']} | {x['actual_pack_tokens_mean']:.1f}/{x['actual_pack_tokens_median']:.1f} | {x['TTFT_median_ms']:.1f} | {x['F1']:.2f} | {x['output_tokens_mean']:.2f} | {x['length_cap_rate']:.1%} |")
    md+=['','三臂各200题，合计600条正式答案；各臂空答和错误均为0。pack对CoMem表示缓存H与在线query组成的序列位置数，并不表示重放原文token。所有答案均在自然EOS协议下达到128-token上限；未设置min_new_tokens或强制固定输出长度。']
    for x in comparisons:md+=['',f"raw k={x['raw_k']}：TTFT比值 {x['latency_ratio']:.4f}，差{x['latency_percent_difference']:+.2f}%；CoMem−raw F1 {x['delta_F1']:+.2f}，配对context簇95%区间 [{x['paired_cluster_CI95'][0]:+.2f}, {x['paired_cluster_CI95'][1]:+.2f}]。测试集满足±5%：{'是' if x['test_matches_within_5_percent'] else '否'}。"]
