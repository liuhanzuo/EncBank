...
OUT = ROOT / "outputs/diagnostics/heartbeat_babilong_20260909_0931.json"
PRIOR = ROOT / "outputs/diagnostics/heartbeat_babilong_20260909_0129.json"
BASELINE_PRIOR = ROOT / "outputs/diagnostics/heartbeat_babilong_20260909_0831.json"
...
snapshot=read(ROOT / "outputs/diagnostics/heartbeat_accuracy_20260909_0931_snapshot.json")
snapshot_keys={(j["cells"][0]["task"],j["cells"][0]["length"],j["arm"]) for j in snapshot["completed_jobs"] if j["benchmark"]=="babilong"}
require(len(already_audited)==49,"Prior49 baseline cells expected")
ready, pending = {}, []
for task in TASKS:
...
require(sum(map(len,ready.values()))==7,"Expected seven new cells in fixed snapshot")
