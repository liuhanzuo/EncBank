...
from babi_q15_16k_priority_bootstrap import R,B,read,alive
state=read(R/'outputs/bootstrap_babi_q15_16k_priority/status.json')
...
plan=read(B/'babi_q15_16k_priority_full_plan.json');job=next(j for j in plan['jobs'] if j['id']==state['active_job']);out=Path(job['output'])
...
(R/'outputs/diagnostics/heartbeat_babi_q15_16k_priority_20260909_0831_health.json').write_text(json.dumps(report,indent=2)+'\n')
