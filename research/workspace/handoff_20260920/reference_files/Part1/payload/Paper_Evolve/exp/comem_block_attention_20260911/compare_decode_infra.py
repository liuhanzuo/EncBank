...
def compare(reference, optimized, destination):
    ...
    lines = ["# 单 token decode 优化：5090 有界对照", "",
        ...
        "", f"同一强 CoMem LoRA、同一输入/缓存/注意力图；问题token数{sorted(set(r['prompt_tokens'] for r in rows))}，固定输出token数{sorted(set(r['generation_tokens'] for r in rows))}，每单元同进程重复次数{sorted(set(r['repetitions'] for r in rows))}。TTFT/E2E取均值，decode TPS为总decode步数/总decode时间。不是QA准确率，也不是独立进程重复的置信区间。",
    ...
    lines += ["", "只有decode算子组织改变，prefill及热缓存构建继承原实现。TTFT的波动不能归因于这次decode优化。D0是sparse wrapper实现对照，不能直接当作原生CoMem。此前原生CoMem测量另存，不与本次新增重复数混成同一组。",
