...
d={'timestamp_utc':datetime.now(timezone.utc).isoformat(),'execution':'remote CPU; CUDA hidden; no model construction or GPU tasks',
   'writeback':[],'comparisons':[],'pending':[],'checks':{},'prior_reference':['heartbeat_extended_20260909_0129.json','heartbeat_extended_20260909_0229.json','heartbeat_extended_20260909_0329.json','heartbeat_extended_20260909_0429.json','heartbeat_extended_20260909_0531.json','heartbeat_extended_20260909_0631.json','heartbeat_extended_20260909_0731.json','heartbeat_extended_20260909_0831.json','heartbeat_extended_20260909_0931.json','heartbeat_extended_20260909_1032.json']}
d['snapshot_timestamp_utc']=snapshot['timestamp_utc']
d['snapshot_file']=str(B/'heartbeat_extended_20260909_1132_snapshot.json')
...
for arm in ('pub','pub_sink','cbos'):
    for p in sorted((O/'ruler'/arm).glob('*.json')):
        ...
        for refarm in ('fix_all','j0'):
            ...
            differences=[a[arm+'_recall']-b[refarm+'_recall'] for a,b in zip(rr,ref['rows'])]
            ref_paired[refarm]={'direction':'new baseline relative to previously audited reference','paired_n':50,
                'new_arm_wins':sum(x>1e-12 for x in differences),'ties':sum(abs(x)<=1e-12 for x in differences),
                'new_arm_losses':sum(x< -1e-12 for x in differences),'new_arm_minus_reference_pp':score-ref_scores[refarm],
                'sample_metadata_CLI_seed_pairing_only':True,'exact_input_pack_claim':False,'reference_scorer_calls':0,'significance_test_performed':False}
        cell={'benchmark':'ruler','task':task,'length':length,'arm':arm,'complete':True,'n':50,'expected_n':50,'score_percent':score,'display':f'{score:.2f}',
            ...
            'paired_reference_scores_percent':ref_scores,'paired_contrasts':ref_paired,'exact_input_tokens_persisted':False,'runtime_selected_pack_persisted':False,'generation_cache_available':False}
...
p=B/'heartbeat_extended_20260909_1132_ruler.json';assert not p.exists();p.write_text(json.dumps(d,indent=2)+'\n')
