...
Checks: (1) EncbankLower(S={}, no sink entry) reproduces the base Encbank generation and
...
low0 = EncbankLower(model, j, tok, lower_layers=[], sink_in_cache=False, chunk_write_sink=False)
...
# recompute path vs KV-cache path inside EncbankLower must agree
fix = EncbankLower(model, j, tok, lower_layers=None)
...
for name, cm in [("fix_S", EncbankLower(model, j, tok, lower_layers=[0])),
                 ("fix_nosink", EncbankLower(model, j, tok, lower_layers=None, chunk_write_sink=False)),
                 ("pub_sink", None), ("j0", Encbank(model, resume_j=0, tokenizer=tok))]:
    if cm is None:
...
fix2 = EncbankLower(model, j, tok, lower_layers=None)
