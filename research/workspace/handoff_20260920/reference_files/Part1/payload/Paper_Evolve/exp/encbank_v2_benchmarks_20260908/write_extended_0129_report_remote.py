...
out=['# 01:29 扩展准确率独立 CPU 核查','',
     '仅审 InfiniteBench、Encbank-adapted LongEval 与 RULER；LongBench、LoCoMo、BABILong 由其他代理负责。本轮没有 GPU/模型计算，没有改论文、同步器或实验结果。全部重新分词、检索、SQLite 和评分计算在 longjing-1 CPU 执行，CUDA 显式隐藏；不使用远端耗时作为论文测速。','',
     '## 新完整结果格','',
...
out+=['',f"InfiniteBench English long-book choice：V2 **130/229 = 56.77%**，全层重算 j0 **121/229 = 52.84%**，差 **{mc['v2_minus_j0_pp']:+.2f} pp**。逐例正确性列联计数为 `{mc['paired_counts']}`，仅报告描述性均值，未进行显著性检验。",'',
      ...
      '两臂各 4 个已完成 shard，源索引精确覆盖 0–228，ID 唯一，无缺失或重复；每个 receipt 的 new+reused generation 数与对应 shard 记录数相等。两臂除 arm 外 driver_options 一致，reader 实际为 EncbankLower j12 与 Encbank j0，冻结 Qwen3-8B BF16、SDPA 和 repeat_kv kernel policy 一致。',
      '',
...
out+=['',
      ...
      '',
      '**证据边界：legacy RULER raw 没有持久化完整 token IDs、运行时 selected pack 或 generation-cache key。** 本轮确认 metadata/答案/长度/CLI/seed 配对，未从源码重建 256k 全文并声称它就是已运行输入，也不能称为 exact-pack runtime verification。recall 使用 Encbank RULER 适配路径的 `_string_match_all_one`，不冒称完整官方 RULER suite。不要把旧 Windows Python/hash/encoding 的 16k 合成样本当作这组远端完全相同样本；需要精确 16k follow-up 时引用既有 matched16k 结果。',
      '',
      '## 新方法与未知项','']
for c in d['comparisons']:
    if c['task']=='longeval lines/4k':
        out.append(f"LongEval pub4k 已完整 50 例，分数 `{c['scores_percent']}`。本轮重新合成 50 条 prompt，三方法共150个源 label/expected/line count、token输入和完整 generation参数缓存键逐条对齐；只重评分新方法及配对参照。历史 raw 没有记录 selected indices，因此不声称另有原始 pack 日志。该任务为 Encbank 适配，首个至少4位数的精确值评分，非官方 LongChat 协议。")
