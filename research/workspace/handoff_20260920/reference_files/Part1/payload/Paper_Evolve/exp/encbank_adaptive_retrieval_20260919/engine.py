...
class Engine:
    ...
    def __init__(self,model,tok,docs,method,concurrency,k=12,hop=2):
        assert method in METHODS
        self.k,self.hop=int(k),int(hop)
        assert 1 <= self.k <= 64 and self.hop > 0
        self.doc_positions=1+self.k*512
        ...
        if self.is_h:
            ...
            if method=='h_cpu':
                ...
                self.host_buffer=torch.empty((concurrency,self.doc_positions,self.reader.hidden_size),dtype=self.bank.dtype,pin_memory=True)
    ...
    def doc_hidden(self,picks,selected,matched):
        include_sink=matched==0;skip=max(0,matched-1);remaining=self.k-skip;batch=len(picks)
        if not self.is_h:
            tokens=[]
    ...
    def infer(self,picks,count=32,capture=False,forced_selected=None,forced_tokens=None):
        ...
        for di,qi in picks:
            ...
            sel=self.indices[di].select(query['query'][:32],self.k,self.hop)
            assert sel==query['selected'] and len(sel)==self.k;chosen.append(sel)
        ...
        self.stats['requests']+=len(picks);self.stats['prefix_positions']+=self.doc_positions*len(picks)
        self.stats['matched_positions']+=sum(matched);self.stats['full_hits']+=sum(n==self.doc_positions for n in matched)
        ...
        for n in range(1,count):
            logits=self.reader.decode_step(token,bottom,top,qpos+n-1,self.doc_positions+query.shape[1]+n-1)
