...
OLD = ROOT / "exp/comem_v2_benchmarks_20260908"
for folder in (ROOT / "COMem", ROOT / "exp", OLD, HERE):
...
def fresh_reference(model, tok, arm, document, row, cap):
    ...
    reader = CoMemLower(model, 12, tok) if arm == "fix_all" else CoMem(model, 0 if arm in ("j0", "prefix") else 12, tokenizer=tok)
...
def main(argv=None):
    ...
    with OutputLock(args.out):
        ...
        try:
            ...
            started = clock()
            warm = CoMem(model, 0, tokenizer=tok)
