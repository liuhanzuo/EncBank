...
edit('sections/tab_accuracy.tex',[
    ('All scores are on a 0--100 scale',r'Both CoMem variants use split depth $j=12$. All scores are on a 0--100 scale'),
    (r'CoMem (without LoRA) ($j=12$)$^*$',r'CoMem (without LoRA)'),
    (r'\textbf{CoMem} ($j=12$)',r'\textbf{CoMem}'),
...
edit('sections/08_protocol.tex',[
    (r'The original adapter-free depth reference uses $j=9$. The new Table~\ref{tab:accuracy} frozen row instead uses $j=12$ with the same memory settings; Appendix~\ref{app:frozen-recheck} identifies its fresh evaluation inputs.',r'The CoMem and CoMem without LoRA configurations both use $j=12$; Appendix~\ref{app:frozen-recheck} specifies the no-adapter evaluation protocol.'),
    ...
    ("LongEval's common-support mean covers 8k--128k. Its separate six-length mean includes the 4k CoMem result of 92, yielding 72.83 over 600 examples.","LongEval's common-support mean covers 8k--128k."),
...
edit('sections/10_benchmarks.tex',[
    ...
    (r'The starred frozen row in Table~\ref{tab:accuracy} uses a new adapter-free Qwen3-8B evaluation at $j=12$.',r'CoMem without LoRA in Table~\ref{tab:accuracy} evaluates Qwen3-8B at $j=12$ with the suffix adapter disabled.'),
    ...
    ('The frozen CoMem control permits 48 generated tokens','CoMem without LoRA permits 48 generated tokens'),
])
...
for name in ('tab_ruler_a.tex','tab_longeval_detail.tex','tab_babilong_detail.tex','tab_longbench_detail.tex'):
    ...
    s=s.replace(r'\caption{',r'\caption{Both CoMem variants use $j=12$. ',1)
    p.write_text(s,encoding='utf-8')
p=PAPER/'sections/tab_locomo_categories.tex';s=p.read_text(encoding='utf-8')
s='\n'.join(l for l in s.splitlines() if not l.startswith('CoMem (without LoRA) $j=9$'))+'\n'
p.write_text(s,encoding='utf-8')
for p in (PAPER/'sections').glob('tab*.tex'):
    ...
    s=s.replace('The native-window losses on qa1/qa2 and the stronger CoMem qa5 mean are both retained. ','')
    s=s.replace("CoMem's separate 4k score of 92 yields a six-length mean of 72.83. ",'')
    s=s.replace(' Timing quantiles are as reported in the original submission.','')
    s=s.replace('The retained CoMem main-table latency','The CoMem main-table latency')
...
edit('sections/11_systems.tex',[
    ('At 8k and 16k, both methods are freshly measured in the same three processes under the 28 GB cap, with shuffled method and phase order. At 32k and 128k, CoMem\'s earlier observations are retained; their original 26 GB allocator cap is below the new limit.','At 8k and 16k, both methods are measured in the same three processes under the 28 GB cap, with shuffled method and phase order. At 32k and 128k, CoMem uses a stricter 26 GB allocator cap.'),
    ('Every successful fresh cell uses one warmup and three formal repetitions:','Dense at every length and CoMem at 8k/16k use one warmup and three formal repetitions per successful cell:'),
    ('Retained 32k/128k CoMem Prefill and TTFT instead','CoMem Prefill and TTFT at 32k/128k'),
    ('retained E2E uses','CoMem E2E at these lengths uses'),
    ...
    (' The separate H20 store-ready cohort uses one warmup and three measurements; dense is stock LoRA-off and CoMem is adapted. Preparation and external fetch are excluded, so its ratios are composed online operating points rather than same-adapter or repeated-query estimates.',''),
    ('Original pooled-IID intervals are retained as sensitivity only','Pooled-IID intervals are reported as sensitivity only'),
])
edit('figures/source_scaling.tex',[("CoMem's earlier 32k/128k measurements used a tighter 26 GB cap.","CoMem's 32k/128k measurements use a tighter 26 GB cap.")])
