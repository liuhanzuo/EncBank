...
plan=read(B/'full_plan.json');prior=read(D/'heartbeat_babilong_20260909_0731.json')
old={(x['task'],x['length'],x['arm']) for x in prior['carried_forward_baseline_cells']+prior['writeback']}
assert len(old)==41
snapshot={'timestamp':datetime.now(timezone.utc).isoformat(),'host':os.uname().nodename,'ssh_alias':'longjing-1','prior_babi_cells':41,'completed_jobs':[],'incomplete_jobs':[],'invalid_receipts':[]}
...
D.mkdir(parents=True,exist_ok=True);(D/'heartbeat_accuracy_20260909_0831_snapshot.json').write_text(json.dumps(snapshot,indent=2)+'\n')
...
for name in ('babi_q15_32k_priority',):
...
(D/'heartbeat_accuracy_20260909_0831_health.json').write_text(json.dumps(health,indent=2)+'\n')
print(json.dumps({'snapshot':snapshot['timestamp'],'new_receipt_jobs':[j['id'] for j in snapshot['completed_jobs']],'invalid':snapshot['invalid_receipts'],'main':health['main_coordinator_counts'],'active':health['main_active'],'training':health['training'],'baseline_status':health['strong_baseline_bootstrap'],'babi_q15_32k':{k:v for k,v in health['babi_q15_32k_priority'].items() if k!='state'},'published':health['published_progress']},indent=2))
