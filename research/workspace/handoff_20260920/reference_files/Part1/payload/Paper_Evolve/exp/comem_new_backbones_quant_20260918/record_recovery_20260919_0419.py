...
根目录本地F:/Paper_Evolve/exp/comem_new_backbones_quant_20260918，远端/srv/encbank/qcomem_align_codex_20260911/comem_new_backbones_quant_20260918，SSH gpu-node1。先读README_zh.md顶部及monitor_state.json，运行collect_recovery_status.py读取当前Slurm、owner、短日志、Judge。monitor里recovery字段仍含9月18日旧恢复，勿误当最新；以owner.recovery_evidence为准。轻量读状态，不反复读/哈希大checkpoint。
...
【调度与恢复】
effective_plan.json28项，科学plan.json保持。独立最多4本agent RUNNING+PENDING GPU请求，每项1卡，其他agent额度独立，绝不操作其任务。共享qcomem_gpu_admission.lock及唯一owner决定接续；owner活跃时不旁路sbatch/起第二owner。本地任务已完成，local_gpu_reservation=null。
...
【独立本地任务已完成，不重启】
F:/Paper_Evolve/exp/comem_bge_matched_20260918/REVIEWER_RESULTS_zh.md及summary/verification已交付。BGE-Qasper A三轮校准及200题三臂600答案真实wait0。raw k6/k8/CoMem k12 TTFT510.827/633.882/577.956ms，F1 4.54653/5.16746/11.81033，未满足预定±5%，只能称邻近工作点；不追加k7/调样本/预算。配对148context簇CI已交付，600正式答案自然封顶128、无空答错误；旧4题Question片段离线边界已修正，原分数不可混用。文档bank/index准备完，store-ready TTFT含真实query BGE/search/fetch/query Write/prefill，不是冷E2E。controller85660和GPU已退出，reservation已释放。
已有2400请求、40更新的B/C整理已交付：Write均值.893752s，Q10 raw/CoMem18.946839/11.165820s，Q80 153.998186/82.306705s，均10/10获益；观察80问内crossover中位2范围1–3。输出长度不同，非固定输出加速。维护为分项核算，追加.064755/.747659s、插入.362858/.697493s、40/40 H相同；含共同全BM25重建+search，非增量BM25或连续wallclock，未计token化/持久IO/元数据事务，非改事实QA。KV独立LoRA已对齐：LongEval74.67/72.67、Qasper4.25/11.37，不重训、不跨环境算速度。条件LoCoMo200对新语义重评分未启动，不自行加API calls。
