...
def main():
    ...
    admission=acquire_gpu(cap_gb=27*2**30/1e9,poll=10,max_wait=21600,tag='encbank_bge_matched_TTFT')
    ...
    readers=dict(raw=original.Encbank(model,0,tokenizer=tok),encbank=original.Encbank(model,12,tokenizer=tok))
    ...
    metadata=dict(phase=a.phase,process=a.process,model_revision=spec['model_revision'],adapter_sha256=adapter_sha,
        ...
        admission=admission,cap_bytes=27*2**30,raw_ks=ks,encbank_k=12,torch=torch.__version__,
    ...
    with (dest/'records.jsonl').open('w',encoding='utf-8') as f,(dest/'write.jsonl').open('w') as wf:
        for row in rows:
            ...
            t=stamp();bank=[readers['encbank'].write_chunk(c).cpu().pin_memory() for c in chunks];write_s=stamp()-t
            ...
            arms=[('encbank',12)]+[('raw',k) for k in ks]
            ...
            for rep in ([-1,0] if a.phase=='quality' else [-1,0,1,2]):
                ...
                for method,k in order:
                    ...
                    states=[bank[i].to('cuda',non_blocking=True) if method=='encbank' else reader.write_chunk(chunks[i]) for i in selected]
