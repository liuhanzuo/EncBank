"""Build the bounded six-benchmark Encbank queue plan; never start model work."""
...
def locomo_categories(workspace):
    remote = Path(path_at(workspace, "exp/encbank_v2_benchmarks_20260908/data/locomo/locomo10.json"))
...
def reused_ruler(arms):
    ...
    add("exp/encbank_v2_benchmarks_20260908/results/ruler_cbos_8k_64k.json", NIAH_TASKS, ["8k", "64k"], ["cbos"])
    return entries


def make_job(args, benchmark, arm, spec, base):
    root = args.workspace_root
    entry_root = path_at(root, "exp/encbank_v2_benchmarks_20260908")
...
def build_plan(args):
    ...
    plan = {"schema_version": 1, "cwd": str(args.workspace_root).replace("\\", "/"),
        ...
        "mode": mode, "arms": arms, "env": {"ENCBANK_REMOTE_QUEUE": "1", "PAPER_EVOLVE_ALLOW_8B": "1"},
