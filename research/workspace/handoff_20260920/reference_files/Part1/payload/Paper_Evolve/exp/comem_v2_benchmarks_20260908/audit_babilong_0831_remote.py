...
OUT = ROOT / "outputs/diagnostics/heartbeat_babilong_20260909_0831.json"
PRIOR = ROOT / "outputs/diagnostics/heartbeat_babilong_20260909_0129.json"
BASELINE_PRIOR = ROOT / "outputs/diagnostics/heartbeat_babilong_20260909_0731.json"
...
snapshot=read(ROOT / "outputs/diagnostics/heartbeat_accuracy_20260909_0831_snapshot.json")
snapshot_keys={(j["cells"][0]["task"],j["cells"][0]["length"],j["arm"]) for j in snapshot["completed_jobs"] if j["benchmark"]=="babilong"}
require(len(already_audited)==41,"Prior41 baseline cells expected")
ready, pending = {}, []
for task in TASKS:
...
require(sum(map(len,ready.values()))==8,"Expected eight new cells in fixed snapshot")
