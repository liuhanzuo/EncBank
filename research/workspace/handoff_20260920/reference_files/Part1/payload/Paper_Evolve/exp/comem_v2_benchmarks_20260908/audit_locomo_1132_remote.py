"""Only newly full isolated KV LoCoMo arm; remote CPU full official/input/cache audit."""
import os
...
snapshot=read(B/'heartbeat_extended_20260909_1132_snapshot.json')
eligible=set(snapshot['eligible_native_markers']);state=snapshot['state']
targets=('cbos',);refs=('fix_all','j0','pub_sink');expected_counts={1:282,2:321,3:96,4:841,5:446}
prior=read(R/'outputs/diagnostics/heartbeat_accuracy_20260909_0005_qa.json')
prior_bos=read(B/'heartbeat_locomo_20260909_1032_new.json')
...
for arm in targets+refs:
    ...
    for shard in range(10):
        ...
        info=marker['reader'];assert info['class']==('CoMemLower' if arm in ('fix_all','cbos') else 'CoMem')
        assert info['requested_j']==12 and info['effective_j']==(0 if arm=='j0' else 36 if arm=='cbos' else 12)
        if arm in targets:assert info['write_sink'] is True
        ...
        for sample,row in zip(samples,rr):
            ...
            if arm in targets:
                ...
                if arm in old_scores:
                    saved=old_scores[arm][row['id']];assert saved['task']==row['task'];near(saved['score'],row['score'])
                    checks['old_v2_j0_per_record_score_identity_checks']+=1
                checks['previously_audited_reference_rows_read']+=1
    ...
    print(f'LoCoMo {arm}: source identities and {"new official scores" if arm in targets else "previously audited reference rows"} 1986/1986',flush=True)
ordered=sorted(samples_byarm['cbos'].values(),key=lambda x:x['index'])
for count,sample in enumerate(ordered,1):
    ...
    if count%200==0:print(f'LoCoMo source/token/ordered-pack/own-cache {count}/1986 x1 new arm',flush=True)
...
for cat,n in expected_counts.items():
    ...
    for arm in refs:
        old=[c for c in (prior['writeback']+prior_bos['writeback']) if c['benchmark']=='locomo' and c['task']==task and c['arm']==arm]
        assert len(old)==1 and old[0]['n']==n;near(scores[arm],old[0]['score_percent']);checks['previously_audited_reference_aggregate_identity_checks']+=1
    ...
    for arm in targets:
        ...
        comparisons.append({'benchmark':'locomo','task':task,'n':n,'new_arm':arm,'v2_minus_new_arm_pp':scores['fix_all']-scores[arm],'j0_minus_new_arm_pp':scores['j0']-scores[arm],'bos_minus_new_arm_pp':scores['pub_sink']-scores[arm]})
        cells.append(cell)
assert checks['official_rescores']==checks['new_exact_source_token_pack_own_generation_cache_checks']==1986
assert checks['reconstructed_source_token_ordered_packs']==1986
assert input_keys.hexdigest()==prior_bos['source_input_keys_sha256']
assert not torch.cuda.is_initialized()
report={'snapshot_timestamp_utc':snapshot['timestamp_utc'],'finished_at_utc':datetime.now(timezone.utc).isoformat(),
        'scope':'New complete cbos LoCoMo only; 1986 distinct outputs partitioned into five categories.',
        ...
        'protocol':{'shared_options':shared_options,'shared_protocol':shared_protocol,'generation_cap':50,
         ...
         'pairing':'Every new source token sequence and selected ordered pack was reconstructed once per source and matched the new arm own SQLite prediction/n_tokens key; references use old audited scores with matching source sample and ordered pack, zero old scorer/cache calls.',
         'limits':'CoMem retrieval/chat/greedy adaptation, not unmodified official HF generation; no overall mixed-category mean, no significance or local timing/memory claim. Original official_qa_driver logs do not print the RULER native admission line: completed remote queue GPU/job/exit records are checked, but per-job GPU admission memory samples were not persisted.'}}
dest=B/'heartbeat_locomo_20260909_1132_new.json';assert not dest.exists();dest.write_text(json.dumps(report,indent=2)+'\n')
