...
def main():
    ...
    readers={arm:CoMem(model,j,tokenizer=tok) for arm,j in [('raw',0),('comem',a.j)]}
    ...
    with sdpa_kernel([SDPBackend.FLASH_ATTENTION,SDPBackend.EFFICIENT_ATTENTION]),(out/'records.jsonl').open('w') as f:
        for row in rows:
            ...
            for rep in [-1,0,1,2]:
                start=stamp();bank=[readers['comem'].write_chunk(ch).cpu().pin_memory() for ch in chunks]
                sink_bank=readers['comem'].write_chunk([151643]).cpu().pin_memory();write_ms=(stamp()-start)*1000
                ...
                for arm in arms:
                    ...
                    record=dict(id=row['id'],cell=row['cell'],source_sha256=row['source_sha256'],j=a.j,arm=arm,rep=rep,warmup=rep<0,
                        ...
                        write_ms=write_ms if arm=='comem' else 0.,H_bytes=cache_bytes if arm=='comem' else 0,index_build_ms=index_ms,
