...
e2e=json.loads((ROOT/'exp/comem_e2e_20260913/summary.json').read_text())
rows=[]; details=[]; report=[]
fields=[('prefill','latency_ms'),('ttft','latency_ms'),('e2e','latency_ms'),('prefill','peak_GB'),('ttft','peak_GB')]
for length,folder,label in [(8192,None,'8k'),(16384,None,'16k'),(32768,'comem_infra_20260912','32k'),(131072,'comem_infra_128k_20260912','128k')]:
    if folder:
        old=json.loads((ROOT/'exp'/folder/'summary.json').read_text())['summary']['comem_k12']
        end=e2e['summary'][str(length)]['comem_k12']; dense=data['summary'][str(length)]
        cm={'prefill':{'status':'ok',**old['read']},'ttft':{'status':'ok',**old['ttft']},'e2e':{'status':'ok',**end}}
    else:
        dense=short['summary'][str(length)]['dense']; cm=short['summary'][str(length)]['comem']
    ...
    rows.append(' & '.join(['',r'\textbf{CoMem}',*cells(c,d)])+r'\\')
    report.append('| '+' | '.join([label,'Dense',*[('OOM' if v is None else f'{v:.1f}') for v in d[:3]],*[('OOM' if v is None else f'{v:.2f}') for v in d[3:]]])+' |')
    report.append('| '+' | '.join([label,'CoMem',*[('OOM' if v is None else f'{v:.1f}') for v in c[:3]],*[('OOM' if v is None else f'{v:.2f}') for v in c[3:]]])+' |')
    for method,measurements in ([('Dense',dense)] if folder else [('Dense',dense),('CoMem',cm)]):
...
At each length, three saved PG19 documents supply the complete source and its next 512 tokens as the query: book indices 0/1/2 at 8k, 16k, and 32k, and 0/1/32 at 128k. Queries therefore differ across source lengths. These are fixed, unscored systems workloads. CoMem's iterative token-ID BM25 uses the first 32 query tokens, hop width two, and top-12 source-order chunks of 512 tokens. Its suffix receives 6,657 residual-state positions at $j=12$: 6,144 cached source positions, 512 query positions, and one BOS sink. Dense instead receives all 8,705, 16,897, 33,281, or 131,585 token IDs without retrieval.
...
text=text.replace('Main-table latency is the median of these three process medians;','The retained CoMem main-table latency is the median of these process medians;')
f.write_text(text,encoding='utf-8')
...
if all(short['summary'][str(n)][m]['e2e']['status']=='ok' for n in (8192,16384) for m in ('dense','comem')):
    ratios=[short['summary'][str(n)]['dense']['e2e']['latency_ms']/short['summary'][str(n)]['comem']['e2e']['latency_ms'] for n in (8192,16384)]
    paragraph+=rf" The Dense-to-CoMem E2E ratios are ${ratios[0]:.2f}\times$ at 8k and ${ratios[1]:.2f}\times$ at 16k, with document Write charged to each CoMem request."
...
text=text.replace(r'Its speedup reaches $2.74\times$ at 128k, but CoMem is slower at 8k and 16k;',
                  r'On B200, the pipeline speedup reaches $2.74\times$ at 128k, but CoMem is slower at 8k and 16k;')
f.write_text(text,encoding='utf-8')
report='\n'.join(['# 5090 主表基线更正：完整 source Dense 对比 CoMem','',
  ...
  '|---|---|---:|---:|---:|---:|---:|',*report,'',
  '按作者要求删除 Read tokens 列。CoMem从选中12个片段的j=12中间层残差缓存继续计算；source长度、原始token输入与缓存状态的序列位置分别说明。所有实测延迟、显存和OOM结果保持原值。', '',
  'Dense 通过标准模型 forward 一次处理 BOS + 全部 source + 512-token query。32k/128k的Dense包含54个文档/进程/阶段组合；新增8k/16k同时测Dense与CoMem，包含108个组合。每组使用三个独立进程和每进程三文档；成功组各1次预热、3次正式重复。28GB为整个CUDA allocator预算，包含权重、KV和临时张量；不是额外28GB激活额度。桌面/驱动context开销另计。Dense没有截断、量化、offload或换用检索包。', '',
  'OOM条目来自实际CUDA内存分配失败，错误和完整输入shape均保存；失败没有完成延迟或完成峰值，不能填0或据此计算speedup。32k/128k CoMem保留原26GB上限下已经完成的观测（低于19GB allocated）；没有增加其重复次数。8k/16k两种方法均在新的28GB上限下测量。', '',
...
for name in ('README.md','LOCAL_INFRA_RESULTS_zh.md','E2E_RESULTS_zh.md','POSITIONING_zh.md','CLAIM_EVIDENCE.md',
    ...
    note=('> Current main-panel correction: Table 2(a) compares full-source Dense with CoMem under a 28 GB total CUDA allocator cap. Earlier top-12 replay values below are retained depth diagnostics, not the Dense baseline. See `DENSE5090_RESULTS_zh.md`.\n\n'
          if name in ('README.md','CLAIM_EVIDENCE.md') else
          '> 当前主表更正：Table 2(a) 改为完整 source Dense 对比 CoMem，使用28GB总CUDA allocator预算。下文早先的top12 replay数值仅为深度诊断，不是Dense基线。见 DENSE5090_RESULTS_zh.md。\n\n')
