...
CONTROL='/tmp/qcm-deploy-control-liuhanzuo-20260919-r1'
...
def main():
    assert json.loads((ROOT/'submission.json').read_text())['job']=='108438'
...
from pathlib import Path
p=json.load(sys.stdin);control=Path('/tmp/qcm-deploy-control-liuhanzuo-20260919-r1')
...
'''
    text=ssh(code,dict(bootstrap=(ROOT/'bootstrap_r1.py').read_text()));record=json.loads(text)
