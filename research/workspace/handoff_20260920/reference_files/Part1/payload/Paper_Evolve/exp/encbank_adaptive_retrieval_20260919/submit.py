...
CONTROL='/tmp/qcm-adaptive-control-liuhanzuo-20260919'
...
def main():
    ...
    with tarfile.open(ROOT/'payload.tar','w') as tar:
        ...
        for name in ['workloads.json','PROTOCOL_zh.md','agent_config.json']:tar.add(ROOT/name,arcname=name)
...
from pathlib import Path
p=json.load(sys.stdin);control=Path('/tmp/qcm-adaptive-control-liuhanzuo-20260919')
...
assert not any('|qcm-adaptive-b300|' in l for l in queue.splitlines())
command=['sbatch','--parsable','--job-name=qcm-adaptive-b300','--dependency=afterok:108559,singleton','--partition=gpu','--nodelist=gpu-node1','--nodes=1','--ntasks=1','--gres=gpu:nvidia_l20d:1','--cpus-per-task=8','--mem=96G','--time=12:00:00','--chdir=/tmp','--output=/tmp/qcm-adaptive-%j.out','--error=/tmp/qcm-adaptive-%j.err']
...
if r.returncode==0:
 record['job']=r.stdout.strip().split(';')[0];assert record['job'].isdigit();record['stage']='/tmp/qcm-adaptive-liuhanzuo-'+record['job'];record['node']='gpu-node1'
