...
ARMS = {"comem": ("comem", 1), "beacon4": ("beacon", 4),
...
def decode_prepared(row):
    ...
    require(protocol.get("format") == "official-full-conversation-qwen3-comem-v1", "Unknown preparation protocol")
...
def main():
    ...
    require(all(initial.get(k) == v for k, v in {"j": args.j, "rank": args.rank, "alpha": args.alpha}.items()), "Original CoMem adapter dimensions differ")
    require(initial.get("step") == 4000 and initial.get("path") == "base", "Initialize from the original published-path 4000-step CoMem adapter")
