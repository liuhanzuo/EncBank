...
def main():
    ...
    dump(out/'metadata.json',{'job':os.environ['SLURM_JOB_ID'],'node':platform.node(),'gpu':torch.cuda.get_device_name(0),'torch':torch.__version__,'transformers':transformers.__version__,'methods':['encbank','replay_no_adapter'],'frozen_method':'re-score existing full j12 1150 saved predictions','prompt':'released last-chunk query','bos':'tokenizer BOS or first input token (released default)','filter':'reference/clean_subset_ids.json; identical full support; filtering applied after generation','source_headline_predictions':'raw files not available through current repository; these full-set evaluations replace corresponding full-set scores after completion'})
    ...
    with (out/'predictions.jsonl').open('w',encoding='utf-8') as f:
        for row in rows:
            chunks,query=parts(row);ix=selection(chunks,row);arms=['encbank','replay_no_adapter'];rng.shuffle(arms)
            ...
            for arm in arms:
                ...
                with contextlib.nullcontext() if arm=='encbank' else wrapper.disable_adapter():
