...
for name in ('comem','eval'):
    dest=HERE/'COMem'/name;dest.mkdir(parents=True,exist_ok=True)
    for p in (HERE.parents[1]/'COMem'/name).glob('*.py'):shutil.copy2(p,dest/p.name)
with tarfile.open(HERE/'payload.tar.gz','w:gz',compresslevel=3) as tar:
    for name in ('common.py','run_quality.py','run_quality.slurm','samples.jsonl.gz','sample_summary.json','PROTOCOL_zh.md','COMem'):
