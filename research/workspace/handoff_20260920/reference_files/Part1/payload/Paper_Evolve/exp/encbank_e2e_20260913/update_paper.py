...
arms=('replay_k12','encbank_k12')
rows=[]; detail=[]; report=[]
for length,old_name,label in [(32768,'encbank_infra_20260912','32k'),(131072,'encbank_infra_128k_20260912','128k')]:
    ...
    for idx,arm in enumerate(arms):
        name=r'\textbf{Encbank}' if arm=='encbank_k12' else 'Raw replay'
        ...
        if arm=='encbank_k12': rows.append(r'\rowcolor{resultshade}')
        rows.append(' & '.join([label if idx==0 else '',name,'12',*cells])+r'\\')
        v=new[arm]
        detail.append(' & '.join([label,'Encbank' if arm=='encbank_k12' else 'Raw replay',
          *[f'{m:.1f}' for m in v['process_medians_ms']],f"{v['latency_ms']:.1f}",f"{v['peak_GB']:.2f}"])+r'\\')
        report.append(f"| {label} | {'Encbank' if arm=='encbank_k12' else 'Raw replay'} | {v['latency_ms']:.1f} | {v['peak_GB']:.2f} |")
...
if all(e2e['summary'][str(n)]['encbank_k12']['latency_ms'] > e2e['summary'][str(n)]['replay_k12']['latency_ms'] for n in (32768,131072)):
    sentence="With document Write charged to each request, Encbank's measured E2E is higher than replay at both lengths."
...
report='\n'.join(['# RTX 5090：完整流程 E2E（含文档 Write，输出 128 tokens）','',
    ...
    'E2E 直接测量从预分词 source/query 到生成完 128 个 token：含新建 CPU pinned store、完整 Encbank document Write、BM25 构建/检索、fetch、sink/query Write、prefill 与 127 次 decode。每次请求重新准备文档，不摊销 Write；两方法强制相同输出长度，EOS 不提前结束。', '',
...
if 'The RTX 5090 E2E supplement' not in text:
    text+='\nThe RTX 5090 E2E supplement directly times source preparation, including full document Write for Encbank, through 128 generated tokens on the same local workloads and adapter. It covers 108 requests in three independent processes. This is first-use model-pipeline cost with pretokenized inputs; the separately reported store-ready TTFT and historical amortization results have different boundaries. See `E2E_RESULTS_zh.md`.\n'
f.write_text(text,encoding='utf-8')
...
import runpy
dense=ROOT/'exp/encbank_dense5090_20260913'
