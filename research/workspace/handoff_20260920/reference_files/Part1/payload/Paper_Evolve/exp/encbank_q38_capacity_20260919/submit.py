...
def main():
    ...
    ssh("import os;from pathlib import Path;p=Path("+repr(CONTROL)+");assert os.getuid()==20021 and not p.exists();p.mkdir(mode=0o700)")
...
old=subprocess.check_output(['scontrol','show','job','108559'],universal_newlines=True)
assert 'JobName=qcm-budget-b300' in old and 'JobState=PENDING' in old,old
subprocess.check_call(['scontrol','hold','108559'])
cmd=['sbatch','--parsable','--job-name=qcm-q38-capacity','--dependency=afterok:108475,singleton','--partition=gpu','--nodelist=gpu-node1','--nodes=1','--ntasks=1','--gres=gpu:nvidia_l20d:1','--cpus-per-task=8','--mem=96G','--time=06:00:00','--chdir=/tmp','--output=/tmp/qcm-q38-capacity-%j.out','--error=/tmp/qcm-q38-capacity-%j.err']
...
if r.returncode==0:
 ...
 subprocess.check_call(['scontrol','update','JobId=108559','Dependency=afterok:'+job+',singleton'])
 subprocess.check_call(['scontrol','release','108559'])
 record['budget_new_dependency']='afterok:'+job+',singleton'
else:subprocess.check_call(['scontrol','release','108559'])
