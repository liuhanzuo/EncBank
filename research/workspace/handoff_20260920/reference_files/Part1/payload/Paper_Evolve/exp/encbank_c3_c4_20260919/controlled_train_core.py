"""Published-path Encbank LoRA: bf16 backbone, restartable, optional layer sharding.

No GPU is selected implicitly. See train_8b_RECIPE.md for the distinction between
the published support-normalized loss and the historical S21 implementation.
"""
...
class PublishedReader:
    ...
    def __init__(self, model, j, grad_checkpoint=True):
        ...
        self.reference = Encbank(model, resume_j=j)
...
def export_adapter(out, payload, args, layers):
    ...
    config = {"peft_type": "LORA", "task_type": "CAUSAL_LM", "inference_mode": True,
              ...
              "layers_to_transform": list(range(args.adapter_start, layers)), "layers_pattern": "layers"}
...
def parser():
    ...
    ap.add_argument("--adapter-start", type=int, default=18)
...
def main():
    ...
    assert 0 < args.j <= args.adapter_start < len(model.model.layers)
    modules = attach_lora(model, args.adapter_start, args.rank, args.alpha, getattr(torch, args.adapter_dtype))
    params = [p for m in modules.values() for p in (m.A, m.B)]
    trainable = [{"name":n,"shape":list(p.shape),"numel":p.numel(),"dtype":str(p.dtype)}
                 for n,p in model.named_parameters() if p.requires_grad]
    assert len(model.model.layers)==36 and args.adapter_start==18
    assert sum(v['numel'] for v in trainable)==sum(p.numel() for p in params)
    assert len(trainable)==18*7*2
    atomic_json(out/'trainable_parameters.json',trainable)
    initial_hash = hashlib.sha256()
    for name, tensor in flat_state(modules).items():
    ...
    config = {k: getattr(args, k) for k in ("j", "adapter_start", "rank", "alpha", "chunk", "n_ctx", "topk", "lam",
    ...
    metadata = {"recipe": config, "args": vars(args), "data": data_meta, "placement": placement,
                ...
                "initial_adapter_sha256": initial_hash.hexdigest(),
                "gpu":torch.cuda.get_device_name(devices[0]),
                "gpu_properties":str(torch.cuda.get_device_properties(devices[0])),
                "slurm_job":os.environ.get('SLURM_JOB_ID'),
                "matched_budget":"Identical trainable layout, initialization, data and steps; not FLOP-matched",
    ...
    def save(step, final=False):
        ...
        atomic_json(out / "status.json", {"step": step, "target_steps": args.steps,
                    ...
                    "loss_query_tokens_processed": step * args.grad_accum * args.chunk,
    ...
    with (out / "train.jsonl").open("a", encoding="utf-8") as log:
        for step in range(completed, stop):
            ...
            if completed == 1 or completed % args.log_every == 0:
                ...
                atomic_json(out / 'progress.json', {**row, 'start_step': int(saved['step']) if args.resume else 0,
                    'target_steps': args.steps, 'j': args.j, 'adapter_start': args.adapter_start,
