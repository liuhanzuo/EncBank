...
def main():
    ...
    reports=[]
    for doc in docs:
        arm={a:sorted([r for r in rows if r['document_id']==doc and r['arm']==a],key=lambda r:r['stream_index']) for a in ['raw','comem']}
        assert all(len(v)==80 and [r['stream_index'] for r in v]==list(range(80)) for v in arm.values())
        assert [r['id'] for r in arm['raw']]==[r['id'] for r in arm['comem']]
        store=load(source/(doc+'_store.json'));write=store['write_seconds']
        assert all(r['cold_write_seconds']==write for r in arm['comem'])
        cumulative={}
        for a,rs in arm.items():
            total=write if a=='comem' else 0.;vals=[]
            for r in rs:total+=r['online_seconds'];vals.append(total)
            cumulative[a]=vals
        gains=[cumulative['comem'][i]<cumulative['raw'][i] for i in range(80)]
        ...
        reports.append(dict(document_id=doc,initial_write_s=write,raw_Q10_s=cumulative['raw'][9],
            comem_Q10_s=cumulative['comem'][9],raw_Q80_s=cumulative['raw'][79],comem_Q80_s=cumulative['comem'][79],
    ...
    summary={k:statistics.mean(r[k] for r in reports) for k in ['initial_write_s','raw_Q10_s','comem_Q10_s','raw_Q80_s','comem_Q80_s']}
    summary.update(gain_Q10_documents=sum(r['gain_Q10'] for r in reports),gain_Q80_documents=sum(r['gain_Q80'] for r in reports),
        ...
        output_tokens={a:statistics.mean(r['generated_tokens'] for r in rows if r['arm']==a) for a in ['raw','comem']})
    ...
    payload=dict(complete=True,new_gpu=False,source=str(source),receipt=receipt,reuse=summary,documents=reports,maintenance=maintenance,
        boundaries=dict(reuse='Pretokenized existing documents; both arms online include same observed BM25 rebuild/search. CoMem adds once-only measured H Write. No separately persisted BM25 index; common input tokenization/tensor setup excluded symmetrically.',
    ...
    md=['# 已有复用与更新日志重算','',f"来源：{source}；2400请求、40更新及真实父进程wait0已核对。无新LLM运行。",'',
        '## 逐文档摊销','', '| 文档 | 初始H Write秒 | Raw Q10秒 | CoMem Q10秒 | Raw Q80秒 | CoMem Q80秒 | 持续crossover | Q80获益 |',
        '|---|---:|---:|---:|---:|---:|---:|---|']
    for r in reports:md.append('| '+r['document_id']+' | '+' | '.join(f"{r[k]:.3f}" for k in ['initial_write_s','raw_Q10_s','comem_Q10_s','raw_Q80_s','comem_Q80_s'])+f" | {r['sustained_crossover_within_80'] or '80问内未出现'} | {'是' if r['gain_Q80'] else '否'} |")
    md+=['',f"平均初始Write {summary['initial_write_s']:.3f}秒；Q10 raw/CoMem {summary['raw_Q10_s']:.3f}/{summary['comem_Q10_s']:.3f}秒，{summary['gain_Q10_documents']}/10文档获益。Q80为{summary['raw_Q80_s']:.3f}/{summary['comem_Q80_s']:.3f}秒，{summary['gain_Q80_documents']}/10获益。",
        f"80问内持续crossover出现于{len(crosses)}/10文档；中位位置{summary['crossover_median']}，范围{summary['crossover_range']}。只对已观察80问成立。",
        f"平均输出tokens raw/CoMem {summary['output_tokens']['raw']:.2f}/{summary['output_tokens']['comem']:.2f}；自然生成工作量不同，不能称固定输出加速。",
        '', '两侧均计入原请求的BM25全量重建+search；没有另一个未计入的持久检索索引。CoMem只额外加一次实际测得的H Write。共同token化、初始CPU chunk构造均在边界外。', '',
