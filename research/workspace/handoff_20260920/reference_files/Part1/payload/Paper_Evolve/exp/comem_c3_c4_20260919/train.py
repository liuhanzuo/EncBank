...
def main():
    ...
    cmd=[sys.executable,'-u','-B',str(config.ROOT/'controlled_train_core.py'),
        ...
        '--out',str(out),'--devices','cuda:0','--j',str(a.j),'--adapter-start',str(config.ADAPTER_START),
    ...
    from controlled_train_core import digest
    (out/'complete.json').write_text(json.dumps(dict(complete=True,smoke=a.smoke,step=status['step'],actual_wait=True,
        j=a.j,seed=a.seed,adapter_start=18,train_input_tokens=status['step']*4096,
        loss_query_tokens=status['step']*512,adapter_sha256=digest(out/'final/adapter_model.safetensors') if not a.smoke else None)))
