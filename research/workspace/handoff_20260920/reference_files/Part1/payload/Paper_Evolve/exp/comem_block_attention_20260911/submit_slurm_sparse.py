"""Render, or explicitly submit, one serial single-L20D sparse-CoMem Slurm job.
...
TASK_ROOT = HOME_BOUNDARY / 'comem_sparse_slurm_20260912'
SOURCE_ROOT = PurePosixPath('/data/liuhanzuo/comem_v2_20260908')
SOURCE_CODE = SOURCE_ROOT / 'workspace/exp/comem_block_attention_20260911'
SOURCE_OUT = SOURCE_ROOT / 'outputs/sparse_comem_20260911'
JOB_NAME = 'comem-sparse-1gpu'
MIGRATION_SCHEMA = 'comem-sparse-slurm-migration-v1'
...
def check_other_jobs(task_root=TASK_ROOT, worker=None):
    ...
    for line in text.splitlines():
        ...
        if same_root or same_worker:
            jobs.append(dict(job_id=job_id, name=name, state=state))
    if jobs:
        raise RuntimeError(f'Existing duplicate sparse-CoMem task prevents another allocation: {jobs}')
...
def main():
    ...
    with confined(Path(HOME_BOUNDARY)/'.codex-comem-sparse-l20d.submit.lock').open('a') as lock:
