...
limits=[
 ...
 'RULER scores use the original Encbank adaptation answer-substring recall; variable_tracking is five-needle retrieval, not fact-composition accuracy. Equal target counts permit the exact matched-target fraction, not whole-example accuracy.',
...
d={'timestamp_utc':datetime.now(timezone.utc).isoformat(),'snapshot_timestamp_utc':s['timestamp_utc'],
   ...
   'negative_results':[
    ...
    'Original Encbank single256k34 is below the previous V2/replay100/100; no general length-monotonicity or causal attribution is established.',
...
md+=['','pub=原始Encbank，cbos=isolated full KV，不能以cbos代表CacheBlend。256k单针为17/50；128k五针为152/250个答案目标的平均substring recall，不是152个独立问题，也不是whole-example accuracy。','',
     '100条新raw输出通过原Encbank RULER评分复算、连续i0–49、完整summary、无OOM、模型/reader参数、远端准入和PYTHONHASHSEED日志核对。与V2/j0旧已核记录的task/length/i/answers/n_tokens及CLI参数逐例一致。旧参考评分没有重新调用。','',
...
md+=['','isolated KV五针128k平均60.8%，低于此前V2的93.2%和Replay的100%；但它有3例得分超过V2，保留该逐例不利结果。Encbank单针256k34%，此前V2和Replay均100%；不同长度不能据此推断单调下降或隔离因果。','',
     '## LoCoMo：覆盖未齐，不报局部均值','',
