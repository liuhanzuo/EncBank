...
def validate(model,tok,by_k):
    ...
    for k,docs in by_k.items():
        for method in ['raw','h_gpu']:
            ...
            logits=CoMem.decode_step(engine.reader,token,bottom,top,qp,length)
    ...
    cb.dump(ROOT/'correctness.json',dict(passed=True,checks=checks,scope='two decoding steps against original CoMem read_prefill/decode_step for every budget and method; not task quality'))
