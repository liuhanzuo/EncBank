...
def collect_remote(requested_job, tail_lines):
    ...
    root = Path('/srv/encbank/encbank_sparse_slurm_20260912')
    output = root / 'outputs/sparse_encbank_20260911'
    now = time.time()
    ...
    if requested_job == 'latest':
        ...
        for path in sorted((root/'logs').glob('submission-*.json')):
            ...
            if (receipt.get('job_name') != 'encbank-sparse-1gpu'
    ...
    try:
        ...
        if proc.returncode == 0:
            if (fields.get('JobId') != job_id or fields.get('JobName') != 'encbank-sparse-1gpu'
...
def main():
    ...
    command = ['ssh', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=12', 'gpu-node1',
               'env PYTHONDONTWRITEBYTECODE=1 CUDA_VISIBLE_DEVICES= /srv/encbank/qencbank_runtime_20260911/python312/bin/python -B -']
    try:
