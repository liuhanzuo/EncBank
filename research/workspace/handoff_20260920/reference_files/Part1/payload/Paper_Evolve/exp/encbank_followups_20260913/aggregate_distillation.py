...
def main():
    ...
    delta=arrays['encbank']-arrays['without_lora'];bdelta=delta[ix].mean(axis=1)
    result={'complete':True,'books':50,'windows':100,'positions_per_arm':51200,'resamples':10000,'unit':'book','arms':arms,'paired_encbank_minus_without_lora':{k:{'mean':float(delta[:,j].mean()),'book_bootstrap_95ci':np.quantile(bdelta[:,j],[.025,.975]).tolist()} for j,k in enumerate(keys)}}
