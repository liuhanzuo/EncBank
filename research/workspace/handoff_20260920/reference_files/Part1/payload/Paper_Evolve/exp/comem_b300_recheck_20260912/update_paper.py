...
full,cm=s['dense_shared'],s['comem_read']
...
panel+=r'\textbf{CoMem} & \textbf{'+f"{cm['latency_ms']:,.1f}"+r'} & \textbf{'+f"{cm['peak_GB']:.2f}"+r'} & $\mathbf{'+f"{ratios['same_adapter_full_vs_comem_read']:.1f}"+r'}\times$\\'+'\n'
table=table[:start]+panel+table[end:]
table=table.replace(r'\textbf{(b)} Stock dense versus adapted CoMem; prewritten store, fetch excluded.',r'\textbf{(b)} Same adapter; full prompt versus selected pack. Document/query Write and fetch excluded; three processes, three documents, three repeats each.')
f.write_text(table,encoding='utf-8')
readrows=[]
for arm,name,adapter,pack in [('dense_stock','Dense','Off','131,585'),('dense_shared','Dense','Shared','131,585'),('replay_read','Raw replay','Shared','6,657'),('comem_read','CoMem','Shared','6,657')]:
    v=s[arm]; readrows.append(' & '.join([name,adapter,pack,f"{v['latency_ms']:,.1f}",f"{v['peak_GB']:.2f}"])+r'\\')
ttftrows=[]
for arm,name in [('replay_ttft','Raw replay'),('comem_ttft','CoMem')]:
...
for arm in ('dense_stock','dense_shared','replay_read','comem_read','replay_ttft','comem_ttft'):
    v=s[arm]; report.append(f"| {arm} | {'Dense' if arm.startswith('dense') else ('CoMem' if arm.startswith('comem') else 'Raw replay')} | {'Off' if arm=='dense_stock' else 'Shared'} | {v['latency_ms']:.1f} | {v['peak_GB']:.2f} |")
report+=['',f"同 adapter 的完整 dense / CoMem Read：{ratios['same_adapter_full_vs_comem_read']:.2f}×。同 pack 的 raw replay / CoMem Read：{ratios['same_pack_depth_read']:.3f}×；对应 TTFT：{ratios['same_pack_ttft']:.3f}×。",'',f"关闭 adapter 的 dense / CoMem Read：{ratios['stock_full_vs_comem_read']:.2f}×，仅作历史口径诊断。H20 的 50.59 / 1.32 = 38.3258× 算术成立；本轮更换了硬件、工作负载、checkpoint 和软件，不能据此证实或否定历史 H20 的精确值。",'','完整输入为 131,585 tokens，选中 pack 为 6,657 tokens；大倍数包含检索压缩上下文和跳过下层两种效应。Read 为 GPU-ready prefill，TTFT 另含 CPU 检索、fetch、sink/query Write；均排除 document Write、tokenization 与 decode。本轮不新增准确率结论。','', '主表中以 B300 同 adapter 复测替换旧 H20 运行点；H20 数值保留在附录原表。原 B200 pipeline 只更正硬件名称，数值不变。']
report='\n'.join(report)+'\n'
