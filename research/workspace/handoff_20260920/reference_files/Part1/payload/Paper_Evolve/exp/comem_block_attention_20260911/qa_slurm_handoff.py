...
DESTINATION = '/srv/encbank/comem_sparse_slurm_20260912'
SCHEMA = 'comem-qa-slurm-resource-handoff-v1'
...
def validate_declaration(path, expected_sha, queue):
    ...
    if (not re.fullmatch(r'[1-9][0-9]*', job) or job != str(value['destination_job_id'])
            ...
            or submission.get('job_name') != 'comem-sparse-1gpu'
    ...
    for key, expected in (('--chdir', DESTINATION), ('--gres', 'gpu:nvidia_l20d:1'),
                          ('--nodes', '1'), ('--ntasks', '1'), ('--job-name', 'comem-sparse-1gpu')):
