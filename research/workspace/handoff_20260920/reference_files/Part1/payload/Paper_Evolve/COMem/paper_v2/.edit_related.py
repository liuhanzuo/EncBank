"""Delete the pre-CoMem material from Section 2, keeping the one thing in it that was load-bearing.

The owner asked for everything before the CoMem subsection to go. Two paragraphs are affected:
...
end = s.index("\\subsection{CoMem: the base method}")
cut = s[start:end]
...
print(f"02_related: cut {len(cut.split())} words before the CoMem subsection "
