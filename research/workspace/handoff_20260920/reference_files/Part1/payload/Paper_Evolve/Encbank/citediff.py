"""Which of Encbank's citations did paper_v2 drop, and which of its own bib entries are dead?
...
print(f"Encbank cites {len(v1c)}, paper_v2 cites {len(v2c)}\n")
print("DROPPED -- Encbank cites these, paper_v2 does not:")
for k in sorted(v1c - v2c):
    print(f"  {k:20s} {title.get(k, '?')[:78]}")
print("\nDEAD -- in paper_v2's bib but never cited:")
for k in sorted(v2b - v2c):
    print(f"  {k:20s} {title.get(k, '?')[:78]}")
print("\nADDED -- paper_v2 cites these, Encbank does not:")
