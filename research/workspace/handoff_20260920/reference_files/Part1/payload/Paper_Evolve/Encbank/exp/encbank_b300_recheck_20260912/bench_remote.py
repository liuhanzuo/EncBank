...
def main():
    ...
    dump(out/'metadata.json',metadata)
    cm,rp=Encbank(model,12,tokenizer=tok),Encbank(model,0,tokenizer=tok)
    ...
    with sdpa_kernel([SDPBackend.FLASH_ATTENTION,SDPBackend.EFFICIENT_ATTENTION]):
        ...
        for wi,w in enumerate(workloads[:1] if a.smoke else workloads):
            ...
            arms=['dense_stock','dense_shared','replay_read','encbank_read','replay_ttft','encbank_ttft']; rng.shuffle(arms)
            for name in arms:
                gc.collect(); torch.cuda.empty_cache()
                ready=None
                if name.endswith('_read'):
                    reader=cm if name.startswith('encbank') else rp
                ...
                def operation():
                    ...
                    r=cm if name.startswith('encbank') else rp
