...
sys.path.insert(0, str(HERE / 'encbank_infra_20260912'))
...
def main():
    ...
    for length in (8192, 16384, 32768, 131072):
        dense = HERE / 'encbank_dense5090_20260913/workloads' / f'{length}.json'
        ...
        if length in (32768, 131072):
            e2e = HERE / 'encbank_e2e_20260913/workloads' / dense.name
            e2e.parent.mkdir(parents=True, exist_ok=True)
            if e2e.exists():
                raise FileExistsError(e2e)
            shutil.copy2(dense, e2e)
        if length == 131072:
            target = HERE / 'encbank_b300_recheck_20260912/workloads.json'
            if target.exists():
