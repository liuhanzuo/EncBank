"""Move two pre-model Hot89 shards away from observed BeeGFS-stalled lj-gpu1."""
import fcntl
import hashlib
import json
import os
...
R = S / 'hot24_peft_node_recovery_20260924'
...
def prepare():
    ...
    R.mkdir()
    rows = []
    for index, old_job in [(2, 140659), (3, 140660)]:
    ...
    assert len(rows) == 2 and len(set(t for row in rows for t in row['tasks'])) == 57
