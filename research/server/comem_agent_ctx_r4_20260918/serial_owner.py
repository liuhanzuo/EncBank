...
OLD = H.parent / 'comem_agent_memory_20260917'
PREFIXES = ('qcomem-agentmem-',)
OUT = H / 'serial_queue'
...
def verify_package():
    ...
    compiler = json.loads((H / 'environment_check.json').read_text())
    assert compiler['actual_compile_exit'] == compiler['actual_import_exit'] == 0 and compiler['actual_parent_wait']
    for name, expected in compiler['headers'].items():
        assert hashlib.sha256((Path(compiler['CPATH']) / name).read_bytes()).hexdigest() == expected, name
    transport = json.loads((H / 'transport_check.json').read_text())
    assert transport['actual_exit_code'] == 0 and transport['actual_parent_wait']
    assert json.loads(transport['stdout'])['status'] == 'SAVED_NATIVE_TOOL_PUBLIC_API_EXECUTION_PASS'
...
assert hashlib.sha256((H / 'agent_plan.json').read_bytes()).hexdigest() == 'ef8f89b86634464c3c99bbae41105aa8596ce112695fc93af3f5c1087db04665'
...
try:
    ...
    for arm in plan['arms']:
        ...
        while True:
            with (H / 'appworld_gpu_admission.lock').open('a+b') as admission:
                fcntl.flock(admission, fcntl.LOCK_EX)
                jobs = queue(); count = sum(j['gpus'] for j in jobs)
                assert not any('qcomem-agentmem-' in j['name'] for j in jobs), 'Another AppWorld job is active'
                if count < 4:
                    verify_package()
                    jobs = queue(); count = sum(j['gpus'] for j in jobs)
                    assert count < 4
                    argv = ['sbatch', '--parsable', str(H / (arm + '.slurm'))]
                    record = dict(at=now(), arm=arm, argv=argv, prior_jobs=jobs, prior_gpu_requests=count, status='submission_intent', shared_admission_lock=str(H / 'appworld_gpu_admission.lock'))
                    save(receipt, record)
