...
# Baselines that bypass the Encbank read-pack entirely and run the stock HF model
...
# Prefill-then-compress KV-cache baselines (see encbank.kvcompress): still a stock
...
def resolve_baseline(baseline, resume_j, lora_adapter):
    ...
    * ``snapkv`` / ``pyramidkv`` -> stock FULL prefill, then compress the stored KV
      to a bounded per-layer budget and decode from it (no Encbank, no LoRA).
    * ``cacheblend``   -> same retrieval / pack / sink as Encbank but the cached
...
def resolve_reader(cm, mode, recompute_ratio=0.15):
    ...
    ``cacheblend`` -> a :class:`encbank.cacheblend.CacheBlend` wrapping ``cm``
    (signature-compatible with :meth:`encbank.Encbank.generate_from_ids`, so the
    ...
    print(f"[encbank-eval] baseline cacheblend: full-depth chunk KV "
...
def install_baseline(model, mode, kv_budget=6657, kv_window=32):
    ...
    if mode not in KVCOMPRESS_MODES:
        return None
    from encbank.kvcompress import install_kv_compression
    cfg = install_kv_compression(model, mode, max_capacity_prompt=kv_budget,
                                 window_size=kv_window)
    print(f"[encbank-eval] baseline {mode}: retained KV budget {kv_budget}/layer "
...
def resolve_dense_retriever(selector, retriever_path, device="cuda:0",
    ...
    print(f"[encbank-eval] selector dense_bge: frozen retriever {retriever_path}")
...
def dense_generate(model, tokenizer, input_ids, mode="dense", max_new_tokens=32,
    ...
    * ``snapkv`` / ``pyramidkv`` -> full (exact) prefill of the whole prompt, then
      ...
      (see :mod:`encbank.kvcompress`; needs :func:`install_baseline` first).
    ...
    try:
        if mode in KVCOMPRESS_MODES:
            from encbank.kvcompress import generate_kvcompress
