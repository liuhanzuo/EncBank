"""One allocated GPU; two real recovery child processes; later independent CPU aggregation."""
...
def main():
 ...
 try:
  identity=require_identity(query(),None);batch['physical_gpu_identity']=identity;save(root/'batch_status.json',batch)
  for arm in ARMS:
  ...
  batch.update(status='two_recovery_arms_exited_complete_pending_independent_aggregation',recovery_analysis_required=True,reused_arms_not_executed=['dense','h16','h8','h4']);save(root/'batch_status.json',batch)
  success=True
 except BaseException as error:
  batch['error']={'type':type(error).__name__,'message':str(error),'traceback':traceback.format_exc()}
 finally:
  batch.update(status='two_recovery_arms_complete_shell_slurm_and_aggregation_pending' if success else 'stopped_without_retry_not_complete_recovery_evidence',finished_at=now())
