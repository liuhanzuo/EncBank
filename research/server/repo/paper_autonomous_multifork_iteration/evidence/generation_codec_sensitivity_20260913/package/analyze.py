"""Saved-output official LongBench QA F1; require all five successful native exits first."""
...
def main():
    ...
    assert batch['plan_sha256']==args.expected_plan_sha256 and batch['status']=='all_five_arms_exited_complete_pending_analysis'
    closed={}
    ...
    label_map={x['id']:x for x in labels['items']};assert set(label_map)==set(ids) and len(ids)==32 and len(groups)==32
    qa_f1,official_scorer=metric_functions(plan);reports={};scores={};prefix_scores={}
    for arm in ARMS:
        ...
        assert result['arm_configuration']==plan['arm_configuration'][arm]
        rows=result['rows'];docs=result['documents'];phases=result['phases'];names=collections.Counter(x['name'] for x in phases)
        assert rows and [x['id'] for x in rows]==ids and len(docs)==32 and len(phases)==193
        assert phases==worker['phases'] and all(x['completed'] for x in phases)
        assert names=={'outer_model_load_native_quality_and_release':1,'document':32,'Write':32,'Store_verification_before':32,'Read_complete_query_and_natural_decode':32,'Store_verification_after':32,'entry_release':32}
        ...
        for index,(row,item) in enumerate(zip(rows,items)):
            ...
            tokens=row['generated_token_ids'];assert 1<=len(tokens)<=64 and 151645 not in tokens
            assert row['generation_length']==len(tokens) and row['max_new_tokens']==64
            assert row['hit_generation_cap']==(len(tokens)==64) and row['stopped_on_eos']==(len(tokens)<64)
        ...
        assert sorted(r['execution_index'] for r in rows)==list(range(32))
        ...
        prefixes=[r['posthoc_prefix32'] for r in rows]
        for row,prefix in zip(rows,prefixes):
            assert prefix['generated_token_ids']==row['generated_token_ids'][:32]
            assert prefix['length']==len(prefix['generated_token_ids']) and not prefix['is_independent_cap32_run']
            assert not prefix['semantic_correctness_measured']
        prefix_predictions=[r['prediction'] for r in prefixes]
        prefix_scores[arm]=[max(qa_f1(pred,ref,all_classes=label_map[item_id]['all_classes']) for ref in references) for item_id,pred,references in zip(ids,prefix_predictions,refs)]
        for doc in docs:
            cfg=plan['arm_configuration'][arm]
            assert doc['store']['bits']==cfg['bits'] and doc['store']['group_size']==cfg['group_size']
            reconstruction=doc['reconstruction'];chunks=[r for r in reconstruction if r['kind']=='chunk'];sink=[r for r in reconstruction if r['kind']=='sink']
            assert len(chunks)==doc['store']['chunk_count'] and len(sink)==1
            assert [r['chunk_index'] for r in chunks]==list(range(len(chunks)))
            assert sum(r['N'] for r in chunks)==doc['document_tokens']*4096
            assert sum(r['stored_tensor_bytes'] for r in chunks)==doc['store']['packed_chunk_H_bytes']
            assert sink[0]['stored_tensor_bytes']==doc['store']['native_FP16_sink_bytes'] and sink[0]['N']==4096
            for r in reconstruction:
                assert r['metadata_rounding_included'] and r['padding_excluded'] and not r['tensor_host_export']
                assert r['device'].startswith('cuda') and r['squared_error_sum']>=0 and r['max_absolute_error']>=0
                assert r['zero_denominator']==(r['reference_squared_sum']==0)
                if r['zero_denominator']:assert r['relative_L2'] is None
                else:assert r['relative_L2']==(r['squared_error_sum']/r['reference_squared_sum'])**0.5
                if r['bits']==16:assert r['squared_error_sum']==r['max_absolute_error']==0
        official=official_scorer('musique',predictions,refs,[]);assert official==round(100*statistics.fmean(scores[arm]),2)
        reports[arm]={'n':32,'documents':32,'official_QA_F1_percent':official,'unrounded_mean_F1_percent':100*statistics.fmean(scores[arm]),
            ...
            'generation_lengths':[len(r['generated_token_ids']) for r in rows],'per_item_request_timing':{r['id']:r['timing'] for r in rows},'per_item_position_usage':{r['id']:r['position_usage'] for r in rows},'per_item_unscaled_regime':{r['id']:r['unscaled_input_regime'] for r in rows},'Writes':32,'phase_count':193,
            'result_sha256':sha(out/'result.json'),'worker_sha256':sha(out/'worker.json'),'execution_sha256':sha(out/'execution.json')}
        reports[arm]['posthoc_prefix32_diagnostic']={'is_independent_cap32_run':False,'semantic_correctness_measured':False,
            'unrounded_QA_F1_percent':100*statistics.fmean(prefix_scores[arm]),
            'per_item_F1':dict(zip(ids,prefix_scores[arm])),
            'per_item_full64_minus_prefix32_F1_points':{i:100*(a-b) for i,a,b in zip(ids,scores[arm],prefix_scores[arm])},
            'per_item_changes':{r['id']:r['full64_vs_posthoc_prefix32_changes'] for r in rows},
            'negative_F1_change_count':sum(a<b for a,b in zip(scores[arm],prefix_scores[arm])),
            'zero_F1_count':sum(x==0 for x in prefix_scores[arm]),'raw_empty_count':sum(not r['prediction'] for r in prefixes)}
        reports[arm]['per_entry_store_and_reconstruction']={d['document_id']:{k:d[k] for k in ('store','reconstruction','reconstruction_summary')} for d in docs}
    # Exact retrieval/input/native-H denominators must match; differences in answers remain evidence.
    base=read(closed['h16'][0]/'result.json')
    for arm in ARMS[1:]:
        current=read(closed[arm][0]/'result.json')
        assert [r['selected_chunk_indices'] for r in current['rows']]==[r['selected_chunk_indices'] for r in base['rows']]
        for a,b in zip(current['documents'],base['documents']):
            assert [(r['N'],r['reference_squared_sum']) for r in a['reconstruction']]==[(r['N'],r['reference_squared_sum']) for r in b['reconstruction']]
        reports[arm]['per_item_vs_h16_output_changes']={a['id']:{'token_sequence_changed':a['generated_token_ids']!=b['generated_token_ids'],'decoded_answer_changed':a['prediction']!=b['prediction'],'F1_delta_points':100*(scores[arm][i]-scores['h16'][i])} for i,(a,b) in enumerate(zip(current['rows'],base['rows']))}
    assert plan['analysis']['seed']==20260911 and plan['analysis']['draws']==10000 and plan['analysis']['zero_based_endpoints']==[249,9749]
    rng=random.Random(20260911);draws=[[rng.randrange(32) for _ in range(32)] for _ in range(10000)]
    contrasts={}
    for arm,ref in plan['analysis']['contrasts']:
        ...
        contrasts[arm+'_minus_'+ref]={'mean_points':statistics.fmean(delta),'descriptive_95_percentile_interval':[values[249],values[9749]],'matched_reader_codec_contrast':True}
        prefix_delta=[100*(a-b) for a,b in zip(prefix_scores[arm],prefix_scores[ref])]
        contrasts[arm+'_minus_'+ref].update(posthoc_prefix32_contrast_mean_points=statistics.fmean(prefix_delta),full64_minus_posthoc_prefix32_contrast_change_points=statistics.fmean(delta)-statistics.fmean(prefix_delta),prefix_is_independent_cap32_run=False)
    save(args.output,{'status':'complete_full_MuSiQue160_answers_160_Writes_965_phases_pending_parent_shell_Slurm_and_independent_verification',
        ...
        'resampling':plan['analysis'],'scope':'Exploratory MuSiQue source-order first32/D32 cap64 codec sensitivity; selected after adverse original H4 result, not heldout; no original cap32 rerun',
        'limits':['Exploratory dataset selection after adverse H4 result; fixed source-order subset is not heldout or a fresh confirmatory benchmark','Same native SDPA primitive policy is not identical CUDA dispatch or attention graph','Intervals including0 do not establish equivalence/noninferiority; unadjusted exploratory contrasts; prefixes are posthoc length diagnostics and not human correctness; additional reconstruction reductions affect Write phase allocator memory; read-only host integrity digest excluded from TTFT']})
