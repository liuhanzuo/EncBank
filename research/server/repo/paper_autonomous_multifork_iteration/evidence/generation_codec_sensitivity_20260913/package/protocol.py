...
CONFIG = {'resume_j': 12, 'chunk_size': 512, 'selector': 'iter_bm25', 'topk': 12,
          'iter_hop_topk': 4, 'iter_rounds': 0, 'H_group_size': 'arm_configuration[arm].group_size',
...
def preflight(args,envelope=False):
    ...
    assert plan['schema'] == 'exploratory_musique32_FP16_generation_codec_v1'
    assert plan['arm_order'] == list(ARMS) and plan['configuration'] == CONFIG
    assert plan['arm_configuration']=={'h16':{'bits':16,'group_size':64},'h8':{'bits':8,'group_size':64},'h4':{'bits':4,'group_size':64},'h4g32':{'bits':4,'group_size':32},'h4g128':{'bits':4,'group_size':128}}
    ...
    assert coverage['N']==32 and coverage['D']==32 and coverage['max_prompt_plus_cap']==17947
    ...
    lengths={x['id']:x for x in coverage['items']};assert len(lengths)==32
    ...
    assert len(items) == len({r['id'] for r in items}) == plan['items_per_arm'] == 32
    assert len(groups) == plan['documents_per_arm'] == 32
    assert plan['expected_complete_phases_per_arm'] == 4*32 + 2*32 + 1 == 193
    for row in items:
        assert not ({'answer','answers','gold','reference','references','outputs','all_classes','evidence'} & set(row))
        assert row['dataset'] == 'musique' and row['max_new_tokens'] == 64
        ...
        assert len(full)==expected['full_prompt_tokens_with_BOS'] and len(full)+64==expected['prompt_plus_cap']<=17947
