...
def main():
 ...
 job='qencbank-ruler-single32k100-cachefix-codex'
 cache='/srv/encbank/qencbank_align_codex_20260911/task_cache/ruler_single32k100_cache_fixed_attempt1'
 ...
 write(P/'qencbank_triton_cache.py',(FIX/'qencbank_triton_cache.py').read_text('utf-8'))
 assert sha(P/'qencbank_triton_cache.py')==fix['fix_module']['sha256'];sources[rel(P/'qencbank_triton_cache.py')]=sha(P/'qencbank_triton_cache.py')
 ...
 plan['triton_cache_cleanup_fix']={'base_plan':bind(BASE/'plan.json'),'base_science_checks':bind(OLD/'focused_checks_attempt1/report.json'),'base_actual_held_CPU_exit':bind(OLD/'focused_checks_attempt1/execution_receipt.json'),
  'module':bind(P/'qencbank_triton_cache.py'),'fix_provenance':bind(FIX/'provenance.json'),'captured_actual_installed_source':fix['captured_actual_installed_source'],'focused_report':fix['focused_report'],'actual_held_CPU_exit':fix['actual_held_CPU_exit'],'assembler':bind(Path(__file__)),
 ...
 paths.add(P/'qencbank_triton_cache.py')
 ...
 save(H/'source_delta.json',{'status':'NEW_CACHE_FIXED_PACKAGE_FOCUSED_CHECKS_PENDING','base_plan':bind(BASE/'plan.json'),'new_plan':bind(P/'plan.json'),'new_manifest':bind(P/'upload_manifest.json'),'sources':deltas,'new_module':bind(P/'qencbank_triton_cache.py'),
