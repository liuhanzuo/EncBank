...
def preflight(args,envelope=False):
    ...
    assert plan['schema'] == 'RULER_VT128K100_FP16_admission_continuation_6_method_v1'
    ...
    assert plan['full_input_original_window']==40960 and plan['full_input_coverage_maximum']==131072
    ...
    assert max(61+len(row['document_token_ids'])+len(row['query_token_ids']) for row in items)==131072
    ...
    for row in items:
        ...
        assert row['dataset'] == 'ruler_variable_tracking_128k' and row['max_new_tokens'] == 60
        ...
        assert token_sha(full) == row['full_prompt_with_BOS_token_sha256'] and len(full)+60 <= 131072
