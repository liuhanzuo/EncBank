"""Six-method RULER VT native Slurm bindings; check-only imports no model framework."""
import argparse, hashlib, importlib.util, json, sys
from pathlib import Path

HERE = Path(__file__).resolve().parent
ROOT = HERE.parents[4]
ARMS = ('h16', 'h8', 'h4', 'kivi2', 'kivi4')
...
CONFIG = {'resume_j': 12, 'chunk_size': 512, 'selector': 'iter_bm25', 'topk': 12,
          ...
          'first_step_EOS_suppressed': True, 'max_new_tokens': 60, 'query_tokens_per_call': 1}
...
def preflight(args,envelope=False):
    ...
    assert plan['schema'] == 'RULER_VT64K100_FP16_admission_continuation_5_method_v1'
    assert plan['arm_order'] == list(ARMS) and plan['configuration'] == CONFIG
    assert plan['recovery_provenance']['fresh_arms'] == list(ARMS)
    assert set(plan['recovery_provenance']['reused_completed_arms']).isdisjoint(ARMS)
    ...
    assert plan['full_input_original_window']==40960 and plan['full_input_coverage_maximum']==65576
    ...
    assert max(61+len(row['document_token_ids'])+len(row['query_token_ids']) for row in items)==65576
    ...
    for row in items:
        ...
        assert row['dataset'] == 'ruler_variable_tracking_64k' and row['max_new_tokens'] == 60
        ...
        assert token_sha(full) == row['full_prompt_with_BOS_token_sha256'] and len(full)+60 <= 65576
