...
def main():
 ...
 try:
  ...
  batch.update(status='five_remaining_arms_exited_complete_pending_independent_aggregation',reused_arms_not_executed=['dense'],independent_aggregation_required=True);save(root/'batch_status.json',batch)
  success=True
 except BaseException as error:
  batch['error']={'type':type(error).__name__,'message':str(error),'traceback':traceback.format_exc()}
 finally:
  batch.update(status='five_remaining_arms_complete_shell_slurm_and_aggregation_pending' if success else 'stopped_without_retry_not_complete_continuation_evidence',finished_at=now())
