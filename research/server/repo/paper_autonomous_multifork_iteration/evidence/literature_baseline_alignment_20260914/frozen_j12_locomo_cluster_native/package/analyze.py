...
def main():
    ...
    labels={'items':[__import__('json').loads(line) for line in local(plan['labels']['path']).read_text(encoding='utf-8').splitlines() if line.strip()]}
    label_map={x['id']:x for x in labels['items']};assert set(label_map)==set(ids) and len(ids)==1986 and len(groups)==10
    score_sample,official_scorer=metric_functions(plan);reports={};scores={}
    assert collections.Counter(x['category'] for x in labels['items'])=={1:282,2:321,3:96,4:841,5:446}
    score_root=local(plan['batch_output'])/'lexical_scoring_only';score_root.mkdir(exist_ok=False)
    for arm in ARMS:
        ...
        assert rows and [x['id'] for x in rows]==ids and len(docs)==10 and len(phases)==4013
        assert phases==worker['phases'] and all(x['completed'] for x in phases)
        assert names=={'outer_model_load_native_quality_and_release':1,'document':10,'Write':10,'Store_verification_before':10,'Read_complete_query_and_natural_decode':1986,'Store_verification_after':1986,'entry_release':10}
        ...
        for index,(row,item) in enumerate(zip(rows,items)):
            ...
            assert isinstance(row['raw_generated_text'],str) and isinstance(row['prediction'],str)
            assert row['source_id']==item['id'] and row['source_ordinal']==item['source_ordinal']
            regime=row['unscaled_input_regime'];assert regime['prompt_plus_cap']==1+len(item['document_token_ids'])+len(item['query_token_ids'])+48<=22299
            assert not regime['above_published32768'] and not regime['above_original40960_with_cap']
        ...
        assert sorted(r['execution_index'] for r in rows)==list(range(1986))
        ...
        native_items=[{'id':row['id'],'pred':row['prediction'],'answers':label_map[row['id']]['answers'],
                      'category':label_map[row['id']]['category'],'is_abstention':label_map[row['id']]['is_abstention'],
                      'question':item['question_text']} for row,item in zip(rows,items)]
        directory=score_root/arm;directory.mkdir(exist_ok=False)
        import json
        ...
        # The exact pinned official run_scoring and its exact score_sample functions execute on CPU only.
        official=official_scorer(directory)
        per_item=[score_sample(item) for item in native_items]
        for key in ('f1','em','acc'):
            assert official['overall_'+key]==sum(x[key] for x in per_item)/1986*100
        assert official['n_samples']==1986 and {k:v['n'] for k,v in official['by_category'].items()}=={'1':282,'2':321,'3':96,'4':841,'5':446}
        scores[arm]=per_item
        reports[arm]={'n':1986,'documents':10,'native_lexical_metrics':official,'per_item_native_lexical':dict(zip(ids,per_item)),
            'raw_empty_predictions':sum(not x for x in predictions),'stripped_empty_predictions':sum(not x.strip() for x in predictions),
            'capped':sum(r['hit_generation_cap'] for r in rows),'EOS_stops':sum(r['stopped_on_eos'] for r in rows),
            'generation_lengths':[len(r['generated_token_ids']) for r in rows],'per_item_request_timing':{r['id']:r['timing'] for r in rows},
            'per_item_position_usage':{r['id']:r['position_usage'] for r in rows},'Writes':10,'phase_count':4013,
            'result_sha256':sha(out/'result.json'),'worker_sha256':sha(out/'worker.json'),'execution_sha256':sha(out/'execution.json'),
            'official_predictions_CPU_only':str(directory/'preds.jsonl'),'official_scores_sha256':sha(directory/'scores.json')}
    assert plan['analysis']['contrasts']==[]
    contrasts={}
    for arm,ref in plan['analysis']['contrasts']:
        contrasts[arm+'_minus_'+ref]={key:100*statistics.fmean(a[key]-b[key] for a,b in zip(scores[arm],scores[ref])) for key in ('f1','em','acc')}
    save(args.output,{'status':'complete_prediction_and_native_lexical_1986_answers_10_Writes_4013_phases_pending_parent_shell_Slurm_and_independent_verification',
