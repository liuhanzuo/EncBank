...
ALLOWED=['gpu-node1','gpu-node3','gpu-node4','gpu-node5','gpu-node6','gpu-node7']
...
def validate_backend_binding(plan,*,allow_unbound=False):
    ...
    assert plan['arm_order'] in (['encbank_frozen_j12'],['public_kvdirect_j0'])
    ...
    assert policy['scheduler']=={'partition':'gpu','constraint':'l20d','exclude':['gpu-node2','gpu-node8'],'nodes':1,'gres':'gpu:1'}
    ...
    assert old['model_root']==plan['model_root'] and old['python']==plan['python'] and all(plan['native_attention_runtime_files'].get(k)==v for k,v in old['native_attention_runtime_files'].items())
