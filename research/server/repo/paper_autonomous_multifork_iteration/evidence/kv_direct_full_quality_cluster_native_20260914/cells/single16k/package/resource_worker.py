...
def main():
 ...
 try:
  ...
  with torch.inference_mode(),profiler.phase('outer_model_load_native_quality_and_release'):
   ...
   assert model.config.max_position_embeddings==40960 and getattr(model.config,'rope_scaling',None) is None
   record['effective_model_config']={key:getattr(model.config,key,None) for key in ('max_position_embeddings','rope_theta','rope_scaling')}
