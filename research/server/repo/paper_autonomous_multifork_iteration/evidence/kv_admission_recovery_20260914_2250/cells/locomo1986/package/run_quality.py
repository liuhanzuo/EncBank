...
def run(model, tokenizer, fixture, arm, *, plan, output, provenance, profiler, natural_read, kernels=None):
    ...
    method = Method(model, tokenizer, arm, resume_j=0, reader_binding=plan['reader_binding'],
                    bos_token_id=151643, eos_token_id=151645, kernels=kernels)
    assert arm=='public_kvdirect_j0' and method.memory.engine.resume_j==0 and method.memory.bits==16
    ...
    result = {'schema':plan['schema'],'status':'running','arm':arm,
        ...
        'adapter_active':False,'resume_j':0,'scope':plan['preparation_scope']}
            ...
            with profiler.phase('document', document_id=document_id):
                try:
                    ...
                    for index in indices:
                        ...
                        row['position_usage']={'mode':'KVDirect_j0_all_chunks_full_contiguous_Read','next_position':next_position,'maximum_consumed_position':next_position-1,'document_lower_layers':0}
