...
def load_labels(plan):
    ...
    labels=read(path);assert labels['fixture_sha256']==plan['fixture']['sha256'];return labels
...
def main():
    ...
    label_map={x['id']:x for x in labels['items']};assert set(label_map)==set(ids) and len(ids)==N and len(groups)==D
    reports={};scores={}
    for arm in ARMS:
