"""LongBench grouped immutable entries and full-query natural QA; no model import at module load."""
...
def run(model, tokenizer, fixture, arm, *, plan, output, provenance, profiler, natural_read, kernels=None):
    ...
    import torch
    from diagnostics import prefix_diagnostic, changes
    Method = load_method(plan)
    from codec_observation import aggregate
    method = Method(model, tokenizer, arm, resume_j=12, reader_binding=plan['reader_binding'],
                    bos_token_id=151643, eos_token_id=151645, kernels=kernels, group_size=plan['arm_configuration'][arm]['group_size'])
    result = {'schema':'exploratory_musique32_FP16_generation_codec_v1','status':'running','arm':arm,
        'configuration':plan['configuration'],'arm_configuration':plan['arm_configuration'][arm],'provenance':provenance,'rows':[],'documents':[],
        'phases':profiler.records,'complete_quality_evidence':False,
        'scope':'Exploratory MuSiQue source-order first32/D32 cap64 codec sensitivity; selected after adverse original H4 result, not heldout; no original cap32 rerun'}
    ...
    try:
        for document_id, indices in document_groups(fixture):
            ...
            with profiler.phase('document', document_id=document_id):
                try:
                    ...
                    doc['entry_writes'] = 1
                    doc['reconstruction'] = list(method.memory.last_reconstruction)
                    doc['reconstruction_summary'] = {k:aggregate(doc['reconstruction'],k) for k in ('chunk','sink')}
                    method.memory.last_reconstruction = []
                    ...
                    for index in indices:
                        ...
                        row['unscaled_input_regime']={'full_prompt_tokens':1+len(item['document_token_ids'])+len(item['query_token_ids']),'prompt_plus_cap':1+len(item['document_token_ids'])+len(item['query_token_ids'])+64,'above_published32768':1+len(item['document_token_ids'])+len(item['query_token_ids'])>32768,'above_original40960_with_cap':1+len(item['document_token_ids'])+len(item['query_token_ids'])+64>40960}
                        generated['question_for_blinded_assessment'] = tokenizer.decode(item['bare_question_token_ids'],skip_special_tokens=True)
                        generated['posthoc_prefix32'] = prefix_diagnostic(generated,tokenizer.decode)
                        generated['full64_vs_posthoc_prefix32_changes'] = changes(generated,generated['posthoc_prefix32'])
                        ...
                        row['retrieval_configuration'] = ({'selector':'iter_bm25','topk':12,'iter_hop_topk':4,
                            'iter_rounds':0,'chunk_size':512} if arm.startswith('h') else None)
                        ...
                        print(f'{arm}: {execution_index}/32 id={item["id"]} tokens={row["generation_length"]}',flush=True)
                finally:
        ...
        assert len(result['rows'])==32 and len(result['documents'])==32
        assert sum(d['entry_writes'] for d in result['documents'])==32
