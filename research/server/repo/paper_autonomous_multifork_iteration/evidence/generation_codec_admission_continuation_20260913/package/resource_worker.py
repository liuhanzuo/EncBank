"""Qualified single-GPU resource envelope; five FP16 H QA arms and natural timing."""
...
def main():
 ...
 try:
  ...
  def gate():
   ...
   require_identity(snapshot,identity);free,actual_total=torch.cuda.mem_get_info(0)
   snapshot.update(cuda_free_bytes=int(free),cuda_total_bytes=int(actual_total));save(output/'worker.json',record)
  ...
  with torch.inference_mode(),profiler.phase('outer_model_load_native_quality_and_release'):
   ...
   record['unscaled_input_coverage']=plan['input_coverage']
   save(output/'worker.json',record)
   ...
   provenance.update(fixture=plan['fixture'],physical_gpu_identity=identity,resource_policy=RESOURCE,scope='Exploratory MuSiQue source-order first32/D32 cap64 codec sensitivity; selected after adverse original H4 result, not heldout; no original cap32 rerun')
   ...
   provenance.update(backbone_dtype='torch.float16',attention_backend='sdpa',LoRA_parameter_dtypes=sorted(set(dtypes.values())),configuration={'resume_j':12,'H_group_size':plan['arm_configuration'][args.arm]['group_size'],'KIVI_group_size':32,'KIVI_residual_length':128,'query_tokens_per_call':1,'max_new_tokens':64,'BOS_token_id':151643,'EOS_token_id':151645,'first_step_EOS_suppressed':True})
   ...
   provenance['unscaled_coverage']={'input_coverage':plan['input_coverage'],'CPU_qualification':plan['high_position_CPU_qualification'],'effective_model_config':record['effective_model_config'],'original_max_position_embeddings':40960,'original_rope_theta':1000000,'YaRN_or_model_config_override_applied':False}
   ...
   provenance['executed_attention_policy']=('KIVI SDPA full-document prefill and official Half cached GEMV' if args.arm.startswith('kivi') else 'Native SDPA per existing selected-H or Dense tokenwise runtime')
   ...
   assert len(result['rows'])==plan['items_per_arm']==32 and len(result['documents'])==plan['documents_per_arm']==32
   assert len(profiler.records)==plan['expected_complete_phases_per_arm']==193 and all(x['completed'] for x in profiler.records[1:])
   del kernels
