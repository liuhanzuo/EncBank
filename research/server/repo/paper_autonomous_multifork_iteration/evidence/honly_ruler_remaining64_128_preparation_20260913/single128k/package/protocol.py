...
def preflight(args,envelope=False):
    ...
    assert plan['schema'] == 'RULER_single2_128K100_FP16_six_method_remote_quality_v1'
    ...
    assert plan['full_input_original_window']==40960 and plan['full_input_coverage_maximum']==130992
    ...
    assert max(49+len(row['document_token_ids'])+len(row['query_token_ids']) for row in items)==130992
    ...
    for row in items:
        ...
        assert row['dataset'] == 'ruler_niah_single_2_128k' and row['max_new_tokens'] == 48
        ...
        assert token_sha(full) == row['full_prompt_with_BOS_token_sha256'] and len(full)+48 <= 130992
