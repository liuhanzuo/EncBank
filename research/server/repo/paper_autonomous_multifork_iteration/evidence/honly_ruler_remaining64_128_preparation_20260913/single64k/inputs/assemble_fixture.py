...
rawpath=O/'scoring_only/official_raw_attempt1/niah_single_2/validation.jsonl'
...
for i,m in enumerate(boundaries):
 ...
 assert len(fullids)+128==raw[i]['length'] and raw[i]['length']<=65536
 assert 1+len(fullids)+48<=65536
 ...
 item_id=f'ruler_niah_single_2_64k_seed42_{i:03d}';doc_id=f'{item_id}_document'
 ...
 items.append({'sample_index':i,'item_id':item_id,'document_id':doc_id,'task':'niah_single_2','subset':'validation',
  ...
  'official_prompt_tokens_without_bos':len(fullids),'official_max_seq_length':65536,'official_generation_reserve':128,
