...
def main():
 ...
 assert args.arm=='public_hcache_j12' and not args.arm.startswith('h')
 ...
 try:
  ...
  with torch.inference_mode(),profiler.phase('outer_model_load_native_quality_and_release'):
   ...
   provenance.update(backbone_dtype='torch.float16',attention_backend='sdpa',LoRA_parameter_dtypes=sorted(set(dtypes.values())),configuration={'resume_j':12,'H_group_size':64,'KIVI_group_size':32,'KIVI_residual_length':128,'query_tokens_per_call':1,'max_new_tokens':plan['configuration']['max_new_tokens'],'BOS_token_id':151643,'EOS_token_id':151645,'first_step_EOS_suppressed':True})
   ...
   provenance['executed_attention_policy']=('KIVI SDPA full-document prefill and official Half cached GEMV' if args.arm.startswith('kivi') else 'Native SDPA public all-chunks contiguous causal recomputation; explicit complete tokenwise query')
