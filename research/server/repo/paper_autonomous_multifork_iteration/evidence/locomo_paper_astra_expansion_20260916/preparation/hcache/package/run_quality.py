...
def load_method(plan):
    ...
    adapter_path=local(plan['allchunks_runtime']['path'])
    assert sha(adapter_path)==plan['allchunks_runtime']['sha256']
    spec=importlib.util.spec_from_file_location('_public_allchunks_bound',adapter_path)
...
def run(model, tokenizer, fixture, arm, *, plan, output, provenance, profiler, natural_read, kernels=None):
    ...
    assert arm=='public_hcache_j12' and method.memory.engine.resume_j==12 and method.memory.bits==16
    ...
    result = {'schema':plan['schema'],'status':'running','arm':arm,
        ...
        'adapter_active':False,'resume_j':12,'scope':plan['preparation_scope']}
            ...
            with profiler.phase('document', document_id=document_id):
                try:
                    ...
                    for index in indices:
                        ...
                        expected=list(range(len(entry.chunks)))
                        assert row['selected_chunk_indices']==expected
                        assert row['packed_read_tokens']==1+len(item['document_token_ids'])+len(item['query_token_ids'])
                        row['retrieval_configuration']={'selector':'all_chunks','retrieval':False,'chunk_size':512,'source_order':True}
                        next_position=row['packed_read_tokens']+row['decode_forward_count']
                        row['position_usage']={'mode':'HCache_j12_all_chunks_upper_contiguous_Read','next_position':next_position,'maximum_consumed_position':next_position-1,'document_lower_layers':12}
                        row['unscaled_input_regime']={'full_prompt_tokens':row['packed_read_tokens'],'prompt_plus_cap':row['packed_read_tokens']+item['max_new_tokens'],'above_original40960_with_cap':row['packed_read_tokens']+item['max_new_tokens']>40960}
