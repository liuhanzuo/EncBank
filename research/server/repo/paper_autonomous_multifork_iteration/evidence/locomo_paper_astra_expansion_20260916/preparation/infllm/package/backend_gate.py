...
def validate_backend_binding(plan,*,allow_unbound=False):
    assert plan['launch_permitted'] is True and plan['attention_backend']=='infllm_upstream_torch_joint_softmax' and plan['backbone_dtype']=='float16'
    assert plan['arm_order'] in (['encbank_frozen_j12'],['infllm_resident_qwen3'])
