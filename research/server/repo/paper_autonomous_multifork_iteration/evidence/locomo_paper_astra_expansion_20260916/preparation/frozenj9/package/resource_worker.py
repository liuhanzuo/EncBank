...
from protocol import ROOT,HERE,RESOURCE,local,model_path,parser,preflight,read,save,sha,resolve_driver,load_fixture,N,D,PHASES
...
def main():
 ...
 assert args.arm=='encbank_frozen_j9' and not args.arm.startswith('h')
 ...
 try:
  ...
  with torch.inference_mode(),profiler.phase('outer_model_load_native_quality_and_release'):
   ...
   record['effective_model_config']={key:getattr(model.config,key,None) for key in ('max_position_embeddings','rope_theta','rope_scaling','rope_parameters','sliding_window','use_sliding_window','layer_types')}
   # HF5.5.4 normalizes the hash-bound null legacy RoPE config to this exact default dictionary.
   expected_rope={'rope_theta':1000000,'rope_type':'default'}
   assert model.config.max_position_embeddings==40960
   assert model.config.rope_parameters==expected_rope and getattr(model.config,'rope_scaling',None)==expected_rope
   assert model.config.sliding_window is None and model.config.use_sliding_window is False
   ...
   provenance.update(fixture=plan['fixture'],physical_gpu_identity=identity,resource_policy=RESOURCE,scope=plan['preparation_scope'])
   ...
   provenance.update(backbone_dtype='torch.float16',attention_backend='sdpa',LoRA_parameter_dtypes=sorted(set(dtypes.values())),configuration={'resume_j':9,'H_group_size':64,'KIVI_group_size':32,'KIVI_residual_length':128,'query_tokens_per_call':1,'max_new_tokens':plan['configuration']['max_new_tokens'],'BOS_token_id':151643,'EOS_token_id':151645,'first_step_EOS_suppressed':True})
   provenance['effective_model_config']=record['effective_model_config']
   provenance['unscaled_full_context_stress']=plan['unscaled_full_context_stress']
   ...
   provenance['executed_attention_policy']=('KIVI SDPA full-document prefill and official Half cached GEMV' if args.arm.startswith('kivi') else 'Native SDPA frozen Encbank j9 iterBM25 selected12; contiguous upper causal recomputation and complete tokenwise query')
   ...
   assert len(result['rows'])==plan['items_per_arm']==N and len(result['documents'])==plan['documents_per_arm']==D
   assert len(profiler.records)==plan['expected_complete_phases_per_arm']==PHASES and all(x['completed'] for x in profiler.records[1:])
   del kernels
