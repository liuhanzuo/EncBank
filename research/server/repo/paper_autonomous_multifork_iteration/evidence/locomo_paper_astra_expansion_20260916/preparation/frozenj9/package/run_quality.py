...
def run(model, tokenizer, fixture, arm, *, plan, output, provenance, profiler, natural_read, kernels=None):
    ...
    method = Method(model, tokenizer, "h16", resume_j=9, reader_binding=plan['reader_binding'],
                    bos_token_id=151643, eos_token_id=151645, kernels=kernels)
    assert arm=='encbank_frozen_j9' and method.memory.engine.resume_j==9 and method.memory.bits==16
                ...
                try:
                    ...
                    for index in indices:
                        ...
                        row['retrieval_configuration'] = ({'selector':'iter_bm25','topk':12,'iter_hop_topk':4,
                            'iter_rounds':0,'chunk_size':512} if arm=='encbank_frozen_j9' else None)
