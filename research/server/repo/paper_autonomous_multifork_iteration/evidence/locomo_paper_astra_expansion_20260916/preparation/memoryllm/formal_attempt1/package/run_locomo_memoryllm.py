...
def execute(args, data, admission_gate=None):
    import numpy as np
    import torch
    ...
    if any(b.device != device for b in model.buffers()) or getattr(model, 'hf_device_map', None) or any(hasattr(m, '_hf_hook') for m in model.modules()):
        raise RuntimeError('Model buffers or hooks violate fully resident no-offload execution')
    ...
    execution = {'protocol_id': args.protocol_id, 'input_sha256': args.input_sha256,
                 ...
                 'actual_parameter_dtypes': sorted({str(p.dtype) for p in model.parameters()}),
                 'actual_buffer_dtypes': sorted({str(b.dtype) for b in model.buffers()}),
                 'actual_memory_dtype': str(model.memory.dtype),
                 'generation_state_check': 'Memory pointer, tensor version and initialization flag; exact bitwise nonmutation was established only by separate synthetic CPU test.',
