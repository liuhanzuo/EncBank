...
def run(model, tokenizer, fixture, arm, *, plan, output, provenance, profiler, natural_read, kernels=None):
    ...
    result = {'schema':'full_hotpotqa200_FP16_six_method_remote_quality_v1','status':'running','arm':arm,
        ...
        'scope':'Full HotpotQA200, D200 reused entries; unscaled model and unchanged full inputs, no dropped long rows; natural quality timing only'}
    ...
    try:
            ...
            with profiler.phase('document', document_id=document_id):
                try:
                    ...
                    for index in indices:
                        ...
                        row['unscaled_input_regime']={'full_prompt_tokens':1+len(item['document_token_ids'])+len(item['query_token_ids']),'prompt_plus_cap':1+len(item['document_token_ids'])+len(item['query_token_ids'])+32,'above_published32768':1+len(item['document_token_ids'])+len(item['query_token_ids'])>32768,'above_original40960_with_cap':1+len(item['document_token_ids'])+len(item['query_token_ids'])+32>40960}
        ...
        assert sum(d['entry_writes'] for d in result['documents'])==200
