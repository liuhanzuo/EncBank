...
def preflight(args,envelope=False):
    ...
    assert plan['schema'] == 'full_qasper200_FP16_six_method_remote_quality_v1'
    ...
    assert coverage['N']==200 and coverage['D']==148 and coverage['max_prompt_plus_cap']==22174
    ...
    assert len(groups) == plan['documents_per_arm'] == 148
    assert plan['expected_complete_phases_per_arm'] == 4*148 + 2*200 + 1 == 993
    for row in items:
        assert not ({'answer','answers','gold','reference','references','outputs','all_classes','evidence'} & set(row))
        assert row['dataset'] == 'qasper' and row['max_new_tokens'] == 128
        ...
        assert len(full)==expected['full_prompt_tokens_with_BOS'] and len(full)+128==expected['prompt_plus_cap']<=22174
