...
def main():
 ...
 try:
  ...
  with torch.inference_mode(),profiler.phase('outer_model_load_native_quality_and_release'):
   ...
   provenance.update(fixture=plan['fixture'],physical_gpu_identity=identity,resource_policy=RESOURCE,scope='Full Qasper200/148 documents, six FP16 methods; original unscaled model with bound22174 input coverage, no YaRN; GPU feasibility unproven until completion')
   ...
   assert len(result['rows'])==plan['items_per_arm']==200 and len(result['documents'])==plan['documents_per_arm']==148
   assert len(profiler.records)==plan['expected_complete_phases_per_arm']==993 and all(x['completed'] for x in profiler.records[1:])
   del kernels
