...
def run(model, tokenizer, fixture, arm, *, plan, output, provenance, profiler, natural_read, kernels=None):
    ...
    result = {'schema':'full_qasper200_FP16_six_method_remote_quality_v1','status':'running','arm':arm,
        ...
        'scope':'Full Qasper200, D148 reused entries; unscaled model and unchanged full inputs, no dropped long rows; natural quality timing only'}
    ...
    try:
        ...
        assert len(result['rows'])==200 and len(result['documents'])==148
        assert sum(d['entry_writes'] for d in result['documents'])==148
