"""Full LoCoMo1986 grouped entries and variable-length QA; imports no model at module load."""
...
def run(model, tokenizer, fixture, arm, *, plan, output, provenance, profiler, natural_read, kernels=None):
    ...
    result = {'schema':'full_LoCoMo1986_FP16_six_method_prediction_and_native_lexical_v1','status':'running','arm':arm,
    ...
    try:
            ...
            with profiler.phase('document', document_id=document_id):
                try:
                    ...
                    for index in indices:
                        ...
                        row['unscaled_input_regime']={'full_prompt_tokens':1+len(item['document_token_ids'])+len(item['query_token_ids']),'prompt_plus_cap':1+len(item['document_token_ids'])+len(item['query_token_ids'])+48,'above_published32768':1+len(item['document_token_ids'])+len(item['query_token_ids'])>32768,'above_original40960_with_cap':1+len(item['document_token_ids'])+len(item['query_token_ids'])+48>40960}
        ...
        assert len(result['rows'])==1986 and len(result['documents'])==10
        assert sum(d['entry_writes'] for d in result['documents'])==10
