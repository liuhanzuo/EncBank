"""Six-method native resource envelope with frozen FP16 full LoCoMo driver and timing."""
import datetime,gc,os,socket,sys,traceback
from pathlib import Path
from protocol import ROOT,HERE,RESOURCE,local,model_path,parser,preflight,read,save,sha,resolve_driver,load_fixture
...
def main():
 ...
 try:
  ...
  record['preparation_stage']='model_and_adapter_hashes_started';record.setdefault('preparation_stage_markers',[]).append({'stage':record['preparation_stage'],'at':now(),'scope':'infrastructure_only_not_quality_timing_or_allocator_phase'});save(output/'worker.json',record)
  ...
  record['preparation_stage']='model_and_adapter_hashes_completed';record['preparation_stage_markers'].append({'stage':record['preparation_stage'],'at':now(),'scope':'infrastructure_only_not_quality_timing_or_allocator_phase'});save(output/'worker.json',record)
  ...
  record['preparation_stage']='framework_imports_started';record['preparation_stage_markers'].append({'stage':record['preparation_stage'],'at':now(),'scope':'infrastructure_only_not_quality_timing_or_allocator_phase'});save(output/'worker.json',record)
  ...
  record['preparation_stage']='framework_imports_completed';record['preparation_stage_markers'].append({'stage':record['preparation_stage'],'at':now(),'scope':'infrastructure_only_not_quality_timing_or_allocator_phase'});save(output/'worker.json',record)
  ...
  with torch.inference_mode(),profiler.phase('outer_model_load_native_quality_and_release'):
   ...
   original_config=read(model_path(plan,'model')/'config.json')
   ...
   save(output/'worker.json',record)
   fixture=load_fixture(plan);assert model.config.bos_token_id==151643 and tokenizer.eos_token_id==151645
   assert model.config.max_position_embeddings==40960
   record['effective_model_config']=model.config.to_dict()
   record['unscaled_input_coverage']={'maximum_full_prompt_plus_cap':22299,'fixture':plan['fixture']}
   save(output/'worker.json',record)
   provenance={'plan_sha256':args.expected_plan_sha256,'fixture_sha256':plan['fixture']['sha256'],'activation_sha256':plan['activation']['sha256'],'source_sha256':plan['source_sha256'],'model_file_sha256':activation['model_file_sha256'],'adapter_file_sha256':activation['files'],'torch':torch.__version__,'transformers':transformers.__version__,'peft':peft.__version__,'gpu':torch.cuda.get_device_name(0),'physical_gpu_identity':identity,'slurm_job_id':os.environ['SLURM_JOB_ID'],'attention_backend':'sdpa','backbone_dtype':'torch.float16','model_load':{k:v for k,v in record.items() if k.startswith('model_')},'resource_policy':RESOURCE,'reader_binding':plan['reader_binding'],'adapter_active':args.arm.startswith('h'),'same_custom_reader_for_H16_H8_H4':True,'exact_paper_checkpoint_replication':False,'first_step_EOS_suppressed':True,'single_EOS_override':151645,'explicit_BOS_from_model_config':151643,'original_tokenizer_BOS':tokenizer.bos_token_id,'original_generation_config_EOS':read(model_path(plan,'model')/'generation_config.json')['eos_token_id'],'lexical_metadata_CPU_is_not_H_or_KV_offload':True,'device_block':'gpu-node1 Slurm single physical GPU; never RTX5090 timing evidence'}
   provenance.update(fixture=plan['fixture'],physical_gpu_identity=identity,resource_policy=RESOURCE,scope='Six full LoCoMo1986 FP16 arms in one allocation; raw predictions and native lexical scope; semantic Judge pending; natural quality timing, no fixed-work timing/capacity claim')
   ...
   else:assert not dtypes
   provenance.update(backbone_dtype='torch.float16',attention_backend='sdpa',LoRA_parameter_dtypes=sorted(set(dtypes.values())),configuration={'resume_j':12,'H_group_size':64,'KIVI_group_size':32,'KIVI_residual_length':128,'query_tokens_per_call':1,'max_new_tokens':48,'BOS_token_id':151643,'EOS_token_id':151645,'first_step_EOS_suppressed':True})
   ...
   provenance['unscaled_coverage']={'maximum_full_prompt_plus_cap':22299,'CPU_input_receipt':plan['input_check_receipt'],'effective_model_config':record['effective_model_config'],'original_max_position_embeddings':40960,'original_rope_theta':1000000,'YaRN_or_model_config_override_applied':False}
   ...
   assert len(result['rows'])==plan['items_per_arm']==1986 and len(result['documents'])==plan['documents_per_arm']==10
   assert len(profiler.records)==plan['expected_complete_phases_per_arm']==4013 and all(x['completed'] for x in profiler.records[1:])
   del kernels
