"""Qualified single-GPU resource envelope; six FP16 VT methods and natural timing."""
...
def main():
 ...
 try:
  ...
  with torch.inference_mode(),profiler.phase('outer_model_load_native_quality_and_release'):
   ...
   provenance.update(fixture=plan['fixture'],physical_gpu_identity=identity,resource_policy=RESOURCE,scope='Full NarrativeQA200/20 documents, six FP16 methods; original unscaled model with bound65529 input coverage, no YaRN; GPU feasibility unproven until completion')
   ...
   provenance.update(backbone_dtype='torch.float16',attention_backend='sdpa',LoRA_parameter_dtypes=sorted(set(dtypes.values())),configuration={'resume_j':12,'H_group_size':64,'KIVI_group_size':32,'KIVI_residual_length':128,'query_tokens_per_call':1,'max_new_tokens':128,'BOS_token_id':151643,'EOS_token_id':151645,'first_step_EOS_suppressed':True})
   ...
   assert len(result['rows'])==plan['items_per_arm']==200 and len(result['documents'])==plan['documents_per_arm']==20
   assert len(profiler.records)==plan['expected_complete_phases_per_arm']==481 and all(x['completed'] for x in profiler.records[1:])
   del kernels
