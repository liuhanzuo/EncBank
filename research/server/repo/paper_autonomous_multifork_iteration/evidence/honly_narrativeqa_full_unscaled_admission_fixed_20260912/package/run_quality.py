"""RULER VT 8K100 grouped entries and variable-length QA; imports no model at module load."""
...
def run(model, tokenizer, fixture, arm, *, plan, output, provenance, profiler, natural_read, kernels=None):
    ...
    result = {'schema':'full_NarrativeQA200_FP16_six_method_unscaled_remote_quality_v1','status':'running','arm':arm,
        ...
        'scope':'Full NarrativeQA200, D20 reused entries; unscaled model and unchanged full inputs, no dropped long rows; natural quality timing only'}
    ...
    try:
            ...
            with profiler.phase('document', document_id=document_id):
                try:
                    ...
                    for index in indices:
                        ...
                        row['unscaled_input_regime']={'full_prompt_tokens':1+len(item['document_token_ids'])+len(item['query_token_ids']),'prompt_plus_cap':1+len(item['document_token_ids'])+len(item['query_token_ids'])+128,'above_published32768':1+len(item['document_token_ids'])+len(item['query_token_ids'])>32768,'above_original40960_with_cap':1+len(item['document_token_ids'])+len(item['query_token_ids'])+128>40960}
                        ...
                        print(f'{arm}: {execution_index}/200 id={item["id"]} tokens={row["generation_length"]}',flush=True)
                finally:
        ...
        assert len(result['rows'])==200 and len(result['documents'])==20
        assert sum(d['entry_writes'] for d in result['documents'])==20
