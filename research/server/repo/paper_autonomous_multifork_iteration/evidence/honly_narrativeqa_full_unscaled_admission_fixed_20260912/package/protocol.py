...
CONFIG = {'resume_j': 12, 'chunk_size': 512, 'selector': 'iter_bm25', 'topk': 12,
          ...
          'first_step_EOS_suppressed': True, 'max_new_tokens': 128, 'query_tokens_per_call': 1}
...
def preflight(args,envelope=False):
    ...
    assert plan['schema'] == 'full_NarrativeQA200_FP16_six_method_unscaled_remote_quality_v1'
    ...
    assert coverage['N']==200 and coverage['D']==20 and coverage['max_prompt_plus_cap']==65529
    ...
    assert len(groups) == plan['documents_per_arm'] == 20
    assert plan['expected_complete_phases_per_arm'] == 4*20 + 2*200 + 1 == 481
    for row in items:
        assert not ({'answer','answers','gold','reference','references','outputs','all_classes','evidence'} & set(row))
        assert row['dataset'] == 'narrativeqa' and row['max_new_tokens'] == 128
        ...
        assert len(full)==expected['full_prompt_tokens_with_BOS'] and len(full)+128==expected['prompt_plus_cap']<=65529
