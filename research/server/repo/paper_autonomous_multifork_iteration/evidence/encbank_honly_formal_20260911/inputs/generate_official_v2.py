...
O=Path(__file__).resolve().parent;ROOT=Path(r'/srv/encbank/legacy_workspace')
assert os.environ.get('PYTHONHASHSEED')=='42'
assert os.environ.get('USE_TORCH')=='0'
DATA=O/'official_ruler/scripts/data';SRC=DATA/'synthetic/niah.py'
sys.path[:0]=[str(O/'dependencies/site'),str(DATA),str(DATA/'synthetic')]
import yaml
from transformers import AutoTokenizer
config=yaml.safe_load((O/'official_ruler/scripts/synthetic.yaml').read_text())['niah_single_2']
...
assert config['args']=={'type_haystack':'essay','type_needle_k':'words','type_needle_v':'numbers','num_needle_k':1,'num_needle_v':1,'num_needle_q':1}
...
save_root=O/'scoring_only/official_raw_attempt2'
assert not save_root.exists()
args=[str(SRC),'--save_dir',str(save_root),'--save_name','niah_single_2','--subset','validation',
 '--tokenizer_path',str(ROOT/'models/Qwen3-8B'),'--tokenizer_type','hf','--max_seq_length','8192',
...
def profile(frame,event,arg):
 ...
 return profile
plan={'status':'FROZEN_BEFORE_NEW_DATA_GENERATION','task':'niah_single_2','official_git_revision':'c3f5e3b4f87f97e048793bb510a3a6b19a46bf3a',
 ...
 'argv':args,'random_seed':42,'PYTHONHASHSEED':42,'official_max_seq_length':8192,'official_generation_reserve':128,'inference_generation_cap':48,
...
(O/'generation_plan_v2.json').write_text(json.dumps(plan,indent=2)+'\n',encoding='utf-8')
sys.argv=args
sys.setprofile(profile)
try:runpy.run_path(str(SRC),run_name='__main__')
finally:sys.setprofile(None)
raw=save_root/'niah_single_2/validation.jsonl'
...
(O/'source_boundaries_v2.jsonl').write_text(''.join(json.dumps(m,ensure_ascii=False)+'\n' for m in selected),encoding='utf-8')
