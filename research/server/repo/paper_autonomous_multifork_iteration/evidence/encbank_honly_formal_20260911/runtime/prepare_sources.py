...
ROOT=HERE.parents[3]
UPSTREAM=ROOT/'tmp_external_baselines/encbank_official/encbank'
CODEC=ROOT/'gpu/qencbank_torch.py'
...
vendor=HERE/'vendor_encbank'; vendor.mkdir(parents=True,exist_ok=True)
for name in ['model.py','selectors.py']:
    p=vendor/name
    assert not p.exists()
    p.write_bytes((UPSTREAM/name).read_bytes())
(vendor/'__init__.py').write_text('"""Immutable official Encbank primitives; no modified model mathematics."""\n',encoding='utf-8')
