...
def main():
    ...
    for arm in ARMS:
        ...
        for index,(row,item) in enumerate(zip(rows,items)):
            assert row['source_index']==index and row['document_id']==item['document_id'] and row['dataset']=='babilong_qa5_32k'
    ...
    save(args.output,{'status':'complete_BABILong_qa5_32k_100_600_answers_600_Writes_3606_phases_pending_parent_shell_Slurm_and_independent_verification',
        ...
        'resampling':plan['analysis'],'scope':'One public qa5/32k BABILong100 matched FP16 cell; not21-cell macro, agent success rate or fixed-work timing/capacity ranking',
