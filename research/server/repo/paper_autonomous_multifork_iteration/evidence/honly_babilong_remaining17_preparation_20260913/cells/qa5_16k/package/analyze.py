"""Saved-output CPU pinned BABILong official label accuracy scoring; require all six native actual successful exits first."""
...
def main():
    ...
    label_map={x['id']:x for x in labels['items']};assert set(label_map)==set(ids) and len(ids)==100 and len(groups)==plan['documents_per_arm']
    compare_answers,task_labels=metric_functions(plan);reports={};scores={}
    for arm in ARMS:
        ...
        assert rows and [x['id'] for x in rows]==ids and len(docs)==plan['documents_per_arm'] and len(phases)==plan['expected_complete_phases_per_arm']
        ...
        for index,(row,item) in enumerate(zip(rows,items)):
            assert row['source_index']==index and row['document_id']==item['document_id'] and row['dataset']=='babilong_qa5_16k'
            ...
            tokens=row['generated_token_ids'];assert 1<=len(tokens)<=20 and 151645 not in tokens
            assert row['generation_length']==len(tokens) and row['max_new_tokens']==20
            assert row['hit_generation_cap']==(len(tokens)==20) and row['stopped_on_eos']==(len(tokens)<20)
        ...
        predictions=[r['prediction'] for r in rows];refs=[label_map[x]['target'] for x in ids]
        task=plan['benchmark_cell']['task'];assert task in task_labels
        assert all(label_map[x]['task']==task for x in ids)
        scores[arm]=[int(compare_answers(label_map[item_id]['target'],prediction,label_map[item_id]['question'],task_labels[task])) for item_id,prediction in zip(ids,predictions)]
        official=100*statistics.fmean(scores[arm])
        reports[arm]={'n':100,'documents':100,'official_accuracy_fraction':statistics.fmean(scores[arm]),'official_accuracy_percent':official,'correct':sum(scores[arm]),
            'per_item_official_correct':dict(zip(ids,scores[arm])),'zero_scores':sum(s==0 for s in scores[arm]),'raw_empty_predictions':sum(not s for s in predictions),
    ...
    rng=random.Random(20260912);draws=[[rng.randrange(len(groups)) for _ in range(len(groups))] for _ in range(10000)]
    contrasts={}
    ...
    save(args.output,{'status':'complete_BABILong_qa5_16k_100_600_answers_600_Writes_3606_phases_pending_parent_shell_Slurm_and_independent_verification',
        ...
        'resampling':plan['analysis'],'scope':'One public qa5/16k BABILong100 matched FP16 cell; not21-cell macro, agent success rate or fixed-work timing/capacity ranking',
