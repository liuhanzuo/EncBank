"""One allocated GPU; one real child process running unchanged tiny kernel checks."""
...
def main():
 ...
 try:
  ...
  for arm in ARMS:
   out=local(plan['outputs'][arm]);args=argparse.Namespace(plan=a.plan,expected_plan_sha256=a.expected_plan_sha256,arm=arm,output=str(out),check_only=False)
  ...
  batch.update(status='tiny_kernel_checks_complete_shell_and_slurm_exit_pending' if success else 'stopped_without_retry_not_complete_remote_kernel_qualification',finished_at=now())
