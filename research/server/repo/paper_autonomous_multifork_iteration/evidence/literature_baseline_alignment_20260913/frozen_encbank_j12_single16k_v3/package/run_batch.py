"""One allocated GPU; one real child then pinned official-scoring CPU analysis."""
...
def main():
 ...
 try:
  ...
  batch.update(status='all_one_arm_exited_complete_pending_analysis');save(root/'batch_status.json',batch)
  ...
  batch.update(status='one_arm_and_analysis_complete_shell_and_slurm_exit_pending' if success else 'stopped_without_retry_not_complete_one_arm_evidence',finished_at=now())
