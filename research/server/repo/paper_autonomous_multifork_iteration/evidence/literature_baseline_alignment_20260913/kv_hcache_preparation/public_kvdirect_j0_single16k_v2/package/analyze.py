...
def main():
    ...
    for arm in ARMS:
        ...
        for index,(row,item) in enumerate(zip(rows,items)):
            assert row['source_index']==index and row['document_id']==item['document_id'] and row['dataset']=='ruler_niah_single_2_16k'
    ...
    contrasts={}
    save(args.output,{'status':'complete_public_kvdirect_j0_RULER_single2_16K100_answers_100_Writes_601_phases_pending_parent_shell_Slurm_and_independent_verification',
        ...
        'resampling':plan['analysis'],'scope':'One new KV-Direct on the exact existing Single-needle16K100 input; same FP16 SDPA, no adapter; original official scoring, no full15 macro or fixed infra claim',
        'limits':['KV-Direct uses the same base weights, no adapter; it is not a repeat of custom-LoRA H16 j12 or a published checkpoint claim','Same native SDPA primitive policy is not identical CUDA dispatch or attention graph','Intervals including0 do not establish equivalence/noninferiority']})
