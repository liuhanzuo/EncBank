...
def main():
 ...
 try:
  ...
  with torch.inference_mode(),profiler.phase('outer_model_load_native_quality_and_release'):
   ...
   provenance.update(fixture=plan['fixture'],physical_gpu_identity=identity,resource_policy=RESOURCE,scope='One adapter-free KV-Direct Single-needle16K100; exact existing input; pinned official scorer and natural quality diagnostics')
   ...
   provenance.update(backbone_dtype='torch.float16',attention_backend='sdpa',LoRA_parameter_dtypes=sorted(set(dtypes.values())),configuration={'resume_j':0,'H_group_size':64,'KIVI_group_size':32,'KIVI_residual_length':128,'query_tokens_per_call':1,'max_new_tokens':48,'BOS_token_id':151643,'EOS_token_id':151645,'first_step_EOS_suppressed':True})
