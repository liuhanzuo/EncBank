...
def run(model, tokenizer, fixture, arm, *, plan, output, provenance, profiler, natural_read, kernels=None):
    ...
    result = {'schema':'RULER_single2_16K100_FP16_public_hcache_j12_v1','status':'running','arm':arm,
        ...
        'adapter_active':False,'resume_j':12,'scope':'One new HCache-style FP16/SDPA Single-needle16K100 cell; no KIVI or other model arm executed; natural diagnostics only'}
    ...
    try:
        ...
        assert len(result['rows'])==100 and len(result['documents'])==100
        assert sum(d['entry_writes'] for d in result['documents'])==100
