...
def model_path(plan,part):
    ...
    return p
CONFIG = {'resume_j': 12, 'chunk_size': 512, 'selector': 'all_chunks', 'topk': None, 'iter_hop_topk': None, 'iter_rounds': None, 'H_group_size': 64, 'KIVI_group_size': 32, 'KIVI_residual_length': 128, 'bos_token_id': 151643, 'eos_token_id': 151645, 'first_step_EOS_suppressed': True, 'max_new_tokens': 48, 'query_tokens_per_call': 1}
...
def preflight(args,envelope=False):
    ...
    assert plan['schema'] == 'RULER_single2_16K100_FP16_public_hcache_j12_v1'
    assert plan['arm_order'] == list(ARMS) and plan['configuration'] == CONFIG
    assert plan['method_configuration']=={'method': 'HCache-style', 'runtime_arm': 'public_hcache_j12', 'resume_j': 12, 'adapter_active': False, 'stored_bits': 16, 'retrieval': 'none_all_chunks_source_order'}
    ...
    for row in items:
        ...
        assert row['dataset'] == 'ruler_niah_single_2_16k' and row['max_new_tokens'] == 48
        ...
        assert token_sha(full) == row['full_prompt_with_BOS_token_sha256'] and len(full)+48 <= 40960
