...
def main():
    ...
    for arm in ARMS:
        ...
        assert prov['backbone_dtype']=='torch.float16' and prov['attention_backend']=='infllm_upstream_torch_joint_softmax'
        assert prov['adapter_active'] is False and prov['LoRA_parameter_dtypes']==[] and prov['method_configuration']==plan['method_configuration']
        ...
        for doc,(_,indices) in zip(docs,groups):
            ...
            assert doc['store']['all_tensors_cuda'] and not doc['store']['runtime_offload'] and doc['store']['unique_storage_bytes']>0
        for index,(row,item) in enumerate(zip(rows,items)):
            ...
            assert selected is None and row['retrieval_configuration']==plan['method_configuration']
        for row in rows:
    ...
    contrasts={}
    save(args.output,{'status':'complete_InfLLM_resident_Qwen3_RULER_single2_16K100_answers_100_Writes_601_phases_pending_parent_shell_Slurm_and_independent_verification',
        ...
        'resampling':plan['analysis'],'scope':'One InfLLM resident Qwen3 adaptation on exact existing Single-needle16K100 input, FP16 LoRA OFF, original official scoring, no full15 macro or fixed infra claim',
        'limits':['Pinned InfLLM memory/attention algorithm with explicit resident-GPU and Qwen3 adaptations; not the old published checkpoint/protocol','InfLLM executes upstream Torch joint-softmax attention; it does not share the native SDPA primitive','Intervals including0 do not establish equivalence/noninferiority']})
