...
def model_path(plan,part):
    ...
    return p
CONFIG = {'bos_token_id': 151643, 'eos_token_id': 151645, 'first_step_EOS_suppressed': True, 'max_new_tokens': 48, 'query_tokens_per_call': 1, 'document_prefill_chunk_size': 2048, 'n_init': 128, 'n_local': 4096, 'block_size': 128, 'topk': 16, 'repr_topk': 4, 'max_cached_block': 32, 'exc_block_size': 512, 'fattn': False, 'cache_strategy': 'lru', 'score_decay': None, 'chunk_topk_calc': None, 'async_global_stream': False, 'pin_memory': False, 'faiss': False, 'perhead': False, 'rope_base': 1000000, 'distance_scale': 1.0}
...
def preflight(args,envelope=False):
    ...
    assert plan['schema'] == 'RULER_single2_16K100_FP16_InfLLM_resident_Qwen3_v1'
    assert plan['arm_order'] == list(ARMS) and plan['configuration'] == CONFIG
    assert plan['method_configuration']=={'method': 'InfLLM', 'upstream_commit': '12b70798f56e56ebb23c53c7018091a3f540a028', 'adapter_active': False, 'KV_dtype': 'float16', 'KV_backing': 'persistent_GPU', 'attention': 'pinned_upstream_Torch_multistage_joint_softmax', 'Qwen3_qk_normalization': 'preserved_before_InfLLM_RoPE', 'prompt_truncation': False, 'same_SDPA_primitive': False}
