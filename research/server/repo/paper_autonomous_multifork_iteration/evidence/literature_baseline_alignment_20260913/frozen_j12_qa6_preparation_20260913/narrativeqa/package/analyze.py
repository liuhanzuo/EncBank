...
def main():
    ...
    label_map={x['id']:x for x in labels['items']};assert set(label_map)==set(ids) and len(ids)==200 and len(groups)==20
    ...
    for arm in ARMS:
        ...
        assert rows and [x['id'] for x in rows]==ids and len(docs)==20 and len(phases)==481
        assert phases==worker['phases'] and all(x['completed'] for x in phases)
        assert names=={'outer_model_load_native_quality_and_release':1,'document':20,'Write':20,'Store_verification_before':20,'Read_complete_query_and_natural_decode':200,'Store_verification_after':200,'entry_release':20}
        ...
        for index,(row,item) in enumerate(zip(rows,items)):
            assert row['source_index']==index and row['document_id']==item['document_id'] and row['dataset']=='narrativeqa'
            ...
            tokens=row['generated_token_ids'];assert 1<=len(tokens)<=128 and 151645 not in tokens
            assert row['generation_length']==len(tokens) and row['max_new_tokens']==128
            assert row['hit_generation_cap']==(len(tokens)==128) and row['stopped_on_eos']==(len(tokens)<128)
        ...
        official=official_scorer('narrativeqa',predictions,refs,[]);assert official==round(100*statistics.fmean(scores[arm]),2)
        reports[arm]={'n':200,'documents':20,'official_QA_F1_percent':official,'unrounded_mean_F1_percent':100*statistics.fmean(scores[arm]),
            ...
            'generation_lengths':[len(r['generated_token_ids']) for r in rows],'per_item_request_timing':{r['id']:r['timing'] for r in rows},'per_item_position_usage':{r['id']:r['position_usage'] for r in rows},'per_item_unscaled_regime':{r['id']:r['unscaled_input_regime'] for r in rows},'Writes':20,'phase_count':481,
    ...
    contrasts={}
    save(args.output,{'status':'complete_frozen_Encbank_j12_narrativeqa_200_answers_20_Writes_481_phases_pending_parent_shell_Slurm_and_independent_verification',
        ...
        'resampling':plan['analysis'],'scope':'New frozen Encbank j12 LoRA OFF narrativeqa: exact 200 questions/20 documents, cap128, original LongBench QA F1, FP16 native SDPA. Natural timing only; no published checkpoint or fixed-work infrastructure claim.',
