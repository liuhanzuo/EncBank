"""New FP16 MFQA six-method quality bindings; native check-only is model-free."""
...
CONFIG = {'resume_j': 12, 'chunk_size': 512, 'selector': 'iter_bm25', 'topk': 12,
          ...
          'first_step_EOS_suppressed': True, 'max_new_tokens': 64, 'query_tokens_per_call': 1}
...
def local(relative):
    ...
    return path
def wsl(path): return '/srv/encbank/legacy_workspace/' + Path(path).resolve().relative_to(ROOT).as_posix()
...
def resolve_driver(plan):
    return (load_function(HERE/'run_quality.py', '_mfqa_fp16_six_run_quality', 'run'),
            load_function(local(plan['natural_qa_adapter']['path']), '_mfqa_fp16_natural_qa', 'read_natural'))
def parser():
...
def preflight(args,envelope=False):
    ...
    assert plan['schema'] == 'full_multifieldqa_en150_FP16_frozen_Encbank_j12_remote_quality_v1'
    ...
    backend = plan['kivi_backend']
    for name, digest in backend['source_sha256'].items(): assert sha(local(backend['root'])/name) == digest
    fixture = read(local(plan['fixture']['path'])); items = fixture['items']; groups = document_groups(fixture)
    assert len(items) == len({r['id'] for r in items}) == plan['items_per_arm'] == 150
    assert len(groups) == plan['documents_per_arm'] == 112
    assert plan['expected_complete_phases_per_arm'] == 4*112 + 2*150 + 1 == 749
    for row in items:
        assert not ({'answer','answers','gold','reference','references','outputs','all_classes','evidence'} & set(row))
        assert row['dataset'] == 'multifieldqa_en' and row['max_new_tokens'] == 64
        ...
        assert token_sha(full) == row['full_prompt_with_BOS_token_sha256'] and len(full)+64 <= 40960
