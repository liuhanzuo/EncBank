...
def main():
    ...
    label_map={x['id']:x for x in labels['items']};assert set(label_map)==set(ids) and len(ids)==150 and len(groups)==112
    ...
    for arm in ARMS:
        ...
        assert rows and [x['id'] for x in rows]==ids and len(docs)==112 and len(phases)==749
        assert phases==worker['phases'] and all(x['completed'] for x in phases)
        assert names=={'outer_model_load_native_quality_and_release':1,'document':112,'Write':112,'Store_verification_before':112,'Read_complete_query_and_natural_decode':150,'Store_verification_after':150,'entry_release':112}
        ...
        for index,(row,item) in enumerate(zip(rows,items)):
            assert row['source_index']==index and row['document_id']==item['document_id'] and row['dataset']=='multifieldqa_en'
        ...
        official=official_scorer('multifieldqa_en',predictions,refs,[]);assert official==round(100*statistics.fmean(scores[arm]),2)
        reports[arm]={'n':150,'documents':112,'official_QA_F1_percent':official,'unrounded_mean_F1_percent':100*statistics.fmean(scores[arm]),
            ...
            'generation_lengths':[len(r['generated_token_ids']) for r in rows],'per_item_request_timing':{r['id']:r['timing'] for r in rows},'per_item_position_usage':{r['id']:r['position_usage'] for r in rows},'per_item_unscaled_regime':{r['id']:r['unscaled_input_regime'] for r in rows},'Writes':112,'phase_count':749,
    ...
    contrasts={}
    save(args.output,{'status':'complete_frozen_Encbank_j12_multifieldqa_en_150_answers_112_Writes_749_phases_pending_parent_shell_Slurm_and_independent_verification',
        ...
        'resampling':plan['analysis'],'scope':'New frozen Encbank j12 LoRA OFF multifieldqa_en: exact 150 questions/112 documents, cap64, original LongBench QA F1, FP16 native SDPA. Natural timing only; no published checkpoint or fixed-work infrastructure claim.',
        'limits':['Frozen Encbank j12 uses no LoRA; new algorithm measurement, not a published checkpoint claim','Same native SDPA primitive policy is not identical CUDA dispatch or attention graph','Intervals including0 do not establish equivalence/noninferiority']})
