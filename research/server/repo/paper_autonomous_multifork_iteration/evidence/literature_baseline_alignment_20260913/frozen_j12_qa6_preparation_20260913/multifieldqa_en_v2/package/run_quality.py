...
def run(model, tokenizer, fixture, arm, *, plan, output, provenance, profiler, natural_read, kernels=None):
    ...
    result = {'schema':'full_multifieldqa_en150_FP16_frozen_Encbank_j12_remote_quality_v1','status':'running','arm':arm,
        ...
        'scope':'New frozen Encbank j12 LoRA OFF multifieldqa_en: exact 150 questions/112 documents, cap64, original LongBench QA F1, FP16 native SDPA. Natural timing only; no published checkpoint or fixed-work infrastructure claim.'}
    ...
    try:
            ...
            with profiler.phase('document', document_id=document_id):
                try:
                    ...
                    for index in indices:
                        ...
                        print(f'{arm}: {execution_index}/150 id={item["id"]} tokens={row["generation_length"]}',flush=True)
                finally:
        ...
        assert len(result['rows'])==150 and len(result['documents'])==112
        assert sum(d['entry_writes'] for d in result['documents'])==112
