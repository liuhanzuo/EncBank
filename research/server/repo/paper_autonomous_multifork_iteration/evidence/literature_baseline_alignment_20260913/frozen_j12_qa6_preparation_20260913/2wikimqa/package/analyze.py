...
def main():
    ...
    for arm in ARMS:
        ...
        for index,(row,item) in enumerate(zip(rows,items)):
            assert row['source_index']==index and row['document_id']==item['document_id'] and row['dataset']=='2wikimqa'
        ...
        official=official_scorer('2wikimqa',predictions,refs,[]);assert official==round(100*statistics.fmean(scores[arm]),2)
    ...
    contrasts={}
    save(args.output,{'status':'complete_frozen_Encbank_j12_2wikimqa_200_answers_200_Writes_1201_phases_pending_parent_shell_Slurm_and_independent_verification',
        ...
        'resampling':plan['analysis'],'scope':'New frozen Encbank j12 LoRA OFF 2wikimqa: exact 200 questions/200 documents, cap32, original LongBench QA F1, FP16 native SDPA. Natural timing only; no published checkpoint or fixed-work infrastructure claim.',
