"""Saved-output official Encbank VT recall; require one successful native exits first."""
...
def main():
    ...
    for arm in ARMS:
        ...
        for index,(row,item) in enumerate(zip(rows,items)):
            assert row['source_index']==index and row['document_id']==item['document_id'] and row['dataset']=='ruler_variable_tracking_128k'
            ...
            tokens=row['generated_token_ids'];assert 1<=len(tokens)<=60 and 151645 not in tokens
            assert row['generation_length']==len(tokens) and row['max_new_tokens']==60
            assert row['hit_generation_cap']==(len(tokens)==60) and row['stopped_on_eos']==(len(tokens)<60)
        ...
        assert all(isinstance(x,list) and len(x)==5 and all(isinstance(v,str) for v in x) for x in refs)
    ...
    contrasts={}
    save(args.output,{'status':'complete_frozen_Encbank_j12_RULER_vt128k_100_answers_100_Writes_601_phases_pending_parent_shell_Slurm_and_independent_verification',
        ...
        'resampling':plan['analysis'],'scope':'New independent frozen Encbank j12 LoRA OFF vt128k100; exact original inputs and cap60; native FP16 SDPA natural quality diagnostics; not a published checkpoint or fixed-work infra claim.',
        'limits':['Frozen Encbank j12 uses no LoRA; this is a new adapter-free algorithm comparison, not a published checkpoint claim','Same native SDPA primitive policy is not identical CUDA dispatch or attention graph','Intervals including0 do not establish equivalence/noninferiority']})
