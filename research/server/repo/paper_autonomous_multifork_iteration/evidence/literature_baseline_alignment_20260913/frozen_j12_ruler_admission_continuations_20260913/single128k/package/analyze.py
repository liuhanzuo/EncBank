...
def main():
    ...
    for arm in ARMS:
        ...
        for index,(row,item) in enumerate(zip(rows,items)):
            assert row['source_index']==index and row['document_id']==item['document_id'] and row['dataset']=='ruler_niah_single_2_128k'
    ...
    contrasts={}
    save(args.output,{'status':'complete_frozen_Encbank_j12_RULER_single128k_100_answers_100_Writes_601_phases_pending_parent_shell_Slurm_and_independent_verification',
        ...
        'resampling':plan['analysis'],'scope':'New independent frozen Encbank j12 LoRA OFF single128k100; exact original inputs and cap48; native FP16 SDPA natural quality diagnostics; not a published checkpoint or fixed-work infra claim.',
