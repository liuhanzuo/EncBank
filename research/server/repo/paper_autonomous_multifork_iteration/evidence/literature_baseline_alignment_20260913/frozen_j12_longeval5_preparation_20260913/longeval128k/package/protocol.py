...
CONFIG = {'resume_j': 12, 'chunk_size': 512, 'selector': 'iter_bm25', 'topk': 12,
          ...
          'first_step_EOS_suppressed': True, 'max_new_tokens': 16, 'query_tokens_per_call': 1}
...
def preflight(args,envelope=False):
    ...
    assert plan['schema'] == 'LongEval_longeval128k_100_FP16_frozen_Encbank_j12_remote_v1'
    ...
    for key in ('fixture', 'activation', 'natural_qa_adapter', 'source_preparation_manifest', 'metadata_erratum', 'input_ready_registration', 'input_check_receipt'):
    ...
    assert plan['full_input_original_window']==40960 and plan['full_input_coverage_maximum']==131749
    ...
    assert max(17+len(row['document_token_ids'])+len(row['query_token_ids']) for row in items)==131749
    ...
    for row in items:
        ...
        assert row['dataset'] == 'longeval_128k' and row['max_new_tokens'] == 16
        ...
        assert token_sha(full) == row['full_prompt_with_BOS_token_sha256'] and len(full)+16 <= 131749
