...
def main():
    ...
    for arm in ARMS:
        ...
        for index,(row,item) in enumerate(zip(rows,items)):
            assert row['source_index']==index and row['document_id']==item['document_id'] and row['dataset']=='longeval_64k'
            ...
            regime=row['unscaled_input_regime'];assert regime['full_prompt_tokens']==1+len(item['document_token_ids'])+len(item['query_token_ids']) and regime['prompt_plus_cap']==regime['full_prompt_tokens']+16<=66145
        ...
        reports[arm]={'n':100,'documents':100,'numeric_exact_accuracy_percent':official,'correct':sum(scores[arm]),'extracted_predictions':dict(zip(ids,extracted)),
            ...
            'generation_lengths':[len(r['generated_token_ids']) for r in rows],'per_item_request_timing':{r['id']:r['timing'] for r in rows},'per_item_position_usage':{r['id']:r['position_usage'] for r in rows},'per_item_unscaled_regime':{r['id']:r['unscaled_input_regime'] for r in rows},'Writes':100,'phase_count':601,
    ...
    contrasts={}
    save(args.output,{'status':'complete_frozen_Encbank_j12_LongEval_longeval64k_100_answers_100_Writes_601_phases_pending_parent_shell_Slurm_and_independent_verification',
        ...
        'resampling':plan['analysis'],'scope':'New independent frozen Encbank j12 LoRA OFF longeval64k100; exact original inputs and cap16; native FP16 SDPA natural quality diagnostics; not a published checkpoint or fixed-work infra claim.',
