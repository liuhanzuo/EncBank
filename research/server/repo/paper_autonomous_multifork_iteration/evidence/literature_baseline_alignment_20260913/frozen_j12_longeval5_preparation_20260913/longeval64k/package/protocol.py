...
def preflight(args,envelope=False):
    ...
    assert plan['schema'] == 'LongEval_longeval64k_100_FP16_frozen_Encbank_j12_remote_v1'
    ...
    assert plan['full_input_original_window']==40960 and plan['full_input_coverage_maximum']==66145
    ...
    assert max(17+len(row['document_token_ids'])+len(row['query_token_ids']) for row in items)==66145
    ...
    for row in items:
        ...
        assert row['dataset'] == 'longeval_64k' and row['max_new_tokens'] == 16
        ...
        assert token_sha(full) == row['full_prompt_with_BOS_token_sha256'] and len(full)+16 <= 66145
