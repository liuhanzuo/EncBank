...
def main():
    ...
    for arm in ARMS:
        ...
        for doc,(_,indices) in zip(docs,groups):
            ...
            if arm=='comem_frozen_j12':assert doc['store']['document_lower_KV_bytes']==0 and doc['store']['resume_j']==12 and doc['store']['bits']==16 and not doc['store']['native_hidden_or_KV_host_offload']
    ...
    contrasts={}
    save(args.output,{'status':'complete_frozen_CoMem_j12_RULER_single2_8K100_answers_100_Writes_601_phases_pending_parent_shell_Slurm_and_independent_verification',
        ...
        'resampling':plan['analysis'],'scope':'One new frozen CoMem j12 on the exact existing Single-needle8K100 input; same FP16 SDPA, no adapter; original official scoring, no full15 macro or fixed infra claim',
        'limits':['Frozen CoMem j12 uses the same base weights, no adapter; it is not a repeat of custom-LoRA H16 j12 or a published checkpoint claim','Same native SDPA primitive policy is not identical CUDA dispatch or attention graph','Intervals including0 do not establish equivalence/noninferiority']})
