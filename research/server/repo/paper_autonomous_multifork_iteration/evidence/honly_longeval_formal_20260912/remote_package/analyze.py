"""Saved-output CPU pinned LongEval numeric exact scoring; require all four native actual successful exits first."""
...
def main():
    ...
    assert batch['plan_sha256']==args.expected_plan_sha256 and batch['status']=='all_four_arms_exited_complete_pending_analysis'
    closed={}
    ...
    save(args.output,{'status':'complete_LongEval400_answers_400_Writes_2404_phases_pending_parent_shell_Slurm_and_independent_verification',
        ...
        'resampling':plan['analysis'],'scope':'One newly generated LongEval 8K100 matched FP16 block; not five lengths, paper same examples, agent success rate or timing/capacity ranking',
        'limits':['H methods use active custom FP32 LoRA; Dense LoRA off; KIVI not run in this remote four-arm block','Same native SDPA primitive policy is not identical CUDA dispatch or attention graph','Intervals including0 do not establish equivalence/noninferiority']})
