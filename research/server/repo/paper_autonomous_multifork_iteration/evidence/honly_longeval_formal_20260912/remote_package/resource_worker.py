"""MuSiQue native resource envelope with frozen FP16 LongEval driver and timing."""
...
def main():
 ...
 try:
  ...
  with torch.inference_mode(),profiler.phase('outer_model_load_native_quality_and_release'):
   ...
   model,tokenizer=load_backbone(str(model_path(plan,'model')),dtype='float16',attn_impl='sdpa',device='cpu',lora_adapter=None if args.arm=='dense' else str(model_path(plan,'adapter')))
   ...
   provenance={'plan_sha256':args.expected_plan_sha256,'fixture_sha256':plan['fixture']['sha256'],'activation_sha256':plan['activation']['sha256'],'source_sha256':plan['source_sha256'],'model_file_sha256':activation['model_file_sha256'],'adapter_file_sha256':activation['files'],'torch':torch.__version__,'transformers':transformers.__version__,'peft':peft.__version__,'gpu':torch.cuda.get_device_name(0),'physical_gpu_identity':identity,'slurm_job_id':os.environ['SLURM_JOB_ID'],'attention_backend':'sdpa','backbone_dtype':'torch.float16','model_load':{k:v for k,v in record.items() if k.startswith('model_')},'resource_policy':RESOURCE,'reader_binding':plan['reader_binding'],'adapter_active':args.arm!='dense','same_custom_reader_for_H16_H8_H4':True,'exact_paper_checkpoint_replication':False,'first_step_EOS_suppressed':True,'single_EOS_override':151645,'explicit_BOS_from_model_config':151643,'original_tokenizer_BOS':tokenizer.bos_token_id,'original_generation_config_EOS':read(model_path(plan,'model')/'generation_config.json')['eos_token_id'],'lexical_metadata_CPU_is_not_H_or_KV_offload':True,'device_block':'gpu-node1 Slurm single physical GPU; never RTX5090 timing evidence'}
   provenance.update(fixture=plan['fixture'],physical_gpu_identity=identity,resource_policy=RESOURCE,scope='New remote LongEval8K100 four-arm FP16 block; KIVI not run; no BF16 reuse or fixed-work timing/capacity claim')
   ...
   provenance['executed_attention_policy']='Native HF SDPA attention, dispatcher not pinned'
   ...
   result=run(model,tokenizer,fixture,args.arm,plan=plan,output=output/'result.json',provenance=provenance,profiler=profiler,natural_read=natural_read,kernels=None)
