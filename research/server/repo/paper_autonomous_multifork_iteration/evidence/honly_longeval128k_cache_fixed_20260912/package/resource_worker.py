...
def main():
 ...
 try:
  ...
  with torch.inference_mode(),profiler.phase('outer_model_load_native_quality_and_release'):
   ...
   record['unscaled_input_coverage']={'maximum_full_prompt_plus_cap':131749,'fixture':plan['fixture']}
   save(output/'worker.json',record)
   ...
   provenance.update(fixture=plan['fixture'],physical_gpu_identity=identity,resource_policy=RESOURCE,scope='Six new LongEval128K100 FP16 arms in one allocation; no 8K result reuse; natural quality timing, no fixed-work timing/capacity claim')
   ...
   provenance.update(backbone_dtype='torch.float16',attention_backend='sdpa',LoRA_parameter_dtypes=sorted(set(dtypes.values())),configuration={'resume_j':12,'H_group_size':64,'KIVI_group_size':32,'KIVI_residual_length':128,'query_tokens_per_call':1,'max_new_tokens':16,'BOS_token_id':151643,'EOS_token_id':151645,'first_step_EOS_suppressed':True})
   ...
   provenance['unscaled_coverage']={'maximum_full_prompt_plus_cap':131749,'CPU_qualification':plan['high_position_CPU_qualification'],'effective_model_config':record['effective_model_config'],'original_max_position_embeddings':40960,'original_rope_theta':1000000,'YaRN_or_model_config_override_applied':False}
