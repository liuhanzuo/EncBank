"""LongEval 128K100 grouped entries and variable-length QA; imports no model at module load."""
...
def run(model, tokenizer, fixture, arm, *, plan, output, provenance, profiler, natural_read, kernels=None):
    ...
    result = {'schema':'LongEval128K100_FP16_six_method_native_quality_v1','status':'running','arm':arm,
        ...
        'scope':'New LongEval 128K100 FP16 controls and KIVI native retrieval quality; no BF16 row reuse, timing or capacity ranking'}
            ...
            with profiler.phase('document', document_id=document_id):
                try:
                    ...
                    for index in indices:
                        ...
                        row['unscaled_input_regime']={'full_prompt_tokens':1+len(item['document_token_ids'])+len(item['query_token_ids']),'prompt_plus_cap':17+len(item['document_token_ids'])+len(item['query_token_ids']),'above_published32768':True,'above_original40960_with_cap':True,'model_config_max_position_embeddings':40960,'rope_theta':1000000,'YaRN':False}
