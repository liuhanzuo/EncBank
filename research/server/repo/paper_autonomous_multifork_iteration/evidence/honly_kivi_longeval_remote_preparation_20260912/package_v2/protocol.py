"""Two-arm KIVI native Slurm LongEval bindings; check-only imports no model framework."""
...
def preflight(args,envelope=False):
    ...
    assert plan['schema'] == 'LongEval8K100_FP16_KIVI_two_method_remote_quality_v1'
    ...
    for row in items:
        ...
        assert row['dataset'] == 'longeval_8k' and row['max_new_tokens'] == 16
        ...
        assert token_sha(full) == row['full_prompt_with_BOS_token_sha256'] and len(full)+16 <= 40960
