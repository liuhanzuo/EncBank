"""One allocated GPU; two real child processes then the original CPU analysis."""
...
def main():
 ...
 try:
  ...
  batch.update(status='all_two_arms_exited_complete_pending_analysis');save(root/'batch_status.json',batch)
  ...
  batch.update(status='two_arms_and_analysis_complete_shell_and_slurm_exit_pending' if success else 'stopped_without_retry_not_complete_two_arm_evidence',finished_at=now())
