...
def main():
 ...
 try:
  ...
  with torch.inference_mode(),profiler.phase('outer_model_load_native_quality_and_release'):
   ...
   provenance={'plan_sha256':args.expected_plan_sha256,'fixture_sha256':plan['fixture']['sha256'],'activation_sha256':plan['activation']['sha256'],'source_sha256':plan['source_sha256'],'model_file_sha256':activation['model_file_sha256'],'adapter_file_sha256':activation['files'],'torch':torch.__version__,'transformers':transformers.__version__,'peft':peft.__version__,'gpu':torch.cuda.get_device_name(0),'physical_gpu_identity':identity,'slurm_job_id':os.environ['SLURM_JOB_ID'],'attention_backend':'sdpa','backbone_dtype':'torch.float16','model_load':{k:v for k,v in record.items() if k.startswith('model_')},'resource_policy':RESOURCE,'reader_binding':plan['reader_binding'],'adapter_active':False,'same_custom_reader_for_H16_H8_H4':True,'exact_paper_checkpoint_replication':False,'first_step_EOS_suppressed':True,'single_EOS_override':151645,'explicit_BOS_from_model_config':151643,'original_tokenizer_BOS':tokenizer.bos_token_id,'original_generation_config_EOS':read(model_path(plan,'model')/'generation_config.json')['eos_token_id'],'lexical_metadata_CPU_is_not_H_or_KV_offload':True,'device_block':'gpu-node1 Slurm single physical GPU; never RTX5090 timing evidence'}
   provenance.update(fixture=plan['fixture'],physical_gpu_identity=identity,resource_policy=RESOURCE,scope='Two new KIVI LongEval8K100 FP16 arms; prior four methods are reused without rerun; separate acquisitions, no fixed-work timing/capacity claim')
   ...
   provenance['executed_attention_policy']='KIVI SDPA full-document prefill and official Half cached GEMV'
