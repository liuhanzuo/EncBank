"""Saved-output CPU pinned LongEval numeric exact scoring; require both new native actual successful exits first."""
...
def main():
    ...
    assert batch['plan_sha256']==args.expected_plan_sha256 and batch['status']=='all_two_arms_exited_complete_pending_analysis'
    closed={}
    ...
    for arm in ARMS:
        ...
        assert prov['actual_kivi_backend']['binary']==plan['kivi_backend']['binary'] and prov['actual_kivi_backend']['source_sha256']==plan['kivi_backend']['source_sha256'] and prov['actual_kivi_backend']['performance_backend']
        ...
        for index,(row,item) in enumerate(zip(rows,items)):
            assert row['source_index']==index and row['document_id']==item['document_id'] and row['dataset']=='longeval_8k'
    ...
    # Read the already scored prior four arms; never invoke their original analyzer.
    from prior_scores import load_prior_scores
    prior=load_prior_scores(plan,ids)
    scores.update(prior['scores'])
    ...
    save(args.output,{'status':'complete_two_KIVI_LongEval200_answers_200_Writes_1202_phases_pending_parent_shell_Slurm_and_independent_verification',
        'plan_sha256':args.expected_plan_sha256,'finished_at':datetime.datetime.now().astimezone().isoformat(),'arms':reports,'paired_contrasts':contrasts,'prior_four_arms_read_only':prior['summary'],'prior_five_contrasts_unchanged':prior['original_contrasts'],
        'resampling':plan['analysis'],'scope':'Two new KIVI LongEval 8K100 arms paired with four previously completed FP16 arms; separate acquisitions; not five lengths, same paper examples, agent success rate or timing/capacity ranking',
        'limits':['Previous H methods use active custom FP32 LoRA; Dense and new KIVI methods LoRA off; whole-method comparison; separate acquisition/GPU blocks','Same native SDPA primitive policy is not identical CUDA dispatch or attention graph','Intervals including0 do not establish equivalence/noninferiority']})
