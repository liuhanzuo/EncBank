...
RELATIVE='paper_autonomous_multifork_iteration/evidence/honly_babilong_remaining17_preparation_20260913/cells/qa2_32k/package'
REMOTE='/srv/encbank/qencbank_align_codex_20260911/repo'
PYTHON='/srv/encbank/qencbank_runtime_20260911/python312/bin/python'
CONTROL='/srv/encbank/qencbank_align_codex_20260911/babilong_qa2_32k_remaining17_dispatch_attempt1'
...
def main():
 ...
 if a.mode=='validate':
  ...
  try:
   ...
   assert plan['launch_permitted'] is True and plan['arm_order']==['dense', 'h16', 'h8', 'h4', 'kivi2', 'kivi4']
   ...
   report.update(status='PASS_sparse_files_original_staging_and_six_CPU_checks',finished_at=now(),resolved_storage_root=str(home),no_model_framework_or_GPU_execution=True)
 ...
 try:
  assert read(control/'package_validation.json')['status']=='PASS_sparse_files_original_staging_and_six_CPU_checks'
  ...
  rec['model_lineage_files_verified']=verified;rec['adapter_loaded_by_KIVI']=False
  ...
  existing=[];project_gpu_count=0;registered_running_prefixes=[]
  for line in queue['stdout'].splitlines():
   ...
   if 'qencbank' in fields[2].lower() or '/qencbank_align_codex_20260911/' in text:
    tres=re.search(r'\bTRES=(\S+)',text) or re.search(r'\bReqTRES=(\S+)',text)
    total=re.search(r'(?:^|,)gres/gpu=(\d+)(?:,|$)',tres.group(1)) if tres else None
    if total:requested=int(total.group(1))
    else:
     per=re.search(r'\bTresPerNode=gres/gpu:(\d+)(?:\s|$)',text)
     nodes=re.search(r'\bNumNodes=(\d+)',text)
     assert per and nodes,'Cannot establish active project GPU allocation; no guessing'
     requested=int(per.group(1))*int(nodes.group(1))
    project_gpu_count+=requested
    command_match=re.search(r'\bCommand=(\S+/)batch\.sbatch(?:\s|$)',text)
    if fields[1]=='RUNNING' and command_match:registered_running_prefixes.append(command_match.group(1))
  assert project_gpu_count+1<=4,'At most four total submitted/running project GPUs including this one'
  rec['project_GPU_admission']={'existing_requested_GPUs':project_gpu_count,'new_requested_GPUs':1,'maximum':4,'counts_pending_and_running':True,'root_serializes_submission_calls':True}
  ...
  unknown=[line for line in workers if not any(prefix in line for prefix in registered_running_prefixes)]
  assert not unknown,'Unregistered task-owned CPU-loading/GPU worker; inspect before submission'
  assert not any(str(here)+'/' in line for line in workers),'Same-cell worker exists'
  ...
  shell=(here/'batch.sbatch').read_text();assert '#SBATCH --nodelist='+','.join(plan['backend_qualification']['qualified_nodes']) in shell
