...
def main():
    ...
    class Provider:
        ...
        def project(self,l):
            ...
            if self.weights is None or l not in self.weights:
                ...
                w,b=self.weights[l];raw=F.linear(unit(self.h[source(l)]),w,b)
    ...
    results.update(complete=True,arm=ARM,depths=DEPTHS,environment=env,selected_ridge=selected,
        ...
        scope='Quick affine ridge-fit pilot, 32 training documents, 4 validation, 8 held-out; no end-to-end logits distillation and no agent task accuracy claim',
