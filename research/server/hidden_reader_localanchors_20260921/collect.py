...
for job in jobs:
    ...
    b=dict(arm=job['arm'],job=job['job'],depths=s.get('depths'),test_mean=s.get('test_mean'),
        head_MiB=s['head_parameter_bytes']/2**20 if 'head_parameter_bytes' in s else None)
    if 'runtime' in s:
