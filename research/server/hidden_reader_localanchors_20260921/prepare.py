"""Immutable shallow-only sweep, reusing the prior document-disjoint protocol."""
...
for depths in configs.values():assert depths[0]==12 and depths[-1]==24 and sorted(set(depths))==depths
...
records=[]
with (R.parent/'hidden_reader_localanchors_submit.lock').open('a') as lock:
    fcntl.flock(lock,fcntl.LOCK_EX)
    queue=subprocess.check_output(['squeue','-u','liuhanzuo','-h','-o','%i|%j|%T|%N'],text=True)
    assert 'hreader-la-' not in queue
    (R/'queue_before.txt').write_text(queue)
    for arm in configs:
        out=R/arm;out.mkdir()
        command=['sbatch','--parsable','--no-requeue','--job-name=hreader-la-'+arm,
        ...
        record=dict(arm=arm,depths=configs[arm],returncode=p.returncode,stdout=p.stdout,stderr=p.stderr,command=command)
