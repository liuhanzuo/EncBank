"""Encbank — Comprehension Memory: a self-contained mid-depth-resume long-context
...
distribution. Encbank exploits this by splitting the backbone at an arbitrary depth
``j`` (``resume_j``):
...
gate (``python -m encbank.selftest``; see the package tests).

Headline results (Qwen3-8B; see the paper): full-context attention collapses to 0
past its RoPE window while Encbank holds RULER ~100 / LongEval ~0.98 at 128k, and
...
# baseline arms are expressed as a re-parameterisation of the Encbank primitives.
#   encbank    : retrieval (selector + topk) + the given resume_j.
...
# choice (build Encbank(resume_j=0) for a faithful kvdirect).
_MODE_NO_RETRIEVAL = {"encbank": False, "kvdirect": True, "hcache": True}
...
class Encbank:
    ...
    def __init__(
        ...
        # Prepend a BOS attention sink during the WRITE pass (see write_chunk).
        # Default False preserves the published behaviour exactly; flip it only on the
        # strength of the end-to-end A/B in exp/s4_write_sink.py.
        self.write_sink: bool = False
    ...
    def _layer_out_hidden(out):
        ...
        unwrap that so Encbank works on both conventions."""
    ...
    # ------------------------------------------------------------------ #
    def _sink_prefix_id(self) -> int:
        """BOS (falling back to EOS) used as the write-time attention sink."""
        tk = getattr(self, "tokenizer", None)
        bos = getattr(tk, "bos_token_id", None) if tk is not None else None
        if bos is None and tk is not None:
            bos = getattr(tk, "eos_token_id", None)
    ...
    def write_chunk(self, token_ids) -> torch.Tensor:
        ...
        With ``self.write_sink`` set, a single BOS token is prepended for the forward
        and its position is dropped from the result, so the returned tensor is the same
        shape and covers the same tokens either way.

        Why the flag exists.  Without a prefix the chunk is forwarded with nothing in
        front of it, which is the regime where attention has no sink to park mass in.
        The READ pack already carries a sink; the write pass did not, so every cached
        h_j was produced in that degenerate regime.

        DO NOT TURN THIS ON UNCONDITIONALLY -- it reverses sign with depth.  End-to-end
        A/B on Qwen3-1.7B (L=28), `exp/s4_write_sink.py`, n=24, metric
        `KL(p_ref||p_arm)/KL(p_ref||p_no_mem)` over the full vocabulary at 32 query
        positions (0 = as good as having the chunk in context, 1 = contributed nothing):

            j        2      8     12     16  |    18     22     26
            off  0.131  0.288  0.435  0.531  | 0.610  0.755  0.849
            on   0.029  0.254  0.372  0.500  | 0.621  0.799  0.955
            d   -0.102 -0.034 -0.063 -0.031  | +0.010 +0.044 +0.106

        The sink helps for j <= 16 and HURTS for j >= 18, crossing over near 60% of
        depth.  At the published operating point (j/L = 12/36 = 1/3, i.e. j ~ 9 here)
        it is worth about -0.04 to -0.05, and at shallow j it removes 78% of the loss.
        The mechanism behind the reversal is NOT established.

        Default stays False so no existing measurement changes silently.  Turning it on
        is recommended only for j/L below ~0.57, and that threshold comes from a single
        model and a single corpus.
        """
        ids = self._as_ids(token_ids)
        use_sink = bool(getattr(self, "write_sink", False))
        if use_sink:
            pre = torch.tensor([[self._sink_prefix_id()]], device=ids.device,
                               dtype=ids.dtype)
        ...
        return h_j[:, 1:, :] if use_sink else h_j  # [1, T, d]
    ...
    def write_chunks(
        ...
        for T, idxs in by_len.items():
            ...
            bs = max(1, bs)
            # one extra position when the sink is on; prepending the same single token
            # to every member of the group keeps the group equal-length, so batching is
            # unaffected and this stays identical to write_chunk applied elementwise
            T_run = T + 1 if use_sink else T
            positions = torch.arange(T_run, device=self.device).unsqueeze(0)  # [1,T_run]
            for s in range(0, len(idxs), bs):
                batch_idx = idxs[s:s + bs]
                ids = torch.cat([ids_list[i] for i in batch_idx], dim=0)  # [B,T]
                if use_sink:
                    pre = torch.full((ids.shape[0], 1), self._sink_prefix_id(),
                                     device=ids.device, dtype=ids.dtype)
                    ids = torch.cat([pre, ids], dim=1)
                inputs_embeds = self.embed_tokens(ids)                    # [B,T_run,d]
                causal_mask, position_embeddings = self._make_mask_and_rope(
                    inputs_embeds, positions
                )
                h = self._run_layers(
    ...
    def generate_from_ids(
        ...
        *,
        ...
        selector needs (see :class:`encbank.selectors.DenseBGERetriever`)."""
        tok = tokenizer if tokenizer is not None else self.tokenizer
        tokens = input_ids[0]
    ...
    def encode(self, context: str, chunk_size: int = 512, sink_tokens: str = "bos"):
        ...
        if self.tokenizer is None:
            raise ValueError("encode(text) needs a tokenizer; pass tokenizer= to "
                             "Encbank(...) or call encode_ids on token ids")
    ...
    def generate_ids(
        ...
        *,
        ...
        Returns the generated token ids. ``mode`` in {encbank, kvdirect, hcache}
        toggles retrieval (kvdirect/hcache pack all chunks; build Encbank(resume_j=0)
        ...
        *,
        ...
        if self.tokenizer is None:
            raise ValueError("generate(text) needs a tokenizer; pass tokenizer= to "
                             "Encbank(...) or call generate_ids on token ids")
