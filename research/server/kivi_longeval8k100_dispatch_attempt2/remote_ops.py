...
RELATIVE='paper_autonomous_multifork_iteration/evidence/honly_kivi_longeval_remote_preparation_20260912/package_v2'
REMOTE='/srv/encbank/qencbank_align_codex_20260911/repo'
PYTHON='/srv/encbank/qencbank_runtime_20260911/python312/bin/python'
CONTROL='/srv/encbank/qencbank_align_codex_20260911/kivi_longeval8k100_dispatch_attempt2'
...
def main():
 ...
 if a.mode=='validate':
  ...
  try:
   ...
   report.update(status='PASS_sparse_files_original_staging_and_two_CPU_checks',finished_at=now(),resolved_storage_root=str(home),no_model_framework_or_GPU_execution=True)
 ...
 try:
  assert read(control/'package_validation.json')['status']=='PASS_sparse_files_original_staging_and_two_CPU_checks'
  ...
  for line in queue['stdout'].splitlines():
   ...
   assert 'qencbank' not in fields[2].lower() and '/qencbank_align_codex_20260911/' not in text,'Another active task-owned GPU/CPU-loading job; inspect before new quality submission'
  ...
  assert not workers,'Task-owned access-host model/quality process exists; no duplicate or concurrent launch'
