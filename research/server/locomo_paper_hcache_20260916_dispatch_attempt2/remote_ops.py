...
RELATIVE='paper_autonomous_multifork_iteration/evidence/locomo_paper_astra_expansion_20260916/preparation/hcache/package'
REMOTE='/srv/encbank/qencbank_align_codex_20260911/repo'
PYTHON='/srv/encbank/qencbank_runtime_20260911/python312/bin/python'
CONTROL='/srv/encbank/qencbank_align_codex_20260911/locomo_paper_hcache_20260916_dispatch_attempt2'
...
def main():
 ...
 if a.mode=='validate':
  ...
  try:
   ...
   assert plan['launch_permitted'] is True and plan['arm_order']==['public_hcache_j12']
   ...
   report.update(status='PASS_sparse_files_original_staging_and_one_public_hcache_j12_CPU_check',finished_at=now(),resolved_storage_root=str(home),no_model_framework_or_GPU_execution=True)
 ...
 try:
  assert read(control/'package_validation.json')['status']=='PASS_sparse_files_original_staging_and_one_public_hcache_j12_CPU_check'
  ...
  queue=held(control,'fresh_squeue',['squeue','--array','--me','--noheader','--format=%i|%T|%j']);rec['pre_submit_queue']=queue;assert queue['actual_exit_code']==0
