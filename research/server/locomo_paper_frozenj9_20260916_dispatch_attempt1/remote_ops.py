...
CONTROL='/srv/encbank/qcomem_align_codex_20260911/locomo_paper_frozenj9_20260916_dispatch_attempt1'
...
def main():
 ...
 if a.mode=='validate':
  ...
  try:
   ...
   assert plan['launch_permitted'] is True and plan['arm_order']==['comem_frozen_j9']
   ...
   report.update(status='PASS_sparse_files_original_staging_and_one_comem_frozen_j9_CPU_check',finished_at=now(),resolved_storage_root=str(home),no_model_framework_or_GPU_execution=True)
 ...
 try:
  assert read(control/'package_validation.json')['status']=='PASS_sparse_files_original_staging_and_one_comem_frozen_j9_CPU_check'
