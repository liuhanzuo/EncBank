...
inventory=P['resource_inventory']
...
eligible=sorted((x for x in inventory if x['memory_mb']<=2048 and x.get('cpus',1)<=1),key=lambda x:x['task'])
by={x['task']:x for x in eligible};assert all(x in by for x in priority)
tasks=priority+[x['task'] for x in eligible if x['task'] not in priority]
tasks=tasks[:32];assert len(tasks)==32
...
for label,arm,concurrency in arms:
    ...
    plan=dict(P,experiment='Qwen3.8-27B Transformers real Terminal-Bench hot-prefix comparison',
        tasks=tasks,arm=arm,arms=[arm],task_concurrency=concurrency,decode_batch_size=concurrency,
