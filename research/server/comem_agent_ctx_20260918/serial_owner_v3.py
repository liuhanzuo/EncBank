...
PREFIXES = ('qcm-', 'qcomem-', 'comem-', 'midcache-', 'cm-')
OUT = H / 'serial_queue_v3'
...
try:
    ...
    for arm in plan['arms']:
        ...
        while True:
            with (H.parent / 'qcomem_gpu_admission.lock').open('a+b') as admission:
                ...
                if count < 4:
                    ...
                    record = dict(at=now(), arm=arm, argv=argv, prior_jobs=jobs, prior_gpu_requests=count, status='submission_intent', shared_admission_lock=str(H.parent / 'qcomem_gpu_admission.lock'))
                    save(receipt, record)
