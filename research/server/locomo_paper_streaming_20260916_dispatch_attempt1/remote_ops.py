...
RELATIVE='paper_autonomous_multifork_iteration/evidence/locomo_paper_astra_expansion_20260916/preparation/streaming/package'
REMOTE='/srv/encbank/qencbank_align_codex_20260911/repo'
PYTHON='/srv/encbank/qencbank_runtime_20260911/python312/bin/python'
CONTROL='/srv/encbank/qencbank_align_codex_20260911/locomo_paper_streaming_20260916_dispatch_attempt1'
...
def main():
 ...
 if a.mode=='validate':
  ...
  try:
   ...
   assert plan['launch_permitted'] is True and plan['arm_order']==['streamingllm_style_encbank']
   ...
   report.update(status='PASS_sparse_files_original_staging_and_one_streamingllm_style_encbank_CPU_check',finished_at=now(),resolved_storage_root=str(home),no_model_framework_or_GPU_execution=True)
 ...
 try:
  assert read(control/'package_validation.json')['status']=='PASS_sparse_files_original_staging_and_one_streamingllm_style_encbank_CPU_check'
  ...
  queue=held(control,'fresh_squeue',['squeue','--me','--array','--noheader','--format=%i|%T|%j']);rec['pre_submit_queue']=queue;assert queue['actual_exit_code']==0
  ...
  shell=(here/'batch.sbatch').read_text();assert all(flag in shell for flag in ('#SBATCH --partition=gpu','#SBATCH --constraint=l20d','#SBATCH --exclude=gpu-node1,gpu-node2,gpu-node8','#SBATCH --nodes=1')) and '#SBATCH --nodelist=' not in shell
