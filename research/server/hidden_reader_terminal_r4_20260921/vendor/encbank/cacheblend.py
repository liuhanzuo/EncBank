"""CacheBlend-style full-depth chunk-KV baseline for Encbank.
...
attention. It is the natural single-variable control for Encbank, because the ONLY
...
Encbank (``resume_j=12``)       one depth-``j`` residual     8 KiB/token
============================  ==========================  ===================
...
flagship Encbank: same selector, chunk size, top-k, sink, and pack order. So
...
Run ``python -m encbank.cacheblend`` for the CPU correctness gate.
"""
...
class CacheBlend:
    ...
    Built from a :class:`encbank.Encbank` so it reuses Encbank's verified primitives
    ...
    conventions as the Encbank arm it is compared against. ``resume_j`` is
    irrelevant here — this path always executes all ``L`` layers — so the Encbank
    instance may be built at any ``j``.

    ``recompute_ratio`` is the default ``r`` for :meth:`generate_from_ids`, whose
    signature mirrors :meth:`Encbank.generate_from_ids` so an eval driver can swap
    the two arms by swapping one object.
    """

    def __init__(self, encbank: Encbank, recompute_ratio: float = 0.15):
        if not isinstance(encbank, Encbank):
            raise TypeError(f"CacheBlend needs a encbank.Encbank instance; got {type(encbank)!r}")
        ...
        self.config = encbank.config
        self.num_layers = encbank.num_layers
        self.device = encbank.device
        self.dtype = encbank.dtype
        self.tokenizer = encbank.tokenizer
    ...
    def prefill_chunk_full(self, token_ids, rope_start: int = 0):
        ...
        Mirrors :meth:`Encbank.write_prefill` but over the FULL band ``layers[0:L]``,
    ...
    # ------------------------------------------------------------------ #
    # end-to-end: same entry-point contract as Encbank.generate_from_ids
    # ------------------------------------------------------------------ #
    @torch.no_grad()
    def generate_from_ids(self, input_ids: torch.Tensor, *, chunk_size: int = 512,
        ...
        with the SAME selector as Encbank, then run the CacheBlend read + decode.

        Signature-compatible with :meth:`Encbank.generate_from_ids` (plus
        ...
        if no_retrieval:
            ...
            # store), so this arm runs the lexical/dense selectors Encbank is
            # compared at.
            if selector in ("reader_attn", "iter_reader_attn"):
                raise ValueError(
                    ...
                    "selector (the arm is compared at Encbank's iter_bm25 anyway)")
...
def run_self_test(tol: float = 1e-4, verbose: bool = True) -> bool:
    ...
    tok = _TinyTok(256)
    cb = CacheBlend(Encbank(model, resume_j=0, tokenizer=tok))
    ...
    if verbose:
        print("=" * 72)
        print(f"Encbank CacheBlend self-test (tiny Qwen3, fp32, L={cb.num_layers}, "
              f"tol={tol:.0e})")
        print("=" * 72)
