...
if phase=='qualify':
    ...
    verify_sources();assert json.loads((ROOT/'jobs'/'qualification_complete.json').read_text())['passed']
    assert json.loads((ROOT/'jobs'/'qualify_parent_exit.json').read_text())['exit_code']==0
queue=subprocess.check_output(['squeue','-u','liuhanzuo','-h','-o','%i|%j|%T|%N'],text=True)
assert not any('bandtb-' in line for line in queue.splitlines()),'Existing owned jobs require review before resubmission'
...
for kind,task in specs:
    name='bandtb-'+(task[:28] if task else kind)
    command=['sbatch','--parsable','--job-name='+name,'--partition=gpu','--time=UNLIMITED','--no-requeue',
        '--cpus-per-task='+('4' if kind=='worker' else '16'),'--mem='+('64G' if kind=='worker' else '48G'),
        '--chdir='+str(ROOT),'--output='+str(ROOT/'logs'/(name+'-%j.slurm.out')),'--error='+str(ROOT/'logs'/(name+'-%j.slurm.err'))]
    if kind=='worker':command+=['--gres=gpu:nvidia_l20d:1']
    else:command+=['--nodelist=gpu-node1']
