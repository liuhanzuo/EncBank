"""Encbank correctness self-test (CPU, tiny random Qwen3, no weights).

Run: ``python -m encbank.selftest``
...
Gates (all must pass; fp32, tolerance 1e-4):
  ...
  (E) the external BASELINE gates, so one command covers the whole repo:
      * ``encbank.cacheblend`` — RoPE reindex exact, ``r=1.0`` == full prefill,
        ``r=0.0`` finite (see :func:`encbank.cacheblend.run_self_test`);
      * ``encbank.kvcompress`` — SnapKV / PyramidKV do not perturb the model when
        no compression fires, and honour their retained-KV budget
        (see :func:`encbank.kvcompress.run_self_test`).
...
class _StubBGERetriever:
    """Offline stand-in for :class:`encbank.selectors.DenseBGERetriever`.
...
def run(n_layers=6, hidden=64, vocab=256, chunk_size=8, tol=1e-4, verbose=True):
    ...
    qc0 = Encbank(model, resume_j=0, tokenizer=tok)
    ...
    diffs_j = {}
    for j in (1, L // 2, L):
        qcj = Encbank(model, resume_j=j, tokenizer=tok)
    ...
    resume_j = max(1, L // 2)
    qc = Encbank(model, resume_j=resume_j, tokenizer=tok)
    n_ctx = 5
    ...
    for selector in ("recency", "bm25", "reader_attn", "iter_reader_attn", "iter_bm25"):
        ...
        erg_ids = qc.generate_ids(
            query_list, selector=selector, topk=3, mode="encbank",
    ...
    if verbose:
        print("=" * 72)
        print(f"Encbank self-test (tiny Qwen3, fp32, L={L}, tol={tol:.0e})")
        print("=" * 72)
        ...
        print(f"  (E) baseline cacheblend (reindex/r=1/r=0)  : "
              ...
              f"[python -m encbank.cacheblend for detail]")
        print(f"  (E) baseline snapkv+pyramidkv (stock/budget): "
              f"{'PASS' if kvc_ok else 'FAIL'}   "
              f"[python -m encbank.kvcompress for detail]")
