...
with (T/'hot24_peft_submission.lock').open('a') as lock:
    ...
    assert len(previous)==1 and previous[0]['job']=='122024'
    ...
    cmd=['sbatch','--parsable','--job-name=tf27b-tb-hot24-peft','--partition=gpu',
        ...
        '--exclude=gpu-node1,gpu-node3,gpu-node6,gpu-node7','--chdir='+str(R),
    ...
    row=dict(label='hot24_peft_merged',arm='hot',variant=p['variant'],concurrency=24,hot_chunks=24,
        ...
        supersedes_speed_pilot='121985 (user redirect, evidence retained)',previous_attempt='122024',previous_root=previous[0]['root'],automatic_retries=0)
    ...
    assert records[index]['job']=='122024'
