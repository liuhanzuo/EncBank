...
async def main():
    ...
    config=dict(task={'path':str(__import__('pathlib').Path(PLAN['task_root'])/task)},
        trial_name=task+'__tf27b_'+arm,trials_dir=str(directory),install_only=qualify,
        agent={'import_path':'tb_agent:JointBandTerminus','model_name':'Qwen3.8-27B/Transformers-'+arm,
            'kwargs':{'enable_summarize':False,'proactive_summarization_threshold':0,'interleaved_thinking':True,'reasoning_effort':'xhigh',
