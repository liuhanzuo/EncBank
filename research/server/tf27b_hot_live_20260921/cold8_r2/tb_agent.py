...
class AgentLLM(BaseLLM):
    ...
    async def call(self,prompt,**kwargs):
        ...
        text=result['text']
        reasoning,text=text.split('</think>',1) if '</think>' in text else (text,'')
        return LLMResponse(content=text.strip(),reasoning_content=reasoning,model_name='Qwen3.8-27B/Transformers-'+self.arm,
            usage=UsageInfo(prompt_tokens=result['prompt_tokens'],completion_tokens=result['generated_tokens'],cache_tokens=result.get('cached_prefix_tokens',0),cost_usd=0),
