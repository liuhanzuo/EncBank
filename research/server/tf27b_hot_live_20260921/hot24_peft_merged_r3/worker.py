...
def active_cache_bytes(rows):
    # Row caches are views of shared batch storage; count each allocation once.
    stores={}
    for row in rows:
        for cache in [row.upper,row.lower]:
...
def main():
    ...
    for n in ['hybrid_hot.py','hybrid_reader.py','batch_cache.py']:
    ...
    save(OUT/'ready.json',dict(job=os.environ['SLURM_JOB_ID'],pid=os.getpid(),gpu=device,torch=torch.__version__,
        arm=P['arm'],variant=P['variant'],lora_execution=P['lora_execution'],max_decode_batch=P['decode_batch_size'],max_tasks=P['task_concurrency'],base_allocated_gib=torch.cuda.memory_allocated()/2**30))
    ...
    def finish(x):
        ...
        payload=dict(request_id=q['request_id'],request_sha256=sha(path),status=x.status,
            ...
            engine='transformers',arm=P['arm'],variant=P['variant'],lora_execution=P['lora_execution'],cached_prefix_tokens=x.events[0].get('cached_prefix_tokens',x.events[0].get('hit_chunks',0)*512))
    ...
    while True:
        ...
        for path in pending:
            ...
            current=torch.cuda.memory_allocated();active_bytes=active_cache_bytes(active)
