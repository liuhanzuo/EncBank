...
with (T/'hot24_peft_submission.lock').open('a') as lock:
    ...
    assert all(x['root']!=str(R) and x['label']!='hot24_peft_merged' for x in records)
    ...
    assert 'tf27b-tb-hot24-peft' not in queue
    ...
    row=dict(label='hot24_peft_merged',arm='hot',variant=p['variant'],concurrency=24,hot_chunks=24,
        ...
        supersedes_speed_pilot='121985 (user redirect, evidence retained)',automatic_retries=0)
