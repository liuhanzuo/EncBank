...
def trial(task,arm):
    ...
    while True:
        admitted,proof=host_admission.acquire(ROOT.parent.name+'-'+ROOT.name,name,memory)
        if admitted:break
    ...
    try:
        ...
        out.close();err.close();host_admission.release(ROOT.parent.name+'-'+ROOT.name,name)
def pair(task,index):
    index=P.get('global_task_order',P['tasks']).index(task)
    if QUALIFY:
