...
def main():
    ...
    with torch.inference_mode(),sdpa_kernel([SDPBackend.FLASH_ATTENTION,SDPBackend.EFFICIENT_ATTENTION]):
        ...
        for count in [1,4,12]:
            ...
            replay=measure_replay(reader,model,cpu_bundle,DEPTHS,count,timed)
            replay_file=OUT/'replay.json'
            previous=json.loads(replay_file.read_text()) if replay_file.exists() else []
            dump('replay.json',previous+[replay])
    ...
    results.update(complete=True,arm=ARM,depths=DEPTHS,environment=env,selected_ridge=selected,
        ...
        h24_scope='All cached depths <=24, from the same fixed teacher joint-memory prefix; layers25..36 share source H24. No arbitrary changed-retrieval reuse claim.',
