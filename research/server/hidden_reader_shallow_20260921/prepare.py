...
records=[]
with (R.parent/'hidden_reader_shallow_submit.lock').open('a') as lock:
    fcntl.flock(lock,fcntl.LOCK_EX)
    queue=subprocess.check_output(['squeue','-u','liuhanzuo','-h','-o','%i|%j|%T|%N'],text=True)
    assert 'hreader-sh-' not in queue
    (R/'queue_before.txt').write_text(queue)
    for arm in configs:
        out=R/arm;out.mkdir()
        command=['sbatch','--parsable','--no-requeue','--job-name=hreader-sh-'+arm,
