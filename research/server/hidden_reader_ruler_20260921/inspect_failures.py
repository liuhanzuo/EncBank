...
for cell in ['multikey8k','vt8k']:
    ...
    for x in rows:
        refs=labels[x['id']];a=x['arms']['encbank']['prediction'];b=x['arms']['kd_selected']['prediction']
        if all(r.lower() in a.lower() for r in refs) and not all(r.lower() in b.lower() for r in refs):
            print(json.dumps(dict(cell=cell,id=x['id'],references=refs,encbank=a,kd640=b,kd2048=x['arms']['kd2048']['prediction'],
