...
for cell,c in configs.items():
    ...
    checks.append(dict(cell=cell,items=100,official_score_verified=True,exact_cache_control=json.loads((out/'exact_cache_control.json').read_text())))
def describe(arrays):
...
jobs=json.loads((R/'submissions.json').read_text());ids=','.join(x['job'] for x in jobs)
