...
lines=['# 四点 hidden 缓存：延长蒸馏训练','',
    '两组均从先前256步模型继续1792次更新，累计2048步，全部正常结束。骨干、原Encbank LoRA、writer和原层投影冻结，只训练20个跨层KV仿射头；缓存H12/H16/H20/H24，不缓存H24之后的hidden。模型权重仅保留在服务器。','',
...
lines += ['',
    ...
    '完整2048步端点另外保存，RULER质量评测对比原Encbank、256步、验证选中640步、同一训练分支2048步端点。没有使用RULER分数来选择训练分支或检查点。','',
...
lines=['# 四点 hidden→KV：RULER质量对照','',
    '这批测试显示当前四点hidden投影方案存在明显任务质量损失；正文续写KL改善没有转化为多键检索和变量追踪能力的恢复。增加到2048步没有解决这一问题，当前结果不支持将它作为原Encbank的无损替代。','',
    'Qwen3-8B + 原Encbank LoRA，900道固定题目，四组主对照共3600次独立贪心生成，另外900次真实KV分开处理对照，共4500次有效生成。各组共享原始题目、分块、检索结果、提示词、EOS、生成长度上限与评分函数。原Encbank是本次同环境重新跑出的基线。','',
    ...
    '| 任务／原始上下文长度 | 原Encbank | 真KV分开处理 | 256步 | 验证选中640步 | 2048步 | 640步相对Encbank |',
    '|---|---:|---:|---:|---:|---:|---:|']
...
for cell,x in s['cells'].items():
    ...
    lines.append(f"| {labels[task]} / {length} | {v['encbank']:.2f} | {v['encbank_split']:.2f} | {v['kd256']:.2f} | {v['kd_selected']:.2f} | {v['kd2048']:.2f} | {x['comparison']['kd_selected']['delta_vs_encbank_pp']:+.2f} pp |")
lines += ['','## 成对统计','',
    '| 范围 | 原Encbank | 真KV分开处理 | 256步 | 640步 | 2048步 | 640步差值的95%区间 |',
    '|---|---:|---:|---:|---:|---:|---|']
for key,label in [('official_niah_only','NIAH两任务×三长度，600题'),('encbank_ruler_style_vt','Encbank变量追踪×三长度，300题')]:
    g=s['groups'][key];v=g['score'];ci=g['comparison']['kd_selected']['paired_95ci_pp']
    lines.append(f"| {label} | {v['encbank']:.2f} | {v['encbank_split']:.2f} | {v['kd256']:.2f} | {v['kd_selected']:.2f} | {v['kd2048']:.2f} | [{ci[0]:+.2f}, {ci[1]:+.2f}] pp |")
for key,label in [('official_niah_only','NIAH'),('encbank_ruler_style_vt','变量追踪')]:
    g=s['groups'][key];c=g['comparison']['kd_selected']
    lines += ['',f"{label}：640步相对原Encbank平均{c['delta_vs_encbank_pp']:+.2f}个百分点；{c['encbank_full_correct_to_student_not_full']}题由完全正确变为非完全正确，{c['student_full_correct_from_encbank_not_full']}题反向改善。"]
    ci=c['paired_95ci_vs_split_pp'];num=g['comparison']['encbank_split']
    lines += [f"真实KV分开处理对照相对原Encbank为{num['delta_vs_encbank_pp']:+.2f}个百分点；640步相对该对照为{c['delta_vs_encbank_split_pp']:+.2f}个百分点，95%区间[{ci[0]:+.2f}, {ci[1]:+.2f}]。"]
lines += ['',
    '置信区间采用按任务和长度分层的10,000次成对题目bootstrap，未做多重比较校正。平均值相近或区间跨零，都不能证明无质量损失；这里只描述这批固定样本。','',
    '## 失败样例核查','',
    '640步模型的多键任务中，89题由原Encbank正确变为错误；这89题的目标答案全部仍在相同的检索文本中。88题首行给出了错误数字，其中84题的错误数字可以在检索文本的其他位置找到。变量追踪中，288题由原先完全正确变为非完全正确，所需变量名全部仍在检索文本中；其中80题首行把问题里的数值重复至少3次。这些是错误模式统计，不能单独证明具体内部机制。详细样例见failure_analysis.json。','',
    ...
    '- 单目标与多键NIAH使用已有固定输入，来自NVIDIA官方RULER生成器；变量追踪来自现有Encbank内置RULER风格生成器，单独标明。此处不是完整13任务RULER总分。',
    ...
    '- 真KV对照把原生Encbank联合prefill的历史KV原样取出，再单独处理问题，使用与学生相同的query/cache调用路径。历史KV逐位不变且缓存长度正确已核验；BF16联合/分开计算本身不保证logits或argmax逐位相同，因此另外测了全部900题的输出，而不将数值差异视为零。',
