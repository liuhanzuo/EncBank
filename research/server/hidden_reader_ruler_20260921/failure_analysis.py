...
tok=AutoTokenizer.from_pretrained('/srv/encbank/encbank_sparse_slurm_20260912/models/Qwen3-8B',local_files_only=True)
inputs={}
...
for task in ['single','multikey','vt']:
    ...
    for x in subset:
        ...
        if x['scores']['encbank']!=1 or x['scores']['kd_selected']==1:continue
        counts['encbank_full_correct_student_not_full']+=1
        ...
        if len(examples[task])<3:
            examples[task].append(dict(cell=x['cell'],id=x['id'],references=x['references'],
                encbank=x['predictions']['encbank'],kd640=x['predictions']['kd_selected'],kd2048=x['predictions']['kd2048']))
