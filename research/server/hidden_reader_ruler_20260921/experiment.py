...
def main():
    ...
    def capture(chunks):
        ...
        return {d:torch.cat(pieces,1) for d,pieces in result.items()}
    ...
    with torch.inference_mode(),sdpa_kernel([SDPBackend.FLASH_ATTENTION,SDPBackend.EFFICIENT_ATTENTION]):
            ...
            for index,case in enumerate(cases):
                ...
                for arm in ['encbank','kd256','kd_selected','kd2048']:
                    ...
                    if arm=='encbank':
                        ...
                        if index==0:exact_cache_control(logits,top,query,n)
        ...
        dump('summary.json',dict(items=len(cases),generations=4*len(cases),elapsed_s=time.perf_counter()-start,
            predictions_sha256=sha(OUT/'predictions.jsonl'),peak_allocated_GiB=torch.cuda.max_memory_allocated()/2**30))
        status('COMPLETE',items=len(cases),generations=4*len(cases))
