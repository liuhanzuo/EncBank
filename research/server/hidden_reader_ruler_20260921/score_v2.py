...
ARMS=['encbank','encbank_split','kd256','kd_selected','kd2048']
...
for cell,c in configs.items():
    ...
    if control_configs[cell]['control_only']:
        for x in predictions:
            ...
            x['arms']['encbank_split']=control['arms']['encbank_split']
...
def describe(arrays):
    ...
    for j,arm in enumerate(ARMS[1:],1):
        ...
        result['comparison'][arm]=dict(delta_vs_encbank_pp=round(float(means[j]-means[0]),4),
            ...
            delta_vs_encbank_split_pp=round(float(means[j]-means[1]),4),
...
result=dict(checkpoints=json.loads((R/'checkpoints.json').read_text()),cells={c:describe([x]) for c,x in scores.items()},
    groups={g:describe([x for c,x in scores.items() if predicate(c)]) for g,predicate in [
        ...
        ('official_niah_multikey',lambda c:c.startswith('multikey')),('encbank_ruler_style_vt',lambda c:c.startswith('vt')),('all_three_tasks_exploratory',lambda c:True)]},
