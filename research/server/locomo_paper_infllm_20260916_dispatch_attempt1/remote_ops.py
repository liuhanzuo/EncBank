...
def main():
 ...
 if a.mode=='validate':
  ...
  try:
   ...
   report.update(status='PASS_sparse_files_original_staging_and_one_infllm_resident_qwen3_CPU_check',finished_at=now(),resolved_storage_root=str(home),no_model_framework_or_GPU_execution=True)
 ...
 try:
  assert read(control/'package_validation.json')['status']=='PASS_sparse_files_original_staging_and_one_infllm_resident_qwen3_CPU_check'
