...
lines=['# 联合因果attention深度实验','',
    ...
    'Qwen3-8B + 原COMem LoRA，冻结全部模型参数，不使用此前训练的hidden→KV头。缓存独立chunk的H12，历史第13～n层联合因果计算，第n+1～36层仅在各chunk内部计算；问题与生成token在上层仍读取全部检索历史。','',
    'n=12代表上层历史全部块内计算，n=36代表上层历史全部联合计算。每层仍执行原Attention和MLP；减少的是跨chunk的attention范围，没有省略Transformer层。原COMem原生联合prefill另外作为native对照。','',
    '## 32题／组合的深度扫描','',
    '| n | 联合层数 | 单目标得分 | 多键得分 | 变量追踪得分 | 12块重建ms | 相对n36时间 |',
    '|---|---:|---:|---:|---:|---:|---:|']
native=[screen['tasks'][t]['score']['native'] for t in labels]
lines.append('| 原COMem | 24 | '+' | '.join(f'{v:.2f}' for v in native)+' | — | — |')
for n in [12,14,16,18,20,24,28,32,36]:
...
for task,label in labels.items():
    ...
    lines += ['',f"{label}：选中配置相对原COMem {x['delta_pp']:+.2f}个百分点，成对bootstrap95%区间[{ci[0]:+.2f}, {ci[1]:+.2f}]；{x['lower_score_items']}题得分下降，{x['higher_score_items']}题提升。"]
lines += ['','## 检索块数量与耗时','',
...
lines += ['','## 实现与测量边界','',
    ...
    '- 通过小型不同长度chunk的完整block-diagonal参考实现，直接检查物理分组计算的K张量，并比较query输出（间接覆盖V）；n36另外与原生COMem比较。BF16不同计算布局可能有数值差异，记录原生对照分数，不宣称逐位等价。',
    ...
    '- 固定贪心生成上限：NIAH48 token，变量追踪60 token；分数为官方string_match_all参考答案召回率。变量追踪来自COMem现有生成器，本轮不是完整13任务RULER总分。',
