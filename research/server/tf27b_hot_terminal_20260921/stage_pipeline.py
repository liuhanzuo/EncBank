...
completed=(Q/'qualification_complete.json').exists() and (Q/'parent_exit.json').exists()
if completed:
...
dependency=[] if completed else ['--dependency=afterok:'+submission['job']]
cmd=['sbatch','--parsable','--job-name=tf27b-stage-live','--partition=gpu','--time=UNLIMITED','--no-requeue',
    '--cpus-per-task=2','--mem=8G',*dependency,
...
record=dict(job=res.stdout.strip().split(';')[0],depends_on=None if completed else submission['job'],qualified_job=submission['job'],qualification_already_complete=completed,qualification_root=str(Q),root=str(R),argv=cmd,epoch=time.time())
