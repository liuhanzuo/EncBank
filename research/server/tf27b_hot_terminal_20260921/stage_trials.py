...
from task_integrity import verify_tasks
...
filtered,task_proof=verify_tasks(P['task_root'],manifest,tasks)
...
for label,arm,concurrency in arms:
    ...
    (R/'task_integrity.json').write_text(json.dumps(task_proof,indent=2)+'\n')
...
for row in jobs:
    ...
    cmd=['sbatch','--parsable','--job-name=tf27b-tb-'+row['label'],'--partition=gpu','--time=UNLIMITED','--no-requeue',
        '--cpus-per-task='+str(cpus),'--mem=160G','--gres=gpu:nvidia_l20d:1','--exclude=gpu-node3,gpu-node6',
