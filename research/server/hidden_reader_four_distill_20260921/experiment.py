...
def main():
    ...
    def source(l):return max(s for s in DEPTHS if s<=l)
    active=[l for l in LAYERS if l not in DEPTHS]
    def timed(fn):
    ...
    def capture(chunks):
        ids=torch.tensor(chunks,device='cuda');local=reader.write_chunk(ids);h=local.reshape(1,-1,4096)
        sink=reader.write_chunk([model.config.bos_token_id]);h=torch.cat([sink,h],dim=1)
        local_sink=sink;local_states={12:h}
        lp=torch.arange(local.shape[1],device='cuda')[None].expand(len(chunks),-1)
        lm=(lp[0,None,:]<=lp[0,:,None])[None,None]
        lr=reader.rotary_emb(local,position_ids=lp)
        sp=torch.zeros((1,1),device='cuda',dtype=torch.long)
        sr=reader.rotary_emb(local_sink,position_ids=sp)
        for layer_idx in range(12,24):
            local=reader._run_layers(local,slice(layer_idx,layer_idx+1),lm,lp,lr,use_cache=False)
            local_sink=reader._run_layers(local_sink,slice(layer_idx,layer_idx+1),None,sp,sr,use_cache=False)
            if layer_idx+1 in DEPTHS:
                local_states[layer_idx+1]=torch.cat([local_sink,local.reshape(1,-1,4096)],1)
        ...
        return dict(states=states,local_states=local_states,kv={l:kv(cache,l) for l in LAYERS},n=n)
    def to_cpu(bundle):
        return dict(states={l:h.cpu() for l,h in bundle['states'].items()},
            local_states={l:h.cpu() for l,h in bundle['local_states'].items()},
    ...
    class Provider:
        def __init__(self,bundle,weights=None,teacher=False):
            ...
            if teacher:self.exact={l:tuple(t.to('cuda') for t in bundle['kv'][l]) for l in LAYERS}
                ...
                bases=DEPTHS
                self.h={l:bundle['local_states'][l].to('cuda') for l in bases}
        ...
        def project(self,l):
            ...
            if self.weights is None or l not in self.weights:
                ...
                w,b=self.weights[l]
                raw=F.linear(unit(self.h[source(l)]),w.to(torch.bfloat16),b.to(torch.bfloat16))
                if l in DEPTHS:raw=raw+raw_project(self.h[l],l)
    ...
    with torch.no_grad(),sdpa_kernel([SDPBackend.FLASH_ATTENTION,SDPBackend.EFFICIENT_ATTENTION]):
        ...
        if False:
            ...
            for i,case in enumerate(dataset['train']):
                ...
                for s in xs:xs[s].append(unit(bundle['local_states'][s][:,ix]).reshape(-1,4096).cpu())
        ...
        weights,training=train_heads({**globals(),**locals()},RUN_CONFIG,ROOT)
        results['distillation']=training
        checkpoint=ROOT/ARM/'heads.pt'
        torch.save(dict(arm=ARM,depths=DEPTHS,config=RUN_CONFIG,weights={l:tuple(t.cpu() for t in pair) for l,pair in weights.items()},training=training),checkpoint)
        results['checkpoint_sha256']=hashlib.sha256(checkpoint.read_bytes()).hexdigest()
        initializer=ROOT.parent/'hidden_reader_localanchors_20260921/four_local/heads.pt'
        baseline_weights=torch.load(initializer,map_location='cuda',weights_only=False)['weights']
        baseline=[]
        for case in dataset['test']:
            baseline.append(dict(id=case['id'],**evaluate(case,baseline_weights)))
        dump('baseline_test.json',baseline);results['baseline_test']=baseline
        status('TEST_HELDOUT',documents=len(dataset['test']))
        test=[]
        ...
        results['test']=test
        fresh_test=[]
        for i,case in enumerate(dataset.get('fresh_test',[])):
            base=evaluate(case,baseline_weights)
            trained=evaluate(case,weights,check=i==0)
            fresh_test.append(dict(id=case['id'],baseline=base,trained=trained))
            dump('fresh_test.json',fresh_test)
            status('TEST_FRESH',completed=i+1,total=len(dataset['fresh_test']))
        results['fresh_test']=fresh_test
    ...
    results.update(complete=True,arm=ARM,depths=DEPTHS,environment=env,selected_ridge=selected,
        head_parameter_bytes=sum(t.numel()*t.element_size() for pair in weights.values() for t in pair),
        test_mean={key:statistics.mean(row[key] for row in results['test']) for key in ['kl','top1_agreement','student_nll','teacher_nll']},
        scope='Four chunk-local depths H12/H16/H20/H24; logits distillation on128 training documents; validation-only checkpoint selection. Reused8-document and additional fresh test are reported separately; no agent task accuracy claim.',
        h24_scope='H12..H24 sources written independently per512-token chunk, sink independently; reused under new joint memory packing. Targets remain original joint-reader KV. Exact per-layer control uses teacher states only, not these local anchors.',
