...
RELATIVE='paper_autonomous_multifork_iteration/evidence/literature_baseline_alignment_20260913/infllm_preparation_lf_v2/package'
REMOTE='/srv/encbank/qencbank_align_codex_20260911/repo'
PYTHON='/srv/encbank/qencbank_runtime_20260911/python312/bin/python'
CONTROL='/srv/encbank/qencbank_align_codex_20260911/literature_infllm_resident_single16k_lfv2_dispatch_attempt1'
...
def main():
 ...
 if a.mode=='validate':
  ...
  try:
   ...
   assert plan['launch_permitted'] is True and plan['arm_order']==['infllm_resident_qwen3']
   ...
   report.update(status='PASS_sparse_files_original_staging_and_one_InfLLM_CPU_check',finished_at=now(),resolved_storage_root=str(home),no_model_framework_or_GPU_execution=True)
 ...
 try:
  assert read(control/'package_validation.json')['status']=='PASS_sparse_files_original_staging_and_one_InfLLM_CPU_check'
  ...
  rec['model_lineage_files_verified']=verified;rec['adapter_loaded']=False;rec['adapter_audited_but_unused']=True
