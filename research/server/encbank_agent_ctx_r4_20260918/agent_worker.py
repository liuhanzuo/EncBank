      ...
      while not (box/'stop.json').exists():
        ...
        for req in pending:
            ...
            if arm=='dense':
                ...
                # Report the native incremental/full BF16 difference; finite and token-prefix/cache-length gates remain strict.
                if data['step']==1:
                    diagnostic_start=stamp()
                    check=model(input_ids=reader.tensor(ledger),use_cache=False,logits_to_keep=1).logits
                    reference=check.float().flatten();actual=logits.float().flatten()
                    ref_logp=reference.log_softmax(-1);actual_logp=actual.log_softmax(-1)
                    parity=dict(max_abs=float((reference-actual).abs().max()),rms=float((reference-actual).square().mean().sqrt()),top1_equal=bool(reference.argmax()==actual.argmax()),kl_incremental_reference=float((actual_logp.exp()*(actual_logp-ref_logp)).sum()),top10_overlap=len(set(reference.topk(10).indices.tolist()) & set(actual.topk(10).indices.tolist()))/10,finite=bool(reference.isfinite().all() and actual.isfinite().all()),old_max_abs_gate_pass=bool((reference-actual).abs().max()<=.5),policy='diagnostic_only_not_numerical_equivalence')
                    dump(R/(task+'.incremental_check.json'),parity);assert parity['finite'],'Nonfinite native/reference logits';response['cache_diagnostic']=parity
                    response['cache_diagnostic_seconds']=stamp()-diagnostic_start
                    response['exclude_request_from_speed_summary']=True
                    del check,reference,actual,ref_logp,actual_logp
            ...
            ended=stamp();active_bytes=cache_bytes(dense_cache) if arm=='dense' else cache_bytes(upper)+(cache_bytes(lower) if arm=='encbank' else 0)
