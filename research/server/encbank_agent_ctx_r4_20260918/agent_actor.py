...
from action_parser import parse_action, VERSION as ACTION_PARSER_VERSION
...
for task in plan['task_ids']:
    ...
    try:
        ...
        for step in range(plan['max_steps']):
            ...
            text=response['text'];parser_error=None
            try: code,action_format=parse_action(text)
            except ValueError as exc: code='';action_format='rejected';parser_error=str(exc)
            messages.append(dict(role='assistant',content=text))
            t=real_clock.real_perf_counter()
            try: output=('Action format error: '+parser_error) if parser_error else world.execute(code)
            ...
            events.append(dict(step=step,request=key,code=code,action_format=action_format,parser_version=ACTION_PARSER_VERSION,output=str(output),tool_ms=tool_ms,model=response))
