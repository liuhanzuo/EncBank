...
terminal=load('terminal_snapshot.json')
if terminal:
    lines+=['','## 真实 Terminal-Bench 新试验','',f"状态快照：{terminal['at']}，有效完成 {terminal['valid_trials']}/{terminal['expected_trials']} 次。",'',
        '| 任务 | 配置 | 有效完成 | reward | 总秒数 | 模型秒数 | 输出tokens | 命中/请求chunk | 当前chunk命中 | 重建chunk |',
        '|---|---|---|---|---:|---:|---:|---:|---:|---:|']
    for r in terminal['rows']:
        wall=f"{r['wall_seconds']:.2f}" if r['wall_seconds'] is not None else '—'
        lines.append(f"| {r['task']} | {r['arm']} | {r['valid']} | {r['rewards']} | {wall} | {r['request_seconds']:.2f} | {r['generated_tokens']} | {r['hit_chunks']}/{r['requested_chunks']} | {r['promoted_hits']} | {r['rebuilt_chunks']} |")
    lines+=['','这些是全新环境、自由生成与原判分器的实际任务，包含环境、终端命令等耗时；轨迹和输出 token 数可能不同。当前已完成的短历史任务双方仍失败，不能据此证明质量保持。短历史任务的全部 KV 命中不保证整体更快，缓存维护和 query forward 仍有开销。其余结果以实时作业为准。']
