...
def main():
 ...
 assert root==Path('/srv/encbank/qcomem_align_codex_20260911/repo') and len(manifest['files'])==25
 relative='paper_autonomous_multifork_iteration/evidence/comem_honly_formal_20260911/next_longbench_qa6/qasper_full200_candidate/package'
 here=root/relative;here.resolve().relative_to(home);root.relative_to(home)
 cache=(home/'qcomem_align_codex_20260911/task_cache/qasper200_attempt1').resolve();cache.relative_to(home)
 ...
 try:
  ...
  assert not queue.stdout.strip(),'An active qcomem batch exists; do not submit a duplicate/overlapping namespace'
  ...
  assert available,'No currently idle GPU meets minimum free bytes; leave occupied tasks untouched'
  argv=['sbatch','--parsable',str(here/'batch.sbatch')]
