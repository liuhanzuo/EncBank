...
H=Path(__file__).resolve().parent;import sys
assert len(sys.argv)==2 and sys.argv[1] in ['dense','raw_base','raw_shared','encbank']
R=H/('run_'+sys.argv[1]);R.mkdir(exist_ok=False);(R/'mailbox').mkdir()
GPU='/srv/encbank/Paper_Evolve/.venv/bin/python'
CPU='/srv/encbank/qencbank_runtime_20260911/appworld_cpu_20260917'+"/bin/python"
def save(p,v):
