...
TOKEN='encbank-bge-matched-20260919'
...
def main():
    lock=(ROOT.parent/'qencbank_gpu_admission.lock').open('a+b');fcntl.flock(lock,fcntl.LOCK_EX)
    ...
    if sum(j['gpus'] for j in before)>3:
        ...
        if candidates:
            ...
            if not any(j['id']==job['id'] for j in current):
                ...
                assert len(parent)==1 and parent[0][1].startswith('CANCELLED') and parent[0][3] in ('Unknown',''),parent
