"""CPU queue owner: admit <=4 total owned RUNNING+PENDING GPU requests."""
...
def eta(task,receipt,slurm):
    ...
    if step>0 and elapsed>0:
        end=when+datetime.timedelta(seconds=elapsed/step*(8000-step))
        return dict(phase='training',step=step,seconds_per_step=elapsed/step,
...
def main():
    ...
    plan=json.loads((ROOT/'plan.json').read_text());assert str(ROOT.resolve())==plan['remote_root']
    ...
    while not (ROOT/'STOP_COORDINATOR').exists():
        ...
        for task in plan['tasks']:
            ...
            details={}
            if rec:
                if not rec.get('job'):state='SUBMISSION_UNCERTAIN';failed.add(task['id'])
                else:
        ...
        with (ROOT.parent/'qencbank_gpu_admission.lock').open('a+b') as admission:
            ...
            for task in plan['tasks']:
                ...
                cmd=['sbatch','--parsable','--job-name='+name,'--partition=gpu','--gres=gpu:nvidia_l20d:1',
                     ...
                     '--chdir='+str(ROOT),str(ROOT/'environment.sh'),task['id']]
        ...
        dump(ROOT/'status.json',dict(phase=phase,at=now(),pid=os.getpid(),owned_queue=jobs,
             ...
             task_count=len(plan['tasks']),tasks=states,
