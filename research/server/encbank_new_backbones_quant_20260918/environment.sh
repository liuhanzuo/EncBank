#!/bin/bash
set -euo pipefail
cd /srv/encbank/qencbank_align_codex_20260911/encbank_new_backbones_quant_20260918
...
export TMPDIR="$PWD/cache/tmp-${SLURM_JOB_ID:-cpu}"
export TRITON_CACHE_DIR="$PWD/cache/triton-${SLURM_JOB_ID:-cpu}"
export CUDA_CACHE_PATH="$PWD/cache/cuda-${SLURM_JOB_ID:-cpu}"
mkdir -p "$TMPDIR" "$TRITON_CACHE_DIR" "$CUDA_CACHE_PATH"
exec /srv/encbank/Paper_Evolve/.venv/bin/python -u -B worker.py "$1"
