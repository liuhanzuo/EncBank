...
def main():
    ...
    # Batch equivalence uses real trace tokens, independent H, and masked KV.
    rows=[Request(Session(r,'hot',24,str(i)),ids[:700+i*19],20+i) for i in range(2)]
    individual=[]
    for i in range(2):
        q=Request(Session(r,'hot',24,str(i)),ids[:700+i*19],20+i)
    ...
    assert max(values)<.02,values
    save(ROOT/'batch_qualification.json',dict(passed=True,kl=values))
