...
class Method:
    def __init__(self,model,tokenizer,arm,*,resume_j=12,reader_binding='unbound',bos_token_id=151643,eos_token_id=151645,kernels=None):
        if arm not in ('dense','h16','h8','h4','kivi2','kivi4'):raise ValueError('Six fixed methods only')
        ...
        self.memory=None
        if arm.startswith('h'):
            self.memory=HOnlyMemory(self.model,tokenizer,resume_j=resume_j,bits=int(arm[1:]),group_size=64,reader_binding=reader_binding,bos_token_id=bos_token_id,eos_token_id=eos_token_id)
        else:
