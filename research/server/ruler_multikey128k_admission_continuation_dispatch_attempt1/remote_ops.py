...
RELATIVE='paper_autonomous_multifork_iteration/evidence/honly_ruler_admission_continuations_20260913/multikey128k/package'
REMOTE='/srv/encbank/qencbank_align_codex_20260911/repo'
PYTHON='/srv/encbank/qencbank_runtime_20260911/python312/bin/python'
CONTROL='/srv/encbank/qencbank_align_codex_20260911/ruler_multikey128k_admission_continuation_dispatch_attempt1'
...
def main():
 ...
 if a.mode=='validate':
  ...
  try:
   ...
   assert plan['launch_permitted'] is True and plan['arm_order']==['h16', 'h8', 'h4', 'kivi2', 'kivi4']
   ...
   report.update(status='PASS_sparse_files_original_staging_and_5_continuation_CPU_checks',finished_at=now(),resolved_storage_root=str(home),no_model_framework_or_GPU_execution=True)
 ...
 try:
  assert read(control/'package_validation.json')['status']=='PASS_sparse_files_original_staging_and_5_continuation_CPU_checks'
  ...
  rec['model_lineage_files_verified']=verified;rec['adapter_loaded_by_KIVI']=False;rec['adapter_loaded_by_H']=True
