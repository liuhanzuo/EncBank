"""Prefill-then-compress KV-cache baselines for Encbank: **SnapKV** and **PyramidKV**.

These are *external comparison points*, not part of the Encbank mechanism: they
...
only the retained KV is bounded. Encbank's contrast is the opposite trade: a
...
Usage
-----
    from encbank.kvcompress import install_kv_compression, generate_kvcompress
...
``python -m encbank.kvcompress`` runs the CPU faithfulness self-test.
"""
...
# Encbank's read pack == BOS 1 + top-12 x 512 + query <= 512 = 6657 tokens.
ENCBANK_READ_BUDGET = 6657
...
class KVCompressCluster:
    ...
    def __init__(self, method: str, *, num_hidden_layers: int, layer_idx: int,
                 max_capacity_prompt: int = ENCBANK_READ_BUDGET, window_size: int = 32,
...
def _kvcompress_attention_forward(module, query, key, value, attention_mask,
    ...
    """
    base_impl = getattr(module.config, "_encbank_kvc_base_attn_impl", "sdpa")
...
def install_kv_compression(model, method: str, *,
                           max_capacity_prompt: int = ENCBANK_READ_BUDGET,
    ...
    cfg._encbank_kvc_prev_attn_impl = prev
    ...
    cfg._encbank_kvc_base_attn_impl = prev
...
def uninstall_kv_compression(model):
    ...
    inner = getattr(model, "model", model)
    cfg = inner.config
    cfg._attn_implementation = getattr(cfg, "_encbank_kvc_prev_attn_impl", "sdpa")
    for attr in ("_encbank_kvc_prev_attn_impl", "_encbank_kvc_base_attn_impl"):
        if hasattr(cfg, attr):
            delattr(cfg, attr)
    for layer in inner.layers:
        attn = layer.self_attn
...
def retained_kv_stats(cache) -> dict:
    ...
    equal-retained-budget diagnostic compares against Encbank's read pack."""
    total = 0
    per_layer = []
...
def run_self_test(tol: float = 1e-4, verbose: bool = True) -> bool:
    ...
    if verbose:
        print("=" * 72)
        print(f"Encbank KV-compression self-test (tiny Qwen3, fp32, budget={budget}, "
