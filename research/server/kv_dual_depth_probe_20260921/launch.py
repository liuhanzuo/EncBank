...
dump('owner.json',dict(pid=os.getpid(),job=os.environ['SLURM_JOB_ID'],root=str(root),
    at=datetime.datetime.now().astimezone().isoformat(),role='H12/H24 dual-depth reconstruction timing only'))
with (out/'stdout.log').open('wb') as stdout,(out/'stderr.log').open('wb') as stderr:
    child=subprocess.Popen(['/srv/encbank/Paper_Evolve/.venv/bin/python','-B','-u',str(root/'benchmark.py')],
