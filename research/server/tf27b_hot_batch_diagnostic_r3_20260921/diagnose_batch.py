...
def case(r,ids,lens,adapter,force_recurrent=False):
    with r.adapter(adapter):
        ...
        for i,block in enumerate(r.core.layers):
            captured={};handles=[];phase=['batch',0]
            def hook(name):
                def record(module,args,out):
                    value=out[0] if isinstance(out,tuple) else out
                    if not torch.is_tensor(value):return
                    if phase[0]=='batch':captured[name]=value.detach().clone()
                    elif name in captured:
                        expected=captured[name];n=value.shape[0];b=phase[1]
                        if expected.shape[0]==n*len(lens):
                            e=error(value,expected[b*n:(b+1)*n])
                            if e['max_abs']:module_errors.append(dict(layer=i,module=name,row=b,**e))
                return record
            if i<16:
                for name,module in block.named_modules():
                    if name:handles.append(module.register_forward_hook(hook(name)))
        ...
        result=dict(lengths=lens,adapter=adapter,prefill_kl=pref,decode_kl=[kl(a,actual[i:i+1]) for i,a in enumerate(expected)],layers=logs,module_errors=module_errors)
        suffix=MODE
        ...
        return {k:v for k,v in result.items() if k not in ['layers','module_errors']}|dict(mode=MODE,first_module_errors=module_errors[:8])
...
def main():
    ...
    results=[]
    for MODE in ['baseline','no_cudnn','math_sdp']:
        torch.backends.cudnn.enabled=MODE=='baseline'
        context=torch.nn.attention.sdpa_kernel(torch.nn.attention.SDPBackend.MATH) if MODE=='math_sdp' else __import__('contextlib').nullcontext()
        with context:results.append(case(r,ids,[700,719],True))
        gc.collect();torch.cuda.empty_cache();save(ROOT/'diagnostic_summary.json',results)
