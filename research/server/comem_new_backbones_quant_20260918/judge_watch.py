    ...
    while not (OUT/'STOP').exists():
        ...
        if ids!=last_set:
            ...
            for r in rows:
                ...
                reused+=1
            if inputs:
                write_json(OUT/'watch_status.json',dict(phase='JUDGING',available=len(rows),reused_original_cache=reused,workers=JUDGE_WORKERS,pid=os.getpid(),at=time.time()))
                report=run_batch(inputs,OUT,protocol,workers=JUDGE_WORKERS,classifier=classify)
        ...
        write_json(OUT/'watch_status.json',dict(phase='COMPLETE' if complete else 'WAITING_GENERATIONS',
             verified_generation_tasks=len(last_set),target_generation_tasks=16,workers=JUDGE_WORKERS,pid=os.getpid(),at=time.time()))
        if complete:
