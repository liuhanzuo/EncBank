"""Encbank on a ``device_map``-sharded MoE backbone (e.g. Tencent Hunyuan Hy3,
``hy_v3``, 80-layer MoE).

Why a separate class
--------------------
:class:`~encbank.model.Encbank` assumes the whole backbone lives on ONE device (it
...
the *next* layer sits on. :class:`EncbankMoE` is that thin, device-aware subclass: it
...
class EncbankMoE(Encbank):
    """Multi-GPU (``device_map``-sharded) Encbank over a stock MoE ``*ForCausalLM``.

    Drop-in for :class:`Encbank` — same ``write_chunk`` / ``read`` / ``read_core`` /
...
def load_moe_encbank(
    ...
    """Load a sharded MoE ``*ForCausalLM`` and wrap it in :class:`EncbankMoE`.
