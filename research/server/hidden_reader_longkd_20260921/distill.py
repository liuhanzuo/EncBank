...
import gc,hashlib,json,math,random,statistics,time
...
def train_heads(api,config,root):
    ...
    prior=root.parent/'hidden_reader_four_distill_20260921/fixed_lr1e5/heads.pt'
    ...
    assert abs(validations[0]['kl']-0.05914738681167364)<1e-5,validations[0]
    best=validations[0]['kl'];best_step=0;best_weights=export()
    dump('distill_validation.json',validations)
    rng=random.Random(20260922);trace=[];begin=time.perf_counter();step=0;epoch=0
    while step<config['updates']:
        cases=dataset['train'].copy();rng.shuffle(cases)
        for case in cases:
            if step>=config['updates']:break
            step+=1
            lr=config['lr']*(0.1+0.9*0.5*(1+math.cos(math.pi*(step-1)/config['updates'])))
            ...
            with torch.enable_grad():
                ...
                optimizer.step()
            trace.append(dict(step=step,total_steps=step+256,lr=lr,epoch=epoch,id=case['id'],train_kl=float(loss.detach()),grad_norm=float(norm)))
            ...
            if step%128==0 or step==config['updates']:
            ...
            if step in [256,768,1792]:
                path=root/api['ARM']/('heads_total'+str(step+256)+'.pt')
                torch.save(dict(weights={l:tuple(t.detach().to(torch.bfloat16).cpu() for t in pair) for l,pair in params.items()},total_steps=step+256,config=config),path)
        epoch+=1
    seconds=time.perf_counter()-begin
    info=dict(**config,actual_updates=step,total_updates=step+256,selected_step=best_step,selected_total_steps=best_step+256,initial_validation_kl=validations[0]['kl'],
        ...
        selection='validation KL over initial256-step checkpoint and each128 additional steps; benchmark never used for selection',
        optimizer_scope='Warm-start model weights; optimizer reinitialized because previous optimizer state was not saved. Cosine LR decay to10% of peak.')
